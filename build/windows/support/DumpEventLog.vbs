...
Sub EVENTSINK_OnObjectReady(oItem, oAsyncContext)
    ...
    If Not bDebug Then On Error Resume Next
    Dim sInsertions, sByteDataInASCII, sByteDataInHex, sComputerName, iTimeInObject, evtDate, evtTime, timeZoneDate
    ...
    With oItem
        If bVerbose Then sInsertions = JoinStringsArray(.InsertionStrings) 'MakeCleanLine(JoinStringsArray(.InsertionStrings)) 'Mostly overlaps with the Message data.
		...
		evtDate = GetDate(.TimeGenerated)
		evtTime = GetTime(.TimeGenerated)

		if GetWinVersion < 6.0 then
			' On Windows xp or 2003, Event generation time is in local time zone.
			timeZoneDate = evtDate & " " & evtTime
		else
			' After Windows 2008(or vista), Event generation time is UTC time. We should adjust it by local time zone.
			timeZoneDate = GetTimeZoneDate(evtDate & " " & evtTime)
		end if

		oRecordSet.AddNew 
        oRecordSet.Fields("Computer").Value = sIPaddress
        oRecordSet.Fields("Date").Value = Quote(DateValue(timeZoneDate))  'Need to sort on these date/time fields, but ADO reformats them when GetString-ing the data.
        oRecordSet.Fields("Time").Value = Quote(TimeValue(timeZoneDate))
...
Sub WriteRecordSetToFile()    
	...
	' Write Header
	Wscript.Echo "Computer,Date,Time,RecordNumber,Log,User,EventCodeID,SourceName,EventIdentifier,Category,EventType,Message,InsertionStrings,ByteDataInASCII,ByteDataInHex"

    Do While Not oRecordSet.EOF
		Dim textResult 
		textResult = oRecordSet.GetString(2, 1, ",", vbCrLf, "")
		
		If (textResult <> "") And (VarType(textResult) <> vbNull) And (Len(textResult) <> 0) Then 
			textResult = Replace(textResult, vbCrLf, "")
			Wscript.Echo textResult
		End if
    Loop
...
Sub ShowUsageAndQuit()
    ...
    sUsage = sUsage & vbCrLf
    sUsage = sUsage & "    Verbose mode includes insertion strings and byte array data in text/hex." & vbCrLf
...
Function GetDate(sWmiDate)
    ...
    GetDate = Mid(sWmiDate,5,2) & "/" & Mid(sWmiDate,7,2) & "/" & Mid(sWmiDate,1,4) 
    GetDate = DateValue(GetDate)  'RecordSet needs a VB Date value.  
End Function



' Ajdusts date time value by current time zone.
' utcDate - utc date value. e.g.) 2010-10-5 AM 09:00:00
Function GetTimeZoneDate(utcDate)
	Dim iOffset
	Dim WshShell
	
	set WshShell = WScript.CreateObject("WScript.Shell")
	iOffset = WshShell.RegRead("HKLM\System\CurrentControlSet\Control\TimeZoneInformation\ActiveTimeBias")
	' iOffset is to go to UTC time from the current time zone. So, we needs to
	' get a negative value of it.
	GetTimeZoneDate = DateAdd("n", -iOffset, utcDate)
End Function


' Retrieves windows version of this system.
Function GetWinVersion()
	Dim WshShell

	set WshShell = WScript.CreateObject("WScript.Shell")
	GetWinVersion = WshShell.RegRead("HKEY_LOCAL_MACHINE\Software\Microsoft\Windows NT\CurrentVersion\CurrentVersion")
End Function
