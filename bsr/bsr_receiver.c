...
#endif
{
	...
	if (i < 0) {
		bsr_warn(connection, "ASSERTION FAILED: %s: %d < 0",
...
bsr_alloc_peer_req(struct bsr_peer_device *peer_device, gfp_t gfp_mask) __must_hold(local)
{
	...
	if (!peer_req) {
		if (!(gfp_mask & __GFP_NOWARN))
			bsr_err(device, "%s: allocation failed", __func__);
		return NULL;
	}
...
static int bsr_recv(struct bsr_connection *connection, void **buf, size_t size, int flags)
{
	...
	if (rv < 0) {
		if (rv == -ECONNRESET)
			bsr_info(connection, "sock was reset by peer");
		else if (rv != -ERESTARTSYS)
			bsr_info(connection, "sock_recvmsg returned %d", rv);
	} else if (rv == 0) {
		...
		bsr_info(connection, "sock was shut down by peer");
	}
...
static int bsr_recv_all_warn(struct bsr_connection *connection, void **buf, size_t size)
{
	...
	if (err && !signal_pending(current))
		bsr_warn(connection, "short read (expected size %d)", (int)size);
	return err;
}
...
start:
	...
	if (err == -EAGAIN) {
		...
		if (connection->cstate[NOW] == C_NETWORK_FAILURE || connection->cstate[NOW] == C_CONNECTING){
			bsr_warn(connection, "cstate is C_NETWORK_FAILURE or C_CONNECTING now goto retry, err=%d", err);
			goto retry;
		}
		else
			bsr_warn(connection, "Failed to initiate connection, err=%d", err);
		goto abort;
	}
	...
	* or the challenge respons authentication could be garbled. */
	...
	if (connection->cram_hmac_tfm) {
		...
		case -1:
			bsr_err(connection, "Authentication of peer failed");
			goto abort;
		case 0:
			bsr_err(connection, "Authentication of peer failed, trying again.");
			goto retry;
		}
	}
	...
	if (!connection->ack_sender) {
		bsr_err(connection, "Failed to create workqueue ack_sender");
		goto abort;
	}
...
retry:
	...
	if (get_t_state(&connection->receiver) == RESTARTING) {
		bsr_warn(connection, "could not retry connection since receiver is restarting");
		return false;
	}
	goto start;
...
int decode_header(struct bsr_connection *connection, void *header, struct packet_info *pi)
{
	...
	if (header_size == sizeof(struct p_header100) &&
	    *(__be32 *)header == cpu_to_be32(BSR_MAGIC_100)) {
		...
		if (h->pad != 0) {
			bsr_err(connection, "Header padding is not zero");
			return -EINVAL;
		}
		...
		bsr_err(connection, "Wrong magic value 0x%08x in protocol version %d",
...
#endif
{
	...
	if (atomic_read(&g_featurelog_flag) & FEATURELOG_FLAG_LATENCY)
		bsr_latency(device, "flush IO latency : minor(%u) %lldus", device->minor, timestamp_elapse(bio->flush_ts, timestamp()));
	...
	if (error) {
		...
		if (ctx)
			ctx->error = error;
		bsr_err(device, "local disk FLUSH FAILED with status %08X", error);
	}
...
static void submit_one_flush(struct bsr_device *device, struct issue_flush_context *ctx)
{
	...
	if (!bio || !octx) {
		bsr_warn(device, "Could not allocate a bio, CANNOT ISSUE FLUSH");
...
#endif
			if (ret == -BSR_SIGKILL) {
				bsr_warn(resource, "thread signaled and no more wait pending:%d, barrier_nr:%lld, primary_node_id:%d", 
					       ...
					       enum epoch_event ev)
{
	...
	if (schedule_flush) {
		...
		if (fw) {
			...
			bsr_warn(resource, "Could not kmalloc a flush_work obj");
...
enum write_ordering_e wo) __must_hold(local)
{
	...
	if (pwo != resource->write_ordering || wo == WO_BIO_BARRIER)
		bsr_info(resource, "Method to ensure write ordering: %s", write_ordering_str[resource->write_ordering]);
}
...
struct bsr_peer_request *peer_req)
{
	...
	bsr_err(device, "received unsupported WRITE_SAME request");
	peer_req->flags |= EE_WAS_ERROR;
	bsr_endio_write_sec_final(peer_req);
#else
	struct block_device *bdev = device->ldev->backing_bdev;
...
static void conn_wait_ee_empty_and_update_timeout(struct bsr_connection *connection, struct list_head *head)
{
	...
	for (;;) {
		...
		if (ee_before_cnt == ee_after_cnt) {
			bsr_debug(connection, "ee not empty, count(%llu), wait time(%u)", (unsigned long long)ee_after_cnt, wait_cnt * CONN_WAIT_TIMEOUT);
			break;
		}

		bsr_debug(connection, "ee count, before(%llu) : after(%llu), wait time(%u)", (unsigned long long)ee_before_cnt, (unsigned long long)ee_after_cnt, wait_cnt * CONN_WAIT_TIMEOUT);
	...
	const int fault_type)
{
	...
	if (test_bit(CONNECTION_ALREADY_FREED, &peer_req->peer_device->flags)) {
		bsr_info(device, "node-id : %d, flag : CONNECTION_ALREADY_FREED", peer_req->peer_device->node_id);
		return 0;
	}
...
next_bio:
	...
	*/
	if (!(op == REQ_OP_WRITE || op == REQ_OP_READ)) {
		bsr_err(device, "Invalid bio op received: 0x%x", op);
		err = -EINVAL;
		goto fail;
	}
...
#endif
	if (!bio) {
		bsr_err(device, "submit_ee: Allocation of a bio failed (nr_pages=%u)", nr_pages);
		goto fail;
	}
	...
	page_chain_for_each(page) {
		...
		if (off > PAGE_SIZE || len > PAGE_SIZE - off || len > data_size || len == 0) {
			bsr_err(device, "invalid page chain: offset %u size %u remaining data_size %u",
				off, len, data_size);
			err = -EINVAL;
			goto fail;
		}
...
int w_e_reissue(struct bsr_work *w, int cancel) __releases(local)
{
	...
	if (previous_epoch(peer_device->connection, peer_req->epoch))
		bsr_warn(device, "Write ordering was not enforced (one time event)");
	...
	default:
		...
		bsr_free_peer_req(peer_req);
		bsr_err(device, "submit failed, triggering re-connect");
		return err;
	}
}
...
static int receive_Barrier(struct bsr_connection *connection, struct packet_info *pi)
{
	...
	if (!epoch) {
		bsr_warn(connection, "Allocation of an epoch failed, slowing down");
...
struct bsr_peer_request_details *d, struct packet_info *pi)
{
	...
	bsr_debug(NO_OBJECT, "sector: %llu block_id: %llu peer_seq: %u dp_flags:%u length:%u bi_size:%u digest_size: %u", 
...
read_in_block(struct bsr_peer_device *peer_device, struct bsr_peer_request_details *d) __must_hold(local)
{
	...
	if (d->sector + (d->bi_size >> 9) > capacity) {
		bsr_err(device, "request from peer beyond end of local disk: "
...
#ifdef _WIN
		bsr_info(device, "Fault injection: Not supported");
#else // _LIN
		struct page *page;
		unsigned long *data;
		bsr_err(device, "Fault injection: Corrupting data on receive, sector %llu",
			d->sector);
	...
	if (d->digest_size) {
		...
		if (memcmp(dig_in, dig_vv, d->digest_size)) {
			bsr_err(device, "Digest integrity check FAILED: %llus +%u",
				d->sector, d->bi_size);
			goto fail;
		}
	}
	...
	sector_t sector, int data_size)
{
	...
	if (digest_size) {
		...
		if (memcmp(dig_in, dig_vv, digest_size)) {
			bsr_err(peer_device, "Digest integrity check FAILED. Broken NICs?");
			return -EINVAL;
		}
	}
	...
	list_for_each_entry_ex(struct bsr_resync_pending_sectors, target, &(peer_device->device->resync_pending_sectors), pending_sectors) {
		if (est <= target->sst) {
			...
			*cst = est;
			...
			bsr_info(peer_device, "dup all out of sync sectors %llu ~ %llu => source %llu ~ %llu, target %llu ~ %llu",
			...
			*cst = target->sst;
			...
			bsr_info(peer_device, "dup front out of sync sectors %llu ~ %llu => source %llu ~ %llu, target %llu ~ %llu",
			...
			*cst = (target->est < est ? target->est : est);
			...
			bsr_info(peer_device, "dup end out of sync sectors %llu ~ %llu => source %llu ~ %llu, target %llu ~ %llu",
		...
		for (i_bb = peer_req->s_bb; i_bb < e_next_bb; i_bb++) {
			if (bsr_bm_test_bit(peer_device, i_bb) == 1) {
				...
				complete_end_sync:
					...
					bsr_debug(peer_device, "--set in sync, bitmap bit start : %llu, range : %llu ~ %llu, size %llu, count %d",
				...
				complete_end_out_of_sync:
					...
					peer_req->i.sector = BM_BIT_TO_SECT(s_bb);
					peer_req->i.size = (unsigned int)BM_BIT_TO_SECT(i_bb - s_bb) << 9;
					bsr_err(peer_device, "--set failed to I/O, bitmap bit start : %llu, range : %llu ~ %llu, size %llu",
...
static bool check_unmarked_and_processing(struct bsr_peer_device *peer_device, struct bsr_peer_request *peer_req)
{
	...
	if (peer_req->unmarked_count && 
		...
		bsr_debug(peer_device, "--finished unmarked s_bb(%llu), e_bb(%llu), sector(%llu), res(%s)", 
...
static int split_e_end_resync_block(struct bsr_work *w, int unused)
{
	...
	bsr_debug(peer_device, "--bitmap bit : %llu ~ %llu", BM_SECT_TO_BIT(peer_req->i.sector), (BM_SECT_TO_BIT(peer_req->i.sector + (peer_req->i.size >> 9)) - 1));
...

{
	...
	if(!data) {
		bsr_err(peer_device, "Failed to allocate buffer size(%u) to get page data.", size);
		bsr_free_peer_req(split_peer_request);
	...
	bsr_debug(peer_device, "##split request s_bb(%llu), e_bb(%llu), sector(%llu), offset(%llu), size(%u)", 
	...
	for (ibb = s_bb; ibb < e_next_bb; ibb++) {
		...
		if (is_marked_rl_bb(peer_device, &marked_rl, ibb)) {
			...
			*e_oos = ibb;
			...
			bsr_debug(peer_device, "##find in sync bitmap bit : %llu, start (%llu) ~ end (%llu)",
...
static int split_recv_resync_read(struct bsr_peer_device *peer_device, struct bsr_peer_request_details *d) __releases(local)
{
	...
	if (!peer_req) {
		bsr_err(peer_device, "failed to allocate peer_req");
		return -EIO;
	}
	...
	if (d->bi_size < BM_BLOCK_SIZE) {
		bsr_warn(peer_device, "bug FIMXME!! bi_size(%lu) < BM_BLOCK_SIZE", (unsigned long)d->bi_size);
	}
	...
	if (peer_device->connection->agreed_pro_version >= 113 &&         
		...
		if (!split_count) {
			bsr_err(peer_device, "failed to allocate split count");
			return -ENOMEM;
		}
			...
			for (i_bb = offset = s_bb; i_bb < e_next_bb; i_bb++) {
				...
				if (is_marked_bb || already_in_sync_bb) {
					if (is_all_sync) {
						...
						bsr_debug(peer_device, "##all, sync bitmap(%llu), start : %llu, end :%llu", 
				...
				submit_peer:
					...
					if (s_split_request) {
						bsr_debug(peer_device, "##sync bitmap bit %llu, split request %llu ~ %llu, size %llu, start(%llu) ~ end(%llu), end out of sync(%llu)",
						...
						if (!split_peer_req) {
							bsr_err(peer_device, "failed to alloc split_peer_req, %llu", (unsigned long long)i_bb);
							err = -ENOMEM;
						...
						if (!is_sync_target(peer_device)) {
							...
							for_each_peer_device(pd, device) {
								if (pd != peer_device && is_sync_target(pd)) {
									bsr_info(peer_device, "resync with other nodes in progress");
					...
					if (marked_rl) {
						...
						if (!unmarked_count) {
							bsr_err(peer_device, "failed to allocate unmakred_count");
						...
						if (!failed_unmarked) {
							bsr_err(peer_device, "failed to allocate failed_unmarked");
						...
						for (i = 0; i < sizeof(marked_rl->marked_rl) * 8; i++) {
							...
							if (!(marked_rl->marked_rl & 1 << i)) {
								...
								if (!split_peer_req) {
									bsr_err(peer_device, "failed to allocate split_peer_req, %llu", (unsigned long long)i_bb);
								...
								bsr_debug(peer_device, "##unmarked bb(%llu), sector(%llu), offset(%d), count(%d)", 
									(unsigned long long)marked_rl->bb, 
									(unsigned long long)BM_BIT_TO_SECT(marked_rl->bb) + i, i, atomic_read(unmarked_count));
								if (!bsr_submit_peer_request(device, split_peer_req, REQ_OP_WRITE, 0, BSR_FAULT_RS_WR) == 0) {
									bsr_err(device, "unmarked, submit failed, triggering re-connect");
									// DW-1923 for interparameter synchronization, an additional 1 was added for the remaining count and modified to use atomic_dec_return.
									atomic_set(unmarked_count, atomic_read(unmarked_count) - (atomic_read(unmarked_count) - submit_count) + 1);
									if (unmarked_count && 0 == atomic_dec_return(unmarked_count)) {
										kfree(failed_unmarked);
	     ...
	     sector_t sector, bool missing_ok, const char *func)
{
	...
	if (bsr_contains_interval(root, sector, &req->i) && req->i.local)
		return req;
	if (!missing_ok) {
		bsr_err(device, "%s: failed to find request 0x%llx, sector %llus", func,
...
static int receive_bm_exchange_state(struct bsr_connection *connection, struct packet_info *pi) {
	...
	case B_COMPLETE:
		...
		bsr_info(peer_device, "bitmap exchange complete");
		break;
	default:
		bsr_err(peer_device, "unknown error(%u)", state);
		break;
	}
	return 0;
}
...
static int receive_RSDataReply(struct bsr_connection *connection, struct packet_info *pi)
{
	...
	if (get_ldev(device)) {
		...
		if (bsr_ratelimit())
			bsr_err(device, "Can not write resync data to local disk.");
...
static int wait_for_and_update_peer_seq(struct bsr_peer_device *peer_device, const u32 peer_seq)
{
	...
	for (;;) {
		...
		if (!timeout) {
			ret = -ETIMEDOUT;
			bsr_err(peer_device, "Timed out waiting for missing ack packets; disconnecting");
			break;
		}
	}
	spin_unlock(&peer_device->peer_seq_lock);
...
static int handle_write_conflicts(struct bsr_peer_request *peer_req)
{
	...
	bsr_for_each_overlap(i, &device->write_requests, sector, size) {
		...
		if (resolve_conflicts) {
			...
			if (!equal)
				bsr_alert(device, "Concurrent writes detected: "
...
static int dedup_from_resync_pending(struct bsr_peer_device *peer_device, sector_t sst, sector_t est)
{
	...
	list_for_each_entry_safe_ex(struct bsr_resync_pending_sectors, target, tmp, &(peer_device->device->resync_pending_sectors), pending_sectors) {
		if (sst <= target->sst && est >= target->est) {
			bsr_info(peer_device, "resync pending remove sector %llu(%llu) ~ %llu(%llu)",
		...
		if (sst >= target->sst && est <= target->est) {
...
#endif
			if (!pending_st) {
				bsr_err(peer_device, "failed to resync pending bits allocate, sector : %llu ~ %llu", (unsigned long long)sst, (unsigned long long)est);
				mutex_unlock(&peer_device->device->resync_pending_fo_mutex);
				return -ENOMEM;
			}
			...
			bsr_info(peer_device, "resync pending split new sector %llu(%llu) ~ %llu(%llu)",
			...
			target->est = sst;
			bsr_info(peer_device, "resync pending split sector %llu(%llu) ~ %llu(%llu)",
				(unsigned long long)target->sst, (unsigned long long)BM_SECT_TO_BIT(target->sst), (unsigned long long)target->est, (unsigned long long)BM_SECT_TO_BIT(target->est));
		}
		...
		if (sst <= target->sst && est > target->sst && est <= target->est) {
			...
			target->sst = est;
			bsr_info(peer_device, "resync pending modify sector %llu(%llu) ~ %llu(%llu)",
				(unsigned long long)target->sst, (unsigned long long)BM_SECT_TO_BIT(target->sst), (unsigned long long)target->est, (unsigned long long)BM_SECT_TO_BIT(target->est));
		}
		...
		if ((BM_BIT_TO_SECT(s_bb) != sst || (BM_BIT_TO_SECT(s_bb) == sst && s_bb == e_bb)) &&
			...
			if (!s_marked_rl) {
				...
				if (s_marked_rl != NULL) {
					...
					bsr_err(peer_device, "failed to marked replicate allocate, bit : %llu", (unsigned long long)s_bb);
					return -ENOMEM;
				}
			}
			...
			bsr_debug(peer_device, "sbb marking bb(%llu), ssector(%llu), sector(%llu), size(%u), marked(%u), offset(%u)",
		...
		if (s_bb != e_bb && BM_BIT_TO_SECT(BM_SECT_TO_BIT(est)) != est &&
			...
			if (!e_marked_rl) {
				...
				if (e_marked_rl != NULL) {
					...
					bsr_err(peer_device, "failed to marked replicate allocate, bit : %llu", (unsigned long long)e_bb);
					return -ENOMEM;
				}
			}
			...
			bsr_debug(peer_device, "marking bb(%llu), esector(%llu), sector(%llu), size(%u), marked(%u), offset(%u)",
...
static int receive_Data(struct bsr_connection *connection, struct packet_info *pi)
{
	...
	if(get_t_state(&connection->ack_receiver) != RUNNING) {
		bsr_info(NO_OBJECT,"ack_receiver is not running... bump mirrored data");
		return 0;
	}
	...
	* Remember the op_flags. */
		...
		* "synchronously" from receiver context */
		if (peer_req->flags & (EE_IS_TRIM | EE_WRITE_SAME | EE_IS_BARRIER)) {
			...
			if (err) {
				...
				bsr_err(peer_device, "disconnect during al begin io (err = %d)", err);
				if (peer_req->flags & EE_SEND_WRITE_ACK) {
					dec_unacked(peer_device);
				}
	...
	if (!err) {	// DW-1012 The data just received is the newest, ignore previously received out-of-sync.
		...
		bsr_info(NO_OBJECT,"receive_Data connection->last_dagtag_sector:%llx ack_receiver thread state:%d",connection->last_dagtag_sector, get_t_state(&connection->ack_receiver));
#endif
		return err;
	}

	/* don't care for the reason here */
	bsr_err(peer_device, "submit failed, triggering re-connect");
	bsr_al_complete_io(device, &peer_req->i);
...
static int receive_DataRequest(struct bsr_connection *connection, struct packet_info *pi)
{
	...
	if (size <= 0 || !IS_ALIGNED(size, 512) || size > BSR_MAX_BIO_SIZE) {
		bsr_err(device, "%s:%d: sector: %llus, size: %d", __FILE__, __LINE__,
				(unsigned long long)sector, size);
		return -EINVAL;
	}
	...
	if (!get_ldev_if_state(device, min_d_state)) {
		...
		if (verb && bsr_ratelimit())
			bsr_err(device, "Can not satisfy peer's read request, "
			    "no local data.");
...
static int bsr_asb_recover_0p(struct bsr_peer_device *peer_device) __must_hold(local)
{
	...
	case ASB_VIOLENTLY:
		bsr_err(peer_device, "Configuration error.");
		break;
	case ASB_DISCONNECT:
		break;
	...
	case ASB_DISCARD_OLDER_PRI:
		...
		/* Else fall through to one of the other strategies... */
		bsr_warn(peer_device, "Discard younger/older primary did not find a decision"
...
static int bsr_asb_recover_1p(struct bsr_peer_device *peer_device) __must_hold(local)
{
	...
	case ASB_DISCARD_REMOTE:
	case ASB_DISCARD_ZERO_CHG:
		bsr_err(device, "Configuration error.");
		break;
	case ASB_DISCONNECT:
		break;
	case ASB_CONSENSUS:
	...
	case ASB_CALL_HELPER:
		...
		if (hg == -2 && resource->role[NOW] == R_PRIMARY) {
			...
			if (rv2 != SS_SUCCESS) {
				...
				bsr_warn(device, "Successfully gave up primary role.");
				rv = hg;
			}
		} else
			rv = hg;
...
static void bsr_uuid_dump_self(struct bsr_peer_device *peer_device, u64 bits, u64 flags)
{
	...
	bsr_info(peer_device, "self %016llX:%016llX:%016llX:%016llX bits:%llu flags:%llX",
...
static void bsr_uuid_dump_peer(struct bsr_peer_device *peer_device, u64 bits, u64 flags)
{
	...
	bsr_info(peer_device, "peer %016llX:%016llX:%016llX:%016llX bits:%llu flags:%llX",
...
static int uuid_fixup_resync_end(struct bsr_peer_device *peer_device, int *rule_nr) __must_hold(local)
{
	...
	if (peer_device->bitmap_uuids[node_id] == (u64)0 && bsr_bitmap_uuid(peer_device) != (u64)0) {
		...
		if ((bsr_bitmap_uuid(peer_device) & ~UUID_PRIMARY) ==
			...
			bsr_info(device, "was SyncSource, missed the resync finished event, corrected myself:");
			...
			*rule_nr = 34;
			...
			bsr_info(device, "was SyncSource (peer failed to write sync_uuid)");
			*rule_nr = 36;
		}
	...
	if (bsr_bitmap_uuid(peer_device) == (u64)0 && peer_device->bitmap_uuids[node_id] != (u64)0) {
		...
		if ((bsr_history_uuid(device, 0) & ~UUID_PRIMARY) ==
			...
			bsr_info(device, "was SyncTarget, peer missed the resync finished event, corrected peer:");
			...
			*rule_nr = 35;
			...
			bsr_info(device, "was SyncTarget (failed to write sync_uuid)");
			*rule_nr = 37;
		}
	...
	if (self == peer) {
		if (peer_device->connection->agreed_pro_version < 96 ?
			...
			*rule_nr = 51;
			...
			bsr_info(device, "Lost last syncUUID packet, corrected:");
			bsr_uuid_dump_peer(peer_device, peer_device->dirty_bits, peer_device->uuid_flags);
	...
	if (self == peer) {
		if (peer_device->connection->agreed_pro_version < 96 ?
			...
			*rule_nr = 71;
			...
			bsr_info(device, "Last syncUUID did not get through, corrected:");
			bsr_uuid_dump_self(peer_device,
					    device->disk_state[NOW] >= D_NEGOTIATING ? bsr_bm_total_weight(peer_device) : 0, 0);
...
static void log_handshake(struct bsr_peer_device *peer_device)
{
	...
	bsr_info(peer_device, "bsr_sync_handshake:");
			  ...
			  bool always_verbose) __must_hold(local)
{
	...
	if (hg || always_verbose)
		bsr_info(peer_device, "uuid_compare()=%d by rule %d", hg, *rule_nr);
...
static int bitmap_mod_after_handshake(struct bsr_peer_device *peer_device, int hg, int peer_node_id)
{
	...
	if (hg == 4) {
		...
		bsr_info(peer_device, "Peer synced up with node %d, copying bitmap", peer_node_id);
...
#if 0 // DW-1099 copying bitmap has a defect, do sync whole out-of-sync until fixed.
		bsr_info(peer_device, "synced up with node %d in the mean time", peer_node_id);
		bsr_suspend_io(device, WRITE_ONLY);
						   ...
						   int hg)
{
	...
	if (hg > 0) { /* become sync source. */
		...
		if (bsr_bitmap_uuid(peer_device)) {
			bsr_info(peer_device, "clearing bitmap UUID and bitmap content (%llu bits)",
			 ...
			 */
			if (bsr_md_test_peer_flag(peer_device, MDF_PEER_PRIMARY_IO_ERROR)) {
				bsr_info(peer_device, "this node contain the oos generated by io-errors at primary.");
				rv = L_WF_BITMAP_S;
			}
			else if (peer_device->uuid_flags & UUID_FLAG_PRIMARY_IO_ERROR) {
				bsr_info(peer_device, "peer node contains the oos generated by io-errors at primary.");
				rv = L_WF_BITMAP_T;
			}
			else {
				...
				if (bsr_md_test_peer_flag(peer_device, MDF_PEER_IN_PROGRESS_SYNC) ||
					...
					bsr_info(peer_device, "ended during synchronization and completed resync with other nodes, clearing bitmap UUID and bitmap content (%llu bits)",
						(unsigned long long)bsr_bm_total_weight(peer_device));
					bsr_uuid_set_bitmap(peer_device, 0);
					bsr_bm_clear_many_bits(peer_device->device, peer_device->bitmap_index, 0, BSR_END_OF_BITMAP);
				}
				else {
					bsr_info(peer_device, "No resync, but %llu bits in bitmap!",
						(unsigned long long)bsr_bm_total_weight(peer_device));
				}
			}
		}
				    ...
				    int *hg, int rule_nr)
{
	...
	if (p)
		bsr_info(device, "Becoming sync %s due to disk states.",
			  *hg > 0 ? "source" : "target");
}
						...
						int *hg)
{
		...
		peer_device->dirty_bits == 0)
	{
		...
		bsr_info(peer_device, "FIXME, both nodes are UpToDate, but have inconsistent bits set. clear it without resync");
		...
		{
			bsr_info(peer_device, "bitmap will be cleared due to inconsistent out-of-sync");
...
out:
	if (*hg)
		bsr_info(device, "Becoming sync %s due to %s.",
		*hg > 0 ? "source" : "target",
						...
						enum bsr_disk_state peer_disk_state) __must_hold(local)
{
	...
	if (hg == -1000) {
		bsr_alert(device, "Unrelated data, aborting!");
		return -1;
	}
	if (hg < -1000) {
		bsr_alert(device, "To resolve this both sides have to support at least protocol %d", -hg - 1000);
		return -1;
	}
	...
	*/
	...
	if (hg <= -2 && /* by intention we do not use disk_state here. */
		...
		case ASB_CALL_HELPER:
			...
			/* fall through */
		case ASB_DISCONNECT:
			bsr_err(device, "I shall become SyncTarget, but I am primary!");
			return -1;
		case ASB_VIOLENTLY:
			bsr_warn(device, "Becoming SyncTarget, violating the stable-data"
			     "assumption");
		}
	}
	...
	if (hg == 3 && device->disk_state[NOW] < D_OUTDATED && 
		...
		bsr_err(device, "I shall become SyncSource, but I am inconsistent!");
		return -1;
	}

	if (hg == -3 && peer_device->uuid_flags & UUID_FLAG_INCONSISTENT) {
		bsr_err(device, "I shall become SyncTarget, but peer is inconsistent!");
		return -1;
	}	

	if (test_bit(CONN_DRY_RUN, &connection->flags)) {
		if (hg == 0)
			bsr_info(device, "dry-run connect: No resync, would become Connected immediately.");
		else
			bsr_info(device, "dry-run connect: Would become %s, doing a %s resync.",
...
static int receive_protocol(struct bsr_connection *connection, struct packet_info *pi)
{
	...
	if (pi->cmd != P_PROTOCOL_UPDATE) {
		...
		if (p_proto != (int)nc->wire_protocol) {
			bsr_err(connection, "incompatible %s settings", "protocol");
			goto disconnect_rcu_unlock;
		}

		if (convert_after_sb(p_after_sb_0p) != (int)nc->after_sb_0p) {
			bsr_err(connection, "incompatible %s settings", "after-sb-0pri");
			goto disconnect_rcu_unlock;
		}

		if (convert_after_sb(p_after_sb_1p) != (int)nc->after_sb_1p) {
			bsr_err(connection, "incompatible %s settings", "after-sb-1pri");
			goto disconnect_rcu_unlock;
		}

		if (convert_after_sb(p_after_sb_2p) != (int)nc->after_sb_2p) {
			bsr_err(connection, "incompatible %s settings", "after-sb-2pri");
			goto disconnect_rcu_unlock;
		}
		...
		if (p_two_primaries != nc->two_primaries) {
			bsr_err(connection, "incompatible %s settings", "allow-two-primaries");
			goto disconnect_rcu_unlock;
		}

		if (strcmp(integrity_alg, nc->integrity_alg)) {
			bsr_err(connection, "incompatible %s settings", "data-integrity-alg");
			goto disconnect_rcu_unlock;
		}
#ifdef _WIN
		// RCU_SPECIAL_CASE
	...
	if (integrity_alg[0]) {
		...
		if (IS_ERR(peer_integrity_tfm)) {
			...
			bsr_err(connection, "peer data-integrity-alg %s not supported",
				 integrity_alg);
			goto disconnect;
		}
		...
		if (!(int_dig_in && int_dig_vv)) {
			bsr_err(connection, "Allocation of buffers for data integrity checking failed");
			goto disconnect;
		}
	}
	...
	if (!new_net_conf) {
		bsr_err(connection, "Allocation of new net_conf failed");
		goto disconnect;
	}
	...
	*new_net_conf = *old_net_conf;
	...
	if (strcmp(old_net_conf->integrity_alg, integrity_alg))
		bsr_info(connection, "peer data-integrity-alg: %s",
...
#endif
	if (IS_ERR(tfm)) {
		bsr_err(device, "Can not allocate \"%s\" as %s (reason: %ld)",
...
{
	bsr_warn(connection, "%s packet received for volume %d, which is not configured locally",
...
static int receive_SyncParam(struct bsr_connection *connection, struct packet_info *pi)
{
	...
	if (get_ldev(device)) {
		...
		if (!new_peer_device_conf) {
			...
			bsr_err(device, "Allocation of new peer_device_conf failed");
			return -ENOMEM;
		}
	...
	if (apv >= 88) {
		if (apv == 88) {
			if (data_size > SHARED_SECRET_MAX || data_size == 0) {
				bsr_err(device, "verify-alg too long, "
		...
		if (strcmp(old_net_conf->verify_alg, p->verify_alg)) {
			if (peer_device->repl_state[NOW] == L_OFF) {
				bsr_err(device, "Different verify-alg settings. me=\"%s\" peer=\"%s\"",
		...
		if (apv >= 89 && strcmp(old_net_conf->csums_alg, p->csums_alg)) {
			if (peer_device->repl_state[NOW] == L_OFF) {
				bsr_err(device, "Different csums-alg settings. me=\"%s\" peer=\"%s\"",
...
#endif
				if (!new_plan) {
					bsr_err(device, "kmalloc of fifo_buffer failed");
					goto disconnect;
				}
			}
		}
		...
		if (verify_tfm || csums_tfm) {
			...
			if (!new_net_conf) {
				bsr_err(device, "Allocation of new net_conf failed");
				goto disconnect;
			}
			...
			*new_net_conf = *old_net_conf;
			...
			if (verify_tfm) {
				...
				bsr_info(device, "using verify-alg: \"%s\"", p->verify_alg);
			}
			if (csums_tfm) {
				...
				bsr_info(device, "using csums-alg: \"%s\"", p->csums_alg);
			}
#ifdef _WIN
			synchronize_rcu_w32_wlock();
#endif
	...
	const char *s, sector_t a, sector_t b)
{
	...
	d = (a > b) ? (a - b) : (b - a);
	if (d > (a>>3) || d > (b>>3))
		bsr_warn(peer_device, "Considerable difference in %s: %llu bytes vs. %llu bytes", s,
...
static void maybe_trigger_resync(struct bsr_device *device, struct bsr_peer_device *peer_device, bool grew, bool skip)
{
	...
	if (test_and_clear_bit(RESIZE_PENDING, &peer_device->flags) ||
		...
		if (peer_device->disk_state[NOW] >= D_INCONSISTENT &&
			...
			if (skip)
				bsr_info(peer_device, "Resync of new storage suppressed with --assume-clean");
			else
...
static int receive_sizes(struct bsr_connection *connection, struct packet_info *pi)
{
    ...
    bsr_info(device, "current_mydisk_size: %llu bytes", (unsigned long long)(cur_size<<9));
    bsr_info(peer_device, "peer_current_size: %llu bytes peer_user_size: %llu bytes peer_disk_size: %llu bytes peer_max_size: %llu bytes",
	...
	if ((p_size && p_csize > p_size) || (p_usize && p_csize > p_usize)) {
		bsr_warn(peer_device, "Peer sent bogus sizes, disconnecting");
		goto disconnect;
	}
	...
	*/
	...
	if (get_ldev(device)) {
		...
		my_max_size = bsr_get_max_capacity(device->ldev);
		        bsr_info(peer_device, "md_effective_size: %llu my_user_size: %llu my_max_size: %llu",
		...
		if (new_size < cur_size &&
            ...
            bsr_err(peer_device, "The peer's disk size is too small! (%llu bytes < %llu bytes)",
		...
		if (my_max_size < p_csize && !is_handshake) {
			bsr_err(peer_device, "Peer's size larger than my maximum capacity (%llu < %llu sectors)",
		...
		if (my_usize != p_usize) {
			...
			if (!new_disk_conf) {
				bsr_err(device, "Allocation of new disk_conf failed");
				err = -ENOMEM;
				goto out;
			}
			...
			*new_disk_conf = *old_disk_conf;
			...
			bsr_info(peer_device, "Peer sets u_size to %llu sectors",
	...
	if (have_ldev) {
		...
		bsr_info(peer_device, "calling bsr_determine_dev_size()");		
		...
		if (size != cur_size) {
			...
			if (p_size == 0) {
				...
				bsr_info(device, "size = %s (%llu KB)", ppsize(ppb, sizeof(ppb), size >> 1),
	...
	if (device->device_conf.max_bio_size > protocol_max_bio_size ||
		...
		bsr_err(device, "Peer cannot deal with requests bigger than %u. "
			...
			enum resync_reason reason) __must_hold(local)
{
	...
	if (new_repl_state == -1) {
		bsr_info(peer_device, "Unexpected result of handshake() %d!", new_repl_state);
		...
		bitmap_mod_after_handshake(peer_device, hg, peer_node_id);
		bsr_info(peer_device, "Becoming %s %s", bsr_repl_str(new_repl_state),
	...
	if (new_repl_state == L_ESTABLISHED && peer_disk_state >= D_CONSISTENT &&
		...
		/* No resync with up-to-date peer -> I should be consistent or up-to-date as well.
		   ...
		   as well. */
		bsr_info(peer_device, "Upgrading local disk to %s after unstable/weak (and no resync).",
...
static void bsr_resync_authoritative(struct bsr_peer_device *peer_device, enum bsr_repl_state side)
{
	...
	if (new_repl_state == -1) {
		bsr_info(peer_device, "Invalid resync side %s", bsr_repl_str(side));
		return;
	}
	...
	if (abs(hg) >= 100)	{
		bsr_err(peer_device, "Can not start resync due to unexpected handshake result(%d)", hg);
		// DW-1360 destroy connection for conflicted data.
		change_cstate_ex(peer_device->connection, C_DISCONNECTING, CS_HARD);
		return;
	}

	bsr_info(peer_device, "Becoming %s due to authoritative node changed", bsr_repl_str(new_repl_state));
...
static int __receive_uuids(struct bsr_peer_device *peer_device, u64 node_mask)
{
	...
	if (peer_device->connection->agreed_pro_version < 110 && bad_server) {
		bsr_err(device, "Can only connect to data with current UUID=%016llX",
	...
	if (get_ldev(device)) {
		...
		if (skip_initial_sync) {
			...
			bsr_info(device, "Accepted new current UUID, preparing to skip initial sync");
		...
		if (resource->state_change_flags) {
			bsr_info(peer_device, "Delaying update of exposed data uuid");
...
static int receive_uuids110(struct bsr_connection *connection, struct packet_info *pi)
{
	...
	if ((peer_device->repl_state[NOW] >= L_STARTING_SYNC_S && peer_device->repl_state[NOW] <= L_WF_BITMAP_T) ||
		...
		if (!bsr_inspect_resync_side(peer_device, peer_device->repl_state[NOW], NOW, false)) {
			...
			bsr_info(peer_device, "Resync will be aborted since peer goes unsyncable.");
			begin_state_change(device->resource, &irq_flags, CS_VERBOSE);
			  ...
			  enum chg_state_flags flags)
{
	...
	if (mask.i) {
		bsr_info(connection, "Remote state change: request %u/%u not "
			   ...
			   union bsr_state mask, union bsr_state val)
{
	...
	if (mask.i) {
		bsr_info(peer_device, "Remote state change: request %u/%u not "
		"understood", mask.i, val.i & mask.i);
		return SS_NOT_SUPPORTED;
	}
	return SS_SUCCESS;
...
static int receive_req_state(struct bsr_connection *connection, struct packet_info *pi)
{
	...
	if (!expect(connection, connection->agreed_pro_version < 110)) {
		bsr_err(connection, "Packet %s not allowed in protocol version %d",
	...
	if (rv != SS_SUCCESS) {
		bsr_info(connection, "Rejecting concurrent remote state change");
...
#endif
{
	...
	if (resource->twopc_work.cb == NULL) {
		bsr_err(resource, "Two-phase commit %u timeout",
...
check_concurrent_transactions(struct bsr_resource *resource, struct twopc_reply *new_r)
{
	...
	if (new_r->initiator_node_id < ongoing->initiator_node_id) {
		if ((unsigned int)ongoing->initiator_node_id == resource->res_opts.node_id) {
			bsr_info(resource, "[TWOPC] CSC_ABORT_LOCAL! new_r->initiator_node_id (%d) ongoing->initiator_node_id (%d)",
					new_r->initiator_node_id, ongoing->initiator_node_id);
			return CSC_ABORT_LOCAL;
		}
		else {
			bsr_info(resource, "[TWOPC] CSC_QUEUE! new_r->initiator_node_id (%d) ongoing->initiator_node_id (%d)",
					new_r->initiator_node_id, ongoing->initiator_node_id);
			return CSC_QUEUE;
		}
		...
		if (ongoing->is_disconnect && new_r->tid != ongoing->tid) {
			bsr_info(resource, "[TWOPC] CSC_UPDATE! new_r->initiator_node_id (%d), new_r->tid (%u)",
				new_r->initiator_node_id, new_r->tid);
			del_timer(&resource->twopc_timer);
			clear_remote_state_change_without_lock(resource);
			return CSC_UPDATE;
		}
		bsr_info(resource, "[TWOPC] CSC_REJECT! new_r->initiator_node_id (%d) ongoing->initiator_node_id (%d)",
					new_r->initiator_node_id, ongoing->initiator_node_id);

		return CSC_REJECT;
	}
	if (new_r->tid != ongoing->tid) {
		bsr_info(resource, "[TWOPC] CSC_TID_MISS! new_r->tid (%u) ongoing->tid (%u)",
...
static int queued_twopc_work(struct bsr_work *w, int cancel)
{
	...
	while (true) {
		if (jiffies - q->start_jif >= t || cancel) {
			if (!cancel)
				bsr_info(connection, "Rejecting concurrent "
...
bsr_commit_size_change(struct bsr_device *device, struct resize_parms *rs, u64 nodes_to_reach)
{
    ...
    if (!get_ldev(device)) {
		...
		bsr_info(device, "size = %s (%llu KB)", ppsize(ppb, sizeof(ppb), tr->new_size >> 1),
	...
	if (my_usize != tr->user_size) {
        ...
        *new_disk_conf = *old_disk_conf;
        ...
        bsr_info(device, "New u_size %llu sectors",
             ...
             ULONG_PTR receive_jif)
{
	...
	if (connection == NULL)
		bsr_err(NO_OBJECT, "process_twopc connection is null");
	...
	bsr_info(resource, "[TWOPC:%u] target_node_id (%d) csc_rv (%d) primary_nodes (%llu) pi->cmd (%s)", 
	...
	if (csc_rv == CSC_CLEAR && pi->cmd != P_TWOPC_ABORT) {
		if (!is_prepare(pi->cmd)) {
			...
			spin_unlock_irq(&resource->req_lock);
			bsr_debug(connection, "Ignoring %s packet %u",
				   bsr_packet_name(pi->cmd),
				   reply->tid);
			...
			if(resource->role[NEW] == R_SECONDARY && reply->primary_nodes != 0 ) {
				struct bsr_device *device;
				int vnr;
				bsr_info(resource, "Peer node is Primary. LastPrimary flag set [TWOPC:%u] target_node_id (%d) primary_nodes (%llu) pi->cmd (%s)", 
				...
				idr_for_each_entry_ex(struct bsr_device *, &resource->devices, device, vnr) {
					if(get_ldev_if_state(device, D_NEGOTIATING)) {
						...
						bsr_info(resource, "LastPrimary got it! But disk state is diskless or failed device->disk_state:%d",device->disk_state[NEW]);
					}
				} 
			}
		...
		if (test_and_set_bit(TWOPC_EXECUTED, &resource->flags)) {
			spin_unlock_irq(&resource->req_lock);
			bsr_info(connection, "Ignoring redundant %s packet %u.",
					bsr_packet_name(pi->cmd),
					reply->tid);
			return 0;
		}
		...
		bsr_info(connection, "Aborting local state change %u to yield to remote "
			  "state change %u.",
			  resource->twopc_reply.tid,
			  reply->tid);
		...
		if (alt_rv == ALT_MATCH) {
			...
			/* abort_local_transaction() comes back unlocked in this case ... */
			bsr_info(connection, "Aborting local state change %u "
		...
		if (err) {
			...
			if (err)
				bsr_info(connection, "Ignoring %s packet %u.",
					  bsr_packet_name(pi->cmd),
					  reply->tid);
		}

		bsr_info(resource, "[TWOPC:%u] target_node_id (%d) abort_starting_twopc ", 
					reply->tid, reply->target_node_id);
			...
			match: 
				...
				if (!reply_cmd) {
					...
					if(list_add_valid(&connection->twopc_parent_list, &resource->twopc_parents)) {	
						...
						bsr_info(connection, "twopc_parent_list(%p) already added.", &connection->twopc_parent_list);
						kref_debug_put(&connection->kref_debug, 9);
						kref_put(&connection->kref, bsr_destroy_connection);
					}
				}
	...
	case P_TWOPC_PREPARE:
		bsr_info(connection, "Preparing remote state change %u "
	...
	case P_TWOPC_PREP_RSZ:
		...
		break;
	case P_TWOPC_ABORT:
		bsr_info(connection, "Aborting remote state change %u",
			  reply->tid);
		flags |= CS_ABORT;
		break;
	case P_TWOPC_COMMIT:
		bsr_info(connection, "Committing remote state change %u",
			  reply->tid);
		break;
	default:
		BUG();
	...
	if (flags & CS_PREPARE) {
		...
		if( (resource->role[NEW] != R_PRIMARY) && (reply->primary_nodes != 0) ) {
			struct bsr_device *device;
			int vnr;
			bsr_info(resource, "Peer node is Primary. LastPrimary flag set [TWOPC:%u] after clear_remote_state_change target_node_id (%d) primary_nodes (%llu) pi->cmd (%s)", 
				reply->tid, reply->target_node_id, reply->primary_nodes, bsr_packet_name(pi->cmd));
			idr_for_each_entry_ex(struct bsr_device *, &resource->devices, device, vnr) {
				if(get_ldev_if_state(device, D_NEGOTIATING)) {
					bsr_md_clear_flag (device, MDF_LAST_PRIMARY );
...
static int receive_state(struct bsr_connection *connection, struct packet_info *pi)
{
	...
	if (peer_state.disk == D_NEGOTIATING) {
		...
		bsr_info(device, "real peer disk state = %s", bsr_disk_str(peer_disk_state));
	}
 ...
 retry:
		 ...
		 * Maybe we should finish it up, too? */
			...
			if (finish_now || old_peer_state.conn == L_SYNC_SOURCE ||
...
#ifdef _DEBUG_OOS
				{
					...
					bsr_info(peer_device, "SyncSource still sees bits set!! FIXME, total(%llu), failed(%llu)", bsr_bm_total_weight(peer_device), peer_device->rs_failed);
	...
	if (connection->agreed_pro_version >= 110 &&
		...
		bsr_info(peer_device, "peer is SyncSource. change to SyncTarget"); // DW-1518
		bsr_start_resync(peer_device, L_SYNC_TARGET);
		return 0;
	}
	...
	if (peer_state.conn == L_AHEAD) {
		if (old_peer_state.conn != L_BEHIND) {
			bsr_info(peer_device, "peer is Ahead. change to Behind mode"); // DW-1518
		}
	...
	if (peer_device->uuids_received &&
		...
		if( (peer_state.role == R_PRIMARY) && (device->resource->role[NOW] == R_PRIMARY) ) {
			bsr_err(device, "2 primary is not allowed.");
			put_ldev(device);
			goto fail;
		}
		...
		if (new_repl_state == -1) {
			...
			if (device->disk_state[NOW] == D_NEGOTIATING) {
				...
				if (connection->agreed_pro_version < 110) {
					bsr_err(device, "Disk attach process on the peer node was aborted.");
		...
		if (peer_device->current_uuid == bsr_current_uuid(device)) {
			bsr_info(peer_device, "Upgrading local disk to D_UP_TO_DATE since current UUID matches.");
	...
	bsr_info(peer_device, "receive new repl state \"%s\"", bsr_repl_str(new_repl_state));
	...
	if (peer_device->disk_state[NEW] == D_CONSISTENT &&
		...
		bsr_err(device, "Aborting Connect, can not thaw IO with an only Consistent peer");
		tl_clear(connection);
	...
	if (rv < SS_SUCCESS) {
		...
		if (connection->cstate[NOW] == C_CONNECTING || rv == SS_NEED_CONNECTION){
			bsr_info(peer_device, "connection->cstate[OLD] == C_CONNECTING, change cstate to NETWORK_FAILURE instead of DISCONNECTING");
...
static int receive_sync_uuid(struct bsr_connection *connection, struct packet_info *pi)
{
	...
	if (get_ldev_if_state(device, D_NEGOTIATING)) {
		...
		bsr_err(device, "Ignoring SyncUUID packet!");
		     ...
		     struct bm_xfer_ctx *c)
{
	...
	if (want != size) {
		bsr_err(peer_device, "%s:want (%llu) != size (%llu)", __func__, (unsigned long long)want,  (unsigned long long)size);
		return -EIO;
	}
	if (want == 0)
		return 0;
		 ...
		 unsigned int len)
{
	...
	for (have = bits; have > 0; s += (ULONG_PTR)rl, toggle = !toggle) {
		...
		if (toggle) {
			e = s + (ULONG_PTR)rl -1;
			if (e >= c->bm_bits) {
				bsr_err(peer_device, "bitmap overflow (e:%lu) while decoding bm RLE packet", e);
				return -EIO;
			}
		...
		if (have < bits) {
			bsr_err(peer_device, "bitmap decoding error: h:%d b:%d la:0x%08llx l:%u/%u",
		...
		unsigned int len)
{
	...
	bsr_err(peer_device, "receive_bitmap_c: unknown encoding %u", p->encoding);
		...
		const char *direction, struct bm_xfer_ctx *c)
{
	...
	r = 1000 - r;
	bsr_info(peer_device, "%s bitmap stats [Bytes(packets)]: plain %u(%u), RLE %u(%u), "
...
static int receive_bitmap_finished(struct bsr_connection *connection, struct bsr_peer_device *peer_device, bool *unlocked)
{
	...
	*unlocked = false;
	...
	if (peer_device->repl_state[NOW] == L_WF_BITMAP_T ||
		...
		bsr_info(peer_device, "unexpected repl_state (%s) in receive_bitmap",
	...
	if (peer_device->repl_state[NOW] == L_WF_BITMAP_S ||
		...
		if (connection->agreed_pro_version >= 113) {
			...
			bsr_info(peer_device, "send that bitmap exchange has been completed");
...
static int receive_bitmap(struct bsr_connection *connection, struct packet_info *pi)
{
	...
	if (peer_device->bitmap_index == -1) {
		bsr_err(peer_device, "No bitmap allocated in receive_bitmap()!");
		return -EIO;
	}
	device = peer_device->device;
	...
	if (pi->cmd == P_BITMAP) {
		...
		* and the feature is enabled! */
		...
		if (pi->size > BSR_SOCKET_BUFFER_SIZE - bsr_header_size(connection)) {
			bsr_err(device, "ReportCBitmap packet too large");
			err = -EIO;
			goto out;
		}
		if (pi->size <= sizeof(*p)) {
			bsr_err(device, "ReportCBitmap packet too small (l:%u)", pi->size);
			err = -EIO;
			goto out;
		}
		...
		bsr_warn(device, "receive_bitmap: cmd neither ReportBitMap nor ReportCBitMap (is 0x%x)", pi->cmd);
		err = -EIO;
		goto out;
	}
...
{
	bsr_warn(connection, "skipping unknown optional packet type %d, l: %d!",
		 pi->cmd, pi->size);
...
static int list_add_resync_pending(struct bsr_device* device, sector_t sst, sector_t est)
{
	...
	if (pending_st) {
		...
		if (!pending_st) {
			bsr_err(device, "failed to resync pending bits allocate, sector : %llu ~ %llu", (unsigned long long)sst, (unsigned long long)est);
...
eof:
	list_for_each_entry_ex(struct bsr_resync_pending_sectors, target, &(device->resync_pending_sectors), pending_sectors)
		bsr_info(device, "%d. resync pending sector %llu(%llu) ~ %llu(%llu)", i++, (unsigned long long)target->sst, (unsigned long long)BM_SECT_TO_BIT(target->sst), (unsigned long long)target->est, (unsigned long long)BM_SECT_TO_BIT(target->est));
	mutex_unlock(&device->resync_pending_fo_mutex);
...
static int receive_out_of_sync(struct bsr_connection *connection, struct packet_info *pi)
{
	...
	if (bResetTimer) {
		bsr_info(peer_device, "received out-of-sync has been set after resync timer has been expired, restart timer to send rs request for rest");
	...
	if (new_repl_state != L_ESTABLISHED) {
...
#ifdef _TRACE_PEER_DAGTAG
		bsr_info(connection, "Reconciliation resync because \'%s\' disappeared. (o=%d) lost_peer:%p lost_peer->last_dagtag_sector:0x%llx be64_to_cpu(p->dagtag):%llx",
			  lost_peer->transport.net_conf->name, (int)dagtag_offset, lost_peer, lost_peer->last_dagtag_sector, be64_to_cpu(p->dagtag));
#else
		bsr_info(connection, "Reconciliation resync because \'%s\' disappeared. (o=%d)",
		...
		idr_for_each_entry_ex(struct bsr_peer_device *, &connection->peer_devices, peer_device, vnr) {
			if (new_repl_state == L_WF_BITMAP_S && test_bit(RECONCILIATION_RESYNC, &peer_device->flags)) {
				if (rv != SS_SUCCESS) {
					bsr_debug(peer_device, "Disable RECONCILIATION_RESYNC flag.");
...
#ifdef _TRACE_PEER_DAGTAG	
		bsr_info(connection, "No reconciliation resync even though \'%s\' disappeared. (o=%d) lost_peer:%p lost_peer->last_dagtag_sector:0x%llx be64_to_cpu(p->dagtag):%llx",
			  lost_peer->transport.net_conf->name, (int)dagtag_offset, lost_peer, lost_peer->last_dagtag_sector, be64_to_cpu(p->dagtag));
#else
		bsr_info(connection, "No reconciliation resync even though \'%s\' disappeared. (o=%d)",
			lost_peer->transport.net_conf->name, (int)dagtag_offset);
#endif
		...
		idr_for_each_entry_ex(struct bsr_peer_device *, &connection->peer_devices, peer_device, vnr) {	
			...
			if(peer_device->disk_state[NOW] > D_INCONSISTENT && get_ldev_if_state(peer_device->device, D_OUTDATED)) {
				...
				bsr_info(connection, "No bsr_bm_clear_many_bits, disk_state:%d peer disk_state:%d",
	...
	if (current_uuid == bsr_current_uuid(device)) {
		...
		if (bsr_current_uuid(device) != 0 && device->ldev) {
			...
			if (peer_md->bitmap_uuid != 0) {
				bsr_info(peer_device, "Clear bitmap_uuid (cur_uuid:%016llX bm_uuid:%016llX)", current_uuid, peer_md->bitmap_uuid);
				bsr_uuid_set_bitmap(peer_device, 0);
			}
		}
...
static void bsrd(struct bsr_connection *connection)
{
	...
	while (get_t_state(&connection->receiver) == RUNNING) {
		...
		if (unlikely(pi.cmd >= ARRAY_SIZE(bsr_cmd_handler) || !cmd->fn)) {
			bsr_err(connection, "Unexpected data packet %s (0x%04x)",
		...
		if (pi.size > shs && !cmd->expect_payload) {
			bsr_err(connection, "No payload expected %s l:%d",
				 bsr_packet_name(pi.cmd), pi.size);
			goto err_out;
		}
		if (pi.size < shs) {
			bsr_err(connection, "%s: unexpected packet size, expected:%d received:%d",
		...
		bsr_debug(connection, "receiving %s, size: %u vnr: %d", bsr_packet_name(pi.cmd), pi.size, pi.vnr);
		err = cmd->fn(connection, &pi);
		if (err) {
			bsr_err(connection, "error receiving %s, e: %d l: %u!",
...
static void cleanup_resync_leftovers(struct bsr_peer_device *peer_device)
{
	...
	*  on the fly. */
	...
	if (peer_device->rs_send_req != peer_device->rs_recv_res ||
		...
		bsr_info(peer_device, "incomplete resync exit, rs_send_req(%llu), rs_recv_res(%llu), rs_written(%lld)",
...
void conn_disconnect(struct bsr_connection *connection)
{
	...
	* peer_request queued to the submitter workqueue. */
	...
	if (!list_empty(&connection->active_ee)) {
		list_for_each_entry_ex(struct bsr_peer_request, peer_req, &connection->active_ee, w.list) {
			...
			bsr_info(device, "add, active_ee => inactive_ee(%p), sector(%llu), size(%u)", 
	...
	if (!list_empty(&connection->sync_ee)) {
		list_for_each_entry_ex(struct bsr_peer_request, peer_req, &connection->sync_ee, w.list) {
			struct bsr_device *device = peer_req->peer_device->device; 
			bsr_info(device, "add, sync_ee => inactive_ee(%p), sector(%llu), size(%u)", 
				peer_req, (unsigned long long)peer_req->i.sector, peer_req->i.size); 
		}
	...
	if (!list_empty(&connection->read_ee)) {
		list_for_each_entry_ex(struct bsr_peer_request, peer_req, &connection->read_ee, w.list) {
			struct bsr_device *device = peer_req->peer_device->device;
			bsr_info(device, "add, read_ee => inactive_ee(%p), sector(%llu), size(%u)", 
				peer_req, (unsigned long long)peer_req->i.sector, peer_req->i.size); 
		}
	...
	* to be "canceled" */
	...
	if (i)
		bsr_err(connection, "read_ee not empty, killed %d entries", i);
	i = bsr_free_peer_reqs(resource, &connection->active_ee, true);
	if (i)
		bsr_err(connection, "active_ee not empty, killed %d entries", i);
	i = bsr_free_peer_reqs(resource, &connection->sync_ee, true);
	if (i)
		bsr_err(connection, "sync_ee not empty, killed %d entries", i);
	i = bsr_free_peer_reqs(resource, &connection->net_ee, true);
	if (i)
		bsr_err(connection, "net_ee not empty, killed %d entries", i);
	...
	if (i)
		bsr_info(connection, "pp_in_use = %d, expected 0", i);
	i = atomic_read(&connection->pp_in_use_by_net);
	if (i)
		bsr_info(connection, "pp_in_use_by_net = %d, expected 0", i);

	if (!list_empty(&connection->current_epoch->list)) {
		struct bsr_epoch *epoch;
		bsr_err(connection, "ASSERTION FAILED: connection->current_epoch->list not empty");
		...
		list_for_each_entry_ex(struct bsr_epoch, epoch, &connection->current_epoch->list, list) {
			bsr_info(connection, "ASSERTION FAILED: remove epoch barrier_nr : %u, epochs:%u", epoch->barrier_nr, connection->epochs);
			list_del(&epoch->list);
			kfree(epoch);
			connection->epochs--;
			if (list_empty(&connection->current_epoch->list))
	...
	bsr_info(connection, "Connection closed");
...
int bsr_do_features(struct bsr_connection *connection)
{
	...
	if (err) {
		...
		if (err == -EAGAIN)
			bsr_err(connection, "timeout while waiting for feature packet");
		return 0;
	}
	bsr_debug_conn("success bsr_recv_header");
	if (pi.cmd != P_CONNECTION_FEATURES) {
		bsr_err(connection, "expected ConnectionFeatures packet, received: %s (0x%04x)",
			 bsr_packet_name(pi.cmd), pi.cmd);
		return -1;
	}

	if (pi.size != (unsigned int)expect) {
		bsr_err(connection, "expected ConnectionFeatures length: %u, received: %u",
		     expect, pi.size);
		return -1;
	}
	...
	if (PRO_VERSION_MAX < p->protocol_min ||
		...
		bsr_err(connection, "incompatible BSR dialects: "
	...
	if (connection->agreed_pro_version < 110) {
		...
		if (multiple) {
			bsr_err(connection, "Peer supports protocols %d-%d, but "
	...
	if (connection->agreed_pro_version >= 110) {
		if (be32_to_cpu(p->sender_node_id) != connection->peer_node_id) {
			bsr_err(connection, "Peer presented a node_id of %d instead of %d",
				 be32_to_cpu(p->sender_node_id), connection->peer_node_id);
			return 0;
		}
		if (be32_to_cpu(p->receiver_node_id) != resource->res_opts.node_id) {
			bsr_err(connection, "Peer expects me to have a node_id of %d instead of %d",
				 be32_to_cpu(p->receiver_node_id), resource->res_opts.node_id);
			return 0;
		}
	}

	bsr_info(connection, "Handshake to peer %d successful: "
...
int bsr_do_auth(struct bsr_connection *connection)
{
	bsr_err(connection, "This kernel was build without CONFIG_CRYPTO_HMAC.");
	bsr_err(connection, "You need to disable 'cram-hmac-alg' in bsr.conf.");
	return -1;
}
#else
...
int bsr_do_auth(struct bsr_connection *connection)
{
	...
	if (rv) {
		bsr_err(connection, "crypto_shash_setkey() failed with %d", rv);
		rv = -1;
		goto fail;
	}
	...
	if (pi.cmd != P_AUTH_CHALLENGE) {
		bsr_err(connection, "expected AuthChallenge packet, received: %s (0x%04x)",
			 bsr_packet_name(pi.cmd), pi.cmd);
		rv = 0;
		goto fail;
	}
	...
	if (pi.size != sizeof(peers_ch->d)) {
		bsr_err(connection, "unexpected AuthChallenge payload.");
		rv = -1;
		goto fail;
	}
	...
	if (peers_ch == NULL) {
		bsr_err(connection, "kmalloc of peers_ch failed");
		rv = -1;
		goto fail;
	}
	...
	if (!memcmp(my_challenge.d, peers_ch->d, sizeof(my_challenge.d))) {
		bsr_err(connection, "Peer presented the same challenge!");
		rv = -1;
		goto fail;
	}
	...
	if (rv) {
		bsr_err(connection, "crypto_shash_digest() failed with %d", rv);
		rv = -1;
		goto fail;
	}
	...
	if (pi.cmd != P_AUTH_RESPONSE) {
		bsr_err(connection, "expected AuthResponse packet, received: %s (0x%04x)",
			 bsr_packet_name(pi.cmd), pi.cmd);
		rv = 0;
		goto fail;
	}
	...
	if (pi.size != resp_size) {
		bsr_err(connection, "expected AuthResponse payload of %u bytes, received %u",
				resp_size, pi.size);
		rv = 0;
		goto fail;
	}
	...
	if (right_response == NULL) {
		bsr_err(connection, "kmalloc of right_response failed");
		rv = -1;
		goto fail;
	}
	...
	if (rv)
		bsr_info(connection, "Peer authenticated using %d bytes HMAC",
		     resp_size);
	else
		rv = -1;
...
static int got_RqSReply(struct bsr_connection *connection, struct packet_info *pi)
{
	...
	if (retcode >= SS_SUCCESS)
		...
		set_bit(TWOPC_NO, &connection->flags);
		bsr_debug(connection, "Requested state change failed by peer: %s (%d)",
...
static int got_twopc_reply(struct bsr_connection *connection, struct packet_info *pi)
{
	...
	if ((unsigned int)resource->twopc_reply.initiator_node_id == be32_to_cpu(p->initiator_node_id) &&
		...
		bsr_debug(connection, "Got a %s reply for state change %u",
			   bsr_packet_name(pi->cmd),
			   resource->twopc_reply.tid);
		...
		bsr_debug(connection, "Ignoring %s reply for state change %u",
			   bsr_packet_name(pi->cmd),
			   be32_to_cpu(p->tid));
	}
	spin_unlock_irq(&resource->req_lock);
...
#ifdef BSR_TRACE	
	bsr_debug(NO_OBJECT,"(%s) validate_req_change_req_state: before __req_mod! IRQL(%d) ", current->comm, KeGetCurrentIrql());
#endif
	__req_mod(req, what, peer_device, &m);
	spin_unlock_irq(&device->resource->req_lock);

#ifdef BSR_TRACE	
	bsr_debug(NO_OBJECT,"(%s) validate_req_change_req_state: after __req_mod! IRQL(%d) ", current->comm, KeGetCurrentIrql());
#endif
...
#ifdef BSR_TRACE
	bsr_debug(NO_OBJECT,"pi-cmd 0x%x(%s) sect:0x%llx sz:%d", pi->cmd, bsr_packet_name(pi->cmd), sector, blksize);
#endif
...
static int got_NegAck(struct bsr_connection *connection, struct packet_info *pi)
{
	...
	if (connection->agreed_pro_version >= 113) {
		...
		if (p->block_id == ID_SYNCER_SPLIT || p->block_id == ID_SYNCER_SPLIT_DONE) {
			bsr_debug(connection, "bsr_rs_failed_io sector : %llu, size %d", (unsigned long long)sector, size);
...
static int got_NegDReply(struct bsr_connection *connection, struct packet_info *pi)
{
	...
	bsr_err(device, "Got NegDReply; Sector %llus, len %u.",
...
#ifdef BSR_TRACE
	bsr_debug(NO_OBJECT,"do tl_release");
#endif
...
static int got_peer_ack(struct bsr_connection *connection, struct packet_info *pi)
{
...
#ifdef _TRACE_PEER_DAGTAG    
	bsr_info(NO_OBJECT,"got_peer_ack dagtag:%llx in_sync:%llx", dagtag, in_sync);
#endif
	...
	bsr_err(connection, "peer request with dagtag %llu not found", dagtag);
	return -EIO;
...
#ifdef _TRACE_PEER_DAGTAG			
			bsr_info(NO_OBJECT,"got_peer_ack bsr_set_sync device:%p, peer_req->i.sector:%llx, peer_req->i.size:%d, in_sync_b:%llx, set_sync_mask:%llx", 
...
int bsr_ack_receiver(struct bsr_thread *thi)
{
	...
	if (rv < 0)
		bsr_err(connection, "bsr_ack_receiver: ERROR set priority, ret=%d", rv);
#endif
	...
	while (get_t_state(thi) == RUNNING) {
		...
		if (connection->resource->breqbuf_overflow_alarm) {
			bsr_err(connection, "bsr_req overflow alarm");
		...
		if (test_and_clear_bit(SEND_PING, &connection->flags)) {
			...
			if (ping_ret) {
				...
				if (ping_ret == -EINTR && current->sig == SIGXCPU) {
					bsr_info(NO_OBJECT,"got SIGXCPU during ping");
					flush_signals(current);
				}
#endif
				bsr_err(connection, "bsr_send_ping has failed (%d)", ping_ret);
				goto reconnect;
			}
		...
		if (likely(rv > 0)) {
			...
			bsr_err(connection, "meta connection shut down by peer.");
			goto reconnect;
		} else if (rv == -EAGAIN) {
			...
			if (ping_timeout_active) {
				bsr_err(connection, "PingAck did not arrive in time.");
				goto reconnect;
			}
			...
			if (current->sig == SIGXCPU) {
				//bsr_info(NO_OBJECT,"got SIGXCPU during rx.");
			}
#endif
			flush_signals(current);
			continue;
		} else {
			bsr_err(connection, "sock_recvmsg returned %d", rv);
			goto reconnect;
		}
		...
		if (received == expect && cmd == NULL) {
			...
			if (pi.cmd >= ARRAY_SIZE(ack_receiver_tbl) || !cmd->fn) {
				bsr_err(connection, "Unexpected meta packet %s (0x%04x)",
					 bsr_packet_name(pi.cmd), pi.cmd);
				goto disconnect;
			}
			expect = (int)(header_size + cmd->pkt_size);
			if (pi.size != expect - header_size) {
				bsr_err(connection, "Wrong packet size on meta (c: %d, l: %u)",
					pi.cmd, pi.size);
				goto reconnect;
			}
			rflags = 0;
		...
		if (received == expect) {
			...
			pi.data = buffer;
			if (cmd) {
				bsr_debug(connection, "receiving %s, l: %d", bsr_packet_name(pi.cmd), pi.size); 
				err = cmd->fn(connection, &pi);
				if (err)
					bsr_debug(connection, "receiving error e: %d", err);
			}

			if (err) {
#ifdef _WIN
				if (err == -EINTR && current->sig == SIGXCPU) {
					//bsr_info(NO_OBJECT,"got SIGXCPU during fn(%s)", bsr_packet_name(pi.cmd));
					flush_signals(current);
					goto ignore_sig;
				}
#else // _LIN
				bsr_err(connection, "%pf failed", cmd->fn);
#endif
				goto reconnect;
			}
#ifdef _WIN
...
disconnect:
	...
	bsr_info(connection, "ack_receiver terminated");
...
void bsr_send_peer_ack_wf(struct work_struct *ws)
{
	...
	if (process_peer_ack_list(connection)){
		...
		if (connection->cstate[NOW] != C_DISCONNECTING) {
			bsr_debug(connection, "change the connection state to C_UNCONNECTED");
			__change_cstate(connection, C_UNCONNECTED);
		}
	}
}
