...
#endif
{
	...
	if (i < 0) {
		bsr_warn(BSR_LC_MEMORY, connection, "ASSERTION FAILED: %s: %d < 0\n",
...
bsr_alloc_peer_req(struct bsr_peer_device *peer_device, gfp_t gfp_mask) __must_hold(local)
{
	...
	if (!peer_req) {
		if (!(gfp_mask & __GFP_NOWARN))
			bsr_err(BSR_LC_TEMP, device, "%s: allocation failed\n", __func__);
		return NULL;
	}
...
static int bsr_recv(struct bsr_connection *connection, void **buf, size_t size, int flags)
{
	...
	if (rv < 0) {
		if (rv == -ECONNRESET)
			bsr_info(BSR_LC_TEMP, connection, "sock was reset by peer\n");
		else if (rv != -ERESTARTSYS)
			bsr_info(BSR_LC_TEMP, connection, "sock_recvmsg returned %d\n", rv);
	} else if (rv == 0) {
		...
		bsr_info(BSR_LC_TEMP, connection, "sock was shut down by peer\n");
	}
...
static int bsr_recv_all_warn(struct bsr_connection *connection, void **buf, size_t size)
{
	...
	if (err && !signal_pending(current))
		bsr_warn(BSR_LC_PROTOCOL, connection, "short read (expected size %d)\n", (int)size);
	return err;
}
...
int connect_work(struct bsr_work *work, int cancel)
{
	...
	if (rv >= SS_SUCCESS) {
		...
		bsr_alert(BSR_LC_CONNECTION, connection, "Split-Brain since more primaries than allowed; dropping connection!\n");
		bsr_khelper(NULL, connection, "split-brain");
	} else {
		bsr_info(BSR_LC_TEMP, connection, "Failure to connect; retrying\n");
...
start:
	...
	if (err == -EAGAIN) {
		...
		if (connection->cstate[NOW] == C_NETWORK_FAILURE || connection->cstate[NOW] == C_CONNECTING){
			bsr_warn(BSR_LC_CONNECTION, connection, "cstate is C_NETWORK_FAILURE or C_CONNECTING now goto retry, err=%d\n", err);
			goto retry;
		}
		else
			bsr_warn(BSR_LC_CONNECTION, connection, "Failed to initiate connection, err=%d\n", err);
		goto abort;
	}
	...
	* or the challenge respons authentication could be garbled. */
	...
	if (connection->cram_hmac_tfm) {
		...
		case -1:
			bsr_err(BSR_LC_TEMP, connection, "Authentication of peer failed\n");
			goto abort;
		case 0:
			bsr_err(BSR_LC_TEMP, connection, "Authentication of peer failed, trying again.\n");
			goto retry;
		}
	}
	...
	if (nc->sndbuf_size >= BSR_SNDBUF_SIZE_MIN) {
		...
		if (send_buffring)
			bsr_info(BSR_LC_TEMP, connection, "send-buffering ok size(%llu) cong_fill(%llu)\n", nc->sndbuf_size, (nc->cong_fill));
		else
			bsr_warn(BSR_LC_SEND_BUFFER, connection, "send-buffering disabled\n");
	} else {
		bsr_warn(BSR_LC_SEND_BUFFER, connection, "send-buffering disabled nc->sndbuf_size:%llu\n", nc->sndbuf_size);
	}
#endif
	...
	if (!connection->ack_sender) {
		bsr_err(BSR_LC_TEMP, connection, "Failed to create workqueue ack_sender\n");
		goto abort;
	}
	...
	if (connection->agreed_pro_version >= 110) {
		if (resource->res_opts.node_id < connection->peer_node_id) {
			...
			bsr_debug(BSR_LC_TEMP, connection, "Waiting for %ums to avoid transaction "
				   "conflicts\n", jiffies_to_msecs(timeout));
			// BSR-634 changed to mod_timer() due to potential kernel panic caused by duplicate calls to add_timer().
			mod_timer(&connection->connect_timer, jiffies + timeout);
		}
		else {
			bsr_debug(BSR_LC_TEMP, connection, "Skip connect_work\n");
#if 0 // _WIN
...
retry:
	...
	if (get_t_state(&connection->receiver) == RESTARTING) {
		bsr_warn(BSR_LC_THREAD, connection, "could not retry connection since receiver is restarting\n");
		return false;
	}
	goto start;
...
int decode_header(struct bsr_connection *connection, void *header, struct packet_info *pi)
{
	...
	if (header_size == sizeof(struct p_header100) &&
	    *(__be32 *)header == cpu_to_be32(BSR_MAGIC_100)) {
		...
		if (h->pad != 0) {
			bsr_err(BSR_LC_TEMP, connection, "Header padding is not zero\n");
			return -EINVAL;
		}
		...
		bsr_err(BSR_LC_TEMP, connection, "Wrong magic value 0x%08x in protocol version %d\n",
...
#endif
{
	...
	if (atomic_read(&g_featurelog_flag) & FEATURELOG_FLAG_LATENCY)
		bsr_latency(BSR_LC_LATENCY, device, "flush IO latency : minor(%u) %lldus\n", device->minor, timestamp_elapse(bio->flush_ts, timestamp()));
	...
	if (error) {
		...
		if (ctx)
			ctx->error = error;
		bsr_err(BSR_LC_TEMP, device, "local disk FLUSH FAILED with status %08X\n", error);
	}
...
static void submit_one_flush(struct bsr_device *device, struct issue_flush_context *ctx)
{
	...
	if (!bio || !octx) {
		bsr_warn(BSR_LC_IO, device, "Could not allocate a bio, CANNOT ISSUE FLUSH\n");
...
#endif
			if (ret == -BSR_SIGKILL) {
				bsr_warn(BSR_LC_IO, resource, "thread signaled and no more wait pending:%d, barrier_nr:%lld, primary_node_id:%d\n", 
					       ...
					       enum epoch_event ev)
{
	...
	if (schedule_flush) {
		...
		if (fw) {
			...
			bsr_warn(BSR_LC_IO, resource, "Could not kmalloc a flush_work obj\n");
...
enum write_ordering_e wo) __must_hold(local)
{
	...
	if (pwo != resource->write_ordering || wo == WO_BIO_BARRIER)
		bsr_info(BSR_LC_TEMP, resource, "Method to ensure write ordering: %s\n", write_ordering_str[resource->write_ordering]);
}
...
struct bsr_peer_request *peer_req)
{
	...
	bsr_err(BSR_LC_TEMP, device, "received unsupported WRITE_SAME request\n");
	peer_req->flags |= EE_WAS_ERROR;
	bsr_endio_write_sec_final(peer_req);
#else
	struct block_device *bdev = device->ldev->backing_bdev;
	...
	const int fault_type)
{
	...
	if (test_bit(CONNECTION_ALREADY_FREED, &peer_req->peer_device->flags)) {
		bsr_info(BSR_LC_TEMP, device, "node-id : %d, flag : CONNECTION_ALREADY_FREED\n", peer_req->peer_device->node_id);
		return 0;
	}
...
next_bio:
	...
	*/
	if (!(op == REQ_OP_WRITE || op == REQ_OP_READ)) {
		bsr_err(BSR_LC_TEMP, device, "Invalid bio op received: 0x%x\n", op);
		err = -EINVAL;
		goto fail;
	}
...
#endif
	if (!bio) {
		bsr_err(BSR_LC_TEMP, device, "submit_ee: Allocation of a bio failed (nr_pages=%u)\n", nr_pages);
		goto fail;
	}
	...
	page_chain_for_each(page) {
		...
		if (off > PAGE_SIZE || len > PAGE_SIZE - off || len > data_size || len == 0) {
			bsr_err(BSR_LC_TEMP, device, "invalid page chain: offset %u size %u remaining data_size %u\n",
				off, len, data_size);
			err = -EINVAL;
			goto fail;
		}
...
int w_e_reissue(struct bsr_work *w, int cancel) __releases(local)
{
	...
	if (previous_epoch(peer_device->connection, peer_req->epoch))
		bsr_warn(BSR_LC_REPLICATION, device, "Write ordering was not enforced (one time event)\n");
	...
	default:
		...
		bsr_free_peer_req(peer_req);
		bsr_err(BSR_LC_TEMP, device, "submit failed, triggering re-connect\n");
		return err;
	}
}
...
static int receive_Barrier(struct bsr_connection *connection, struct packet_info *pi)
{
	...
	epoch = kmalloc(sizeof(struct bsr_epoch), GFP_NOIO, '12DW');
	if (!epoch) {
		bsr_warn(BSR_LC_REPLICATION, connection, "Allocation of an epoch failed, slowing down\n");
...
struct bsr_peer_request_details *d, struct packet_info *pi)
{
	...
	bsr_debug(BSR_LC_TEMP, NO_OBJECT, "sector: %llu block_id: %llu peer_seq: %u dp_flags:%u length:%u bi_size:%u digest_size: %u\n", 
...
read_in_block(struct bsr_peer_device *peer_device, struct bsr_peer_request_details *d) __must_hold(local)
{
	...
	if (d->sector + (d->bi_size >> 9) > capacity) {
		bsr_err(BSR_LC_TEMP, device, "request from peer beyond end of local disk: "
...
#ifdef _WIN
		bsr_info(BSR_LC_TEMP, device, "Fault injection: Not supported\n");
#else // _LIN
		struct page *page;
		unsigned long *data;
		bsr_err(BSR_LC_TEMP, device, "Fault injection: Corrupting data on receive, sector %llu\n",
			d->sector);
	...
	if (d->digest_size) {
		...
		if (memcmp(dig_in, dig_vv, d->digest_size)) {
			bsr_err(BSR_LC_TEMP, device, "Digest integrity check FAILED: %llus +%u\n",
				d->sector, d->bi_size);
			goto fail;
		}
	}
	...
	sector_t sector, int data_size)
{
	...
	if (digest_size) {
		...
		if (memcmp(dig_in, dig_vv, digest_size)) {
			bsr_err(BSR_LC_TEMP, peer_device, "Digest integrity check FAILED. Broken NICs?\n");
			return -EINVAL;
		}
	}
...
static bool get_resync_pending_range(struct bsr_peer_device* peer_device, sector_t sst, sector_t est, sector_t *cst)
{
	...
	list_for_each_entry_ex(struct bsr_resync_pending_sectors, target, &(peer_device->device->resync_pending_sectors), pending_sectors) {
		if (est <= target->sst) {
			...
			*cst = est;
			...
			bsr_info(BSR_LC_TEMP, peer_device, "dup all out of sync sectors %llu ~ %llu => source %llu ~ %llu, target %llu ~ %llu\n",
			...
			*cst = target->sst;
			...
			bsr_info(BSR_LC_TEMP, peer_device, "dup front out of sync sectors %llu ~ %llu => source %llu ~ %llu, target %llu ~ %llu\n",
			...
			*cst = (target->est < est ? target->est : est);
			...
			bsr_info(BSR_LC_TEMP, peer_device, "dup end out of sync sectors %llu ~ %llu => source %llu ~ %llu, target %llu ~ %llu\n",
		...
		for (i_bb = peer_req->s_bb; i_bb < e_next_bb; i_bb++) {
			if (bsr_bm_test_bit(peer_device, i_bb) == 1) {
				...
				complete_end_sync:
					...
					bsr_debug(BSR_LC_TEMP, peer_device, "--set in sync, bitmap bit start : %llu, range : %llu ~ %llu, size %llu, count %d\n",
				...
				complete_end_out_of_sync:
					...
					peer_req->i.sector = BM_BIT_TO_SECT(s_bb);
					peer_req->i.size = (unsigned int)BM_BIT_TO_SECT(i_bb - s_bb) << 9;
					bsr_err(BSR_LC_TEMP, peer_device, "--set failed to I/O, bitmap bit start : %llu, range : %llu ~ %llu, size %llu\n",
...
static bool check_unmarked_and_processing(struct bsr_peer_device *peer_device, struct bsr_peer_request *peer_req)
{
	...
	if (peer_req->unmarked_count && 
		...
		bsr_debug(BSR_LC_TEMP, peer_device, "--finished unmarked s_bb(%llu), e_bb(%llu), sector(%llu), res(%s)\n", 
					(unsigned long long)peer_req->s_bb, 
					(unsigned long long)(peer_req->e_next_bb - 1), 
					(unsigned long long)peer_req->i.sector,
					(atomic_read(peer_req->failed_unmarked) == 1 ? "failed" : "success"));
...

{
	...
	if(!data) {
		bsr_err(BSR_LC_TEMP, peer_device, "Failed to allocate buffer to get page data.\n");
		goto alloc_fail;
	}
...
alloc_fail:
	...
	bsr_debug(BSR_LC_TEMP, peer_device, "##split request s_bb(%llu), e_bb(%llu), sector(%llu), offset(%llu), size(%u)\n", 
	...
	for (ibb = s_bb; ibb < e_next_bb; ibb++) {
		...
		if (is_marked_rl_bb(peer_device, &marked_rl, ibb)) {
			...
			*e_oos = ibb;
			...
			bsr_debug(BSR_LC_TEMP, peer_device, "##find in sync bitmap bit : %llu, start (%llu) ~ end (%llu)\n",
...
static int split_recv_resync_read(struct bsr_peer_device *peer_device, struct bsr_peer_request_details *d) __releases(local)
{
	...
	if (!peer_req) {
		bsr_err(BSR_LC_TEMP, peer_device, "failed to allocate peer_req\n");
		return -EIO;
	}
	...
	if (d->bi_size < BM_BLOCK_SIZE) {
		bsr_warn(BSR_LC_RESYNC_OV, peer_device, "bug FIMXME!! bi_size(%lu) < BM_BLOCK_SIZE\n", (unsigned long)d->bi_size);
	}
	...
	if (peer_device->connection->agreed_pro_version >= 113 &&         
		...
		if (!split_count) {
			bsr_err(BSR_LC_TEMP, peer_device, "failed to allocate split count\n");
			return -ENOMEM;
		}
			...
			for (i_bb = offset = s_bb; i_bb < e_next_bb; i_bb++) {
				...
				if (is_marked_bb || already_in_sync_bb) {
					if (is_all_sync) {
						...
						bsr_debug(BSR_LC_TEMP, peer_device, "##all, sync bitmap(%llu), start : %llu, end :%llu\n", 
				...
				submit_peer:
					...
					if (s_split_request) {
						bsr_debug(BSR_LC_TEMP, peer_device, "##sync bitmap bit %llu, split request %llu ~ %llu, size %llu, start(%llu) ~ end(%llu), end out of sync(%llu)\n",
						...
						if (!split_peer_req) {
							bsr_err(BSR_LC_TEMP, peer_device, "failed to alloc split_peer_req, %llu\n", (unsigned long long)i_bb);
							err = -ENOMEM;
							goto split_error_clear;
						}

						BSR_VERIFY_DATA("%s, split, sector(%llu), size(%u), bitmap(%llu ~ %llu), s_rl_bb(%llu), e_rl_bb(%llu)\n",
						...
						if (!is_sync_target(peer_device)) {
							...
							for_each_peer_device(pd, device) {
								if (pd != peer_device && is_sync_target(pd)) {
									bsr_info(BSR_LC_TEMP, peer_device, "resync with other nodes in progress\n");
					...
					if (marked_rl) {
						...
						if (!unmarked_count) {
							bsr_err(BSR_LC_TEMP, peer_device, "failed to allocate unmakred_count\n");
						...
						if (!failed_unmarked) {
							bsr_err(BSR_LC_TEMP, peer_device, "failed to allocate failed_unmarked\n");
						...
						for (i = 0; i < sizeof(marked_rl->marked_rl) * 8; i++) {
							...
							if (!(marked_rl->marked_rl & 1 << i)) {
								...
								if (!split_peer_req) {
									bsr_err(BSR_LC_TEMP, peer_device, "failed to allocate split_peer_req, %llu\n", (unsigned long long)i_bb);
								...
								BSR_VERIFY_DATA("%s, marked, sector(%llu), size(%u), bitmap(%llu ~ %llu), s_rl_bb(%llu), e_rl_bb(%llu)\n",
									__FUNCTION__, (unsigned long long)split_peer_req->i.sector, split_peer_req->i.size, (unsigned long long)BM_SECT_TO_BIT(split_peer_req->i.sector), (unsigned long long)BM_SECT_TO_BIT(split_peer_req->i.sector + (split_peer_req->i.size >> 9)), (unsigned long long)device->s_rl_bb, (unsigned long long)device->e_rl_bb);
								...
								bsr_debug(BSR_LC_TEMP, peer_device, "##unmarked bb(%llu), sector(%llu), offset(%d), count(%d)\n", 
									(unsigned long long)marked_rl->bb, 
									(unsigned long long)BM_BIT_TO_SECT(marked_rl->bb) + i, i, atomic_read(unmarked_count));
								if (!bsr_submit_peer_request(device, split_peer_req, REQ_OP_WRITE, 0, BSR_FAULT_RS_WR) == 0) {
									bsr_err(BSR_LC_TEMP, device, "unmarked, submit failed, triggering re-connect\n");
									// DW-1923 for interparameter synchronization, an additional 1 was added for the remaining count and modified to use atomic_dec_return.
									atomic_set(unmarked_count, atomic_read(unmarked_count) - (atomic_read(unmarked_count) - submit_count) + 1);
									if (unmarked_count && 0 == atomic_dec_return(unmarked_count)) {
										kfree(failed_unmarked);
	     ...
	     sector_t sector, bool missing_ok, const char *func)
{
	...
	if (bsr_contains_interval(root, sector, &req->i) && req->i.local)
		return req;
	if (!missing_ok) {
		bsr_err(BSR_LC_TEMP, device, "%s: failed to find request 0x%llx, sector %llus\n", func,
...
static int receive_bm_exchange_state(struct bsr_connection *connection, struct packet_info *pi) {
	...
	case B_COMPLETE:
		...
		bsr_info(BSR_LC_TEMP, peer_device, "bitmap exchange complete\n");
		break;
	default:
		bsr_err(BSR_LC_TEMP, peer_device, "unknown error(%u)\n", state);
		break;
	}
	return 0;
}
 ...
 * @peer_req:	peer request
 */
int bsr_send_ack(struct bsr_peer_device *peer_device, enum bsr_packet cmd,
		  struct bsr_peer_request *peer_req)
{
	return _bsr_send_ack(peer_device, cmd,
...
static int receive_RSDataReply(struct bsr_connection *connection, struct packet_info *pi)
{
	...
	if (get_ldev(device)) {
		...
		if (bsr_ratelimit())
			bsr_err(BSR_LC_TEMP, device, "Can not write resync data to local disk.\n");
...
static int wait_for_and_update_peer_seq(struct bsr_peer_device *peer_device, const u32 peer_seq)
{
	...
	for (;;) {
		...
		if (!timeout) {
			ret = -ETIMEDOUT;
			bsr_err(BSR_LC_TEMP, peer_device, "Timed out waiting for missing ack packets; disconnecting\n");
			break;
		}
	}
	spin_unlock(&peer_device->peer_seq_lock);
...
static int handle_write_conflicts(struct bsr_peer_request *peer_req)
{
	...
	bsr_for_each_overlap(i, &device->write_requests, sector, size) {
		...
		if (resolve_conflicts) {
			...
			if (!equal)
				bsr_alert(BSR_LC_ETC, device, "Concurrent writes detected: "
...
static int dedup_from_resync_pending(struct bsr_peer_device *peer_device, sector_t sst, sector_t est)
{
	...
	list_for_each_entry_safe_ex(struct bsr_resync_pending_sectors, target, tmp, &(peer_device->device->resync_pending_sectors), pending_sectors) {
		if (sst <= target->sst && est >= target->est) {
			bsr_info(BSR_LC_TEMP, peer_device, "resync pending remove sector %llu(%llu) ~ %llu(%llu)\n",
		...
		if (sst >= target->sst && est <= target->est) {
...
#ifdef _WIN
			pending_st = ExAllocatePoolWithTag(NonPagedPool, sizeof(struct bsr_resync_pending_sectors), 'E9DW');
#else // _LIN
			pending_st = (struct bsr_resync_pending_sectors *)kmalloc(sizeof(struct bsr_resync_pending_sectors), GFP_ATOMIC|__GFP_NOWARN, '');
#endif
			if (!pending_st) {
				bsr_err(BSR_LC_TEMP, peer_device, "failed to resync pending bits allocate, sector : %llu ~ %llu\n", (unsigned long long)sst, (unsigned long long)est);
				mutex_unlock(&peer_device->device->resync_pending_fo_mutex);
				return -ENOMEM;
			}
			...
			bsr_info(BSR_LC_TEMP, peer_device, "resync pending split new sector %llu(%llu) ~ %llu(%llu)\n",
			...
			target->est = sst;
			bsr_info(BSR_LC_TEMP, peer_device, "resync pending split sector %llu(%llu) ~ %llu(%llu)\n",
				(unsigned long long)target->sst, (unsigned long long)BM_SECT_TO_BIT(target->sst), (unsigned long long)target->est, (unsigned long long)BM_SECT_TO_BIT(target->est));
		}
		...
		if (sst <= target->sst && est > target->sst && est <= target->est) {
			...
			target->sst = est;
			bsr_info(BSR_LC_TEMP, peer_device, "resync pending modify sector %llu(%llu) ~ %llu(%llu)\n",
				(unsigned long long)target->sst, (unsigned long long)BM_SECT_TO_BIT(target->sst), (unsigned long long)target->est, (unsigned long long)BM_SECT_TO_BIT(target->est));
		}
...
static int list_add_marked(struct bsr_peer_device* peer_device, sector_t sst, sector_t est, unsigned int size, ULONG_PTR in_sync) 
{
	...
	if ((s_resync_bb <= e_bb && n_resync_bb >= e_bb) ||
		...
		if ((BM_BIT_TO_SECT(s_bb) != sst || (BM_BIT_TO_SECT(s_bb) == sst && s_bb == e_bb)) &&
			...
			if (!s_marked_rl) {
#ifdef _WIN
				s_marked_rl = ExAllocatePoolWithTag(NonPagedPool, sizeof(struct bsr_marked_replicate), 'E8DW');
#else // _LIN
				...
				if (s_marked_rl != NULL) {
					...
					bsr_err(BSR_LC_TEMP, peer_device, "failed to marked replicate allocate, bit : %llu\n", (unsigned long long)s_bb);
					return -ENOMEM;
				}
			}
			...
			bsr_debug(BSR_LC_TEMP, peer_device, "sbb marking bb(%llu), ssector(%llu), sector(%llu), size(%u), marked(%u), offset(%u)\n",
		...
		if (s_bb != e_bb && BM_BIT_TO_SECT(BM_SECT_TO_BIT(est)) != est &&
			...
			if (!e_marked_rl) {
#ifdef _WIN
				e_marked_rl = ExAllocatePoolWithTag(NonPagedPool, sizeof(struct bsr_marked_replicate), 'E8DW');
#else // _LIN
			...
			bsr_debug(BSR_LC_TEMP, peer_device, "marking bb(%llu), esector(%llu), sector(%llu), size(%u), marked(%u), offset(%u)\n",
...
static int receive_Data(struct bsr_connection *connection, struct packet_info *pi)
{
	...
	if(get_t_state(&connection->ack_receiver) != RUNNING) {
		bsr_info(BSR_LC_TEMP, NO_OBJECT,"ack_receiver is not running... bump mirrored data\n");
		return 0;
	}
	...
	* Remember the op_flags. */
		...
		* "synchronously" from receiver context */
		if (peer_req->flags & (EE_IS_TRIM | EE_WRITE_SAME | EE_IS_BARRIER)) {
			...
			if (err) {
				...
				bsr_err(BSR_LC_TEMP, peer_device, "disconnect during al begin io (err = %d)\n", err);
				if (peer_req->flags & EE_SEND_WRITE_ACK) {
					dec_unacked(peer_device);
				}
	...
	if (!err) {	// DW-1012 The data just received is the newest, ignore previously received out-of-sync.
		...
		bsr_info(BSR_LC_TEMP, NO_OBJECT,"receive_Data connection->last_dagtag_sector:%llx ack_receiver thread state:%d\n",connection->last_dagtag_sector, get_t_state(&connection->ack_receiver));
#endif
		return err;
	}

	/* don't care for the reason here */
	bsr_err(BSR_LC_TEMP, peer_device, "submit failed, triggering re-connect\n");
	bsr_al_complete_io(device, &peer_req->i);
...
static int receive_DataRequest(struct bsr_connection *connection, struct packet_info *pi)
{
	...
	if (size <= 0 || !IS_ALIGNED(size, 512) || size > BSR_MAX_BIO_SIZE) {
		bsr_err(BSR_LC_TEMP, device, "%s:%d: sector: %llus, size: %d\n", __FILE__, __LINE__,
				(unsigned long long)sector, size);
		return -EINVAL;
	}
    ...
    bsr_debug_rs("cmd(%s) sector(0x%llx), size(%d)\n", bsr_packet_name(pi->cmd), (u64)sector, size);
	...
	if (!get_ldev_if_state(device, min_d_state)) {
		...
		if (verb && bsr_ratelimit())
			bsr_err(BSR_LC_TEMP, device, "Can not satisfy peer's read request, "
			    "no local data.\n");
...
static int bsr_asb_recover_0p(struct bsr_peer_device *peer_device) __must_hold(local)
{
	...
	case ASB_VIOLENTLY:
		bsr_err(BSR_LC_TEMP, peer_device, "Configuration error.\n");
		break;
	case ASB_DISCONNECT:
		break;
	...
	case ASB_DISCARD_OLDER_PRI:
		...
		/* Else fall through to one of the other strategies... */
		bsr_warn(BSR_LC_RESYNC_OV, peer_device, "Discard younger/older primary did not find a decision\n"
...
static int bsr_asb_recover_1p(struct bsr_peer_device *peer_device) __must_hold(local)
{
	...
	case ASB_DISCARD_REMOTE:
	case ASB_DISCARD_ZERO_CHG:
		bsr_err(BSR_LC_TEMP, device, "Configuration error.\n");
		break;
	case ASB_DISCONNECT:
		break;
	case ASB_CONSENSUS:
	...
	case ASB_CALL_HELPER:
		...
		if (hg == -2 && resource->role[NOW] == R_PRIMARY) {
			...
			if (rv2 != SS_SUCCESS) {
				...
				bsr_warn(BSR_LC_RESYNC_OV, device, "Successfully gave up primary role.\n");
				rv = hg;
			}
		} else
			rv = hg;
...
static void bsr_uuid_dump_self(struct bsr_peer_device *peer_device, u64 bits, u64 flags)
{
	...
	bsr_info(BSR_LC_TEMP, peer_device, "self %016llX:%016llX:%016llX:%016llX bits:%llu flags:%llX\n",
...
static void bsr_uuid_dump_peer(struct bsr_peer_device *peer_device, u64 bits, u64 flags)
{
	...
	bsr_info(BSR_LC_TEMP, peer_device, "peer %016llX:%016llX:%016llX:%016llX bits:%llu flags:%llX\n",
...
static int uuid_fixup_resync_end(struct bsr_peer_device *peer_device, int *rule_nr) __must_hold(local)
{
	...
	if (peer_device->bitmap_uuids[node_id] == (u64)0 && bsr_bitmap_uuid(peer_device) != (u64)0) {
		...
		if ((bsr_bitmap_uuid(peer_device) & ~UUID_PRIMARY) ==
			...
			bsr_info(BSR_LC_TEMP, device, "was SyncSource, missed the resync finished event, corrected myself:\n");
			...
			*rule_nr = 34;
			...
			bsr_info(BSR_LC_TEMP, device, "was SyncSource (peer failed to write sync_uuid)\n");
			*rule_nr = 36;
		}
	...
	if (bsr_bitmap_uuid(peer_device) == (u64)0 && peer_device->bitmap_uuids[node_id] != (u64)0) {
		...
		if ((bsr_history_uuid(device, 0) & ~UUID_PRIMARY) ==
			...
			bsr_info(BSR_LC_TEMP, device, "was SyncTarget, peer missed the resync finished event, corrected peer:\n");
			...
			*rule_nr = 35;
			...
			bsr_info(BSR_LC_TEMP, device, "was SyncTarget (failed to write sync_uuid)\n");
			*rule_nr = 37;
		}
	...
	if (self == peer) {
		if (peer_device->connection->agreed_pro_version < 96 ?
			...
			*rule_nr = 51;
			...
			bsr_info(BSR_LC_TEMP, device, "Lost last syncUUID packet, corrected:\n");
			bsr_uuid_dump_peer(peer_device, peer_device->dirty_bits, peer_device->uuid_flags);
	...
	if (self == peer) {
		if (peer_device->connection->agreed_pro_version < 96 ?
			...
			*rule_nr = 71;
			...
			bsr_info(BSR_LC_TEMP, device, "Last syncUUID did not get through, corrected:\n");
			bsr_uuid_dump_self(peer_device,
					    device->disk_state[NOW] >= D_NEGOTIATING ? bsr_bm_total_weight(peer_device) : 0, 0);
...
static void log_handshake(struct bsr_peer_device *peer_device)
{
	...
	bsr_info(BSR_LC_TEMP, peer_device, "bsr_sync_handshake:\n");
			  ...
			  bool always_verbose) __must_hold(local)
{
	...
	if (hg || always_verbose)
		bsr_info(BSR_LC_TEMP, peer_device, "uuid_compare()=%d by rule %d\n", hg, *rule_nr);
...
static int bitmap_mod_after_handshake(struct bsr_peer_device *peer_device, int hg, int peer_node_id)
{
	...
	if (hg == 4) {
		...
		bsr_info(BSR_LC_TEMP, peer_device, "Peer synced up with node %d, copying bitmap\n", peer_node_id);
...
#if 0 // DW-1099 copying bitmap has a defect, do sync whole out-of-sync until fixed.
		bsr_info(BSR_LC_TEMP, peer_device, "synced up with node %d in the mean time\n", peer_node_id);
		bsr_suspend_io(device, WRITE_ONLY);
		...
		if (!isFastInitialSync() ||
			...
			bsr_info(BSR_LC_TEMP, peer_device, "Writing the whole bitmap, full sync required after bsr_sync_handshake.\n");			
						   ...
						   int hg)
{
	...
	if (hg > 0) { /* become sync source. */
		...
		if (bsr_bitmap_uuid(peer_device)) {
			bsr_info(BSR_LC_TEMP, peer_device, "clearing bitmap UUID and bitmap content (%llu bits)\n",
			 ...
			 */
			if (bsr_md_test_peer_flag(peer_device, MDF_PEER_PRIMARY_IO_ERROR)) {
				bsr_info(BSR_LC_TEMP, peer_device, "this node contain the oos generated by io-errors at primary.\n");
				rv = L_WF_BITMAP_S;
			}
			else if (peer_device->uuid_flags & UUID_FLAG_PRIMARY_IO_ERROR) {
				bsr_info(BSR_LC_TEMP, peer_device, "peer node contains the oos generated by io-errors at primary.\n");
				rv = L_WF_BITMAP_T;
			}
			else {
				...
				if (bsr_md_test_peer_flag(peer_device, MDF_PEER_IN_PROGRESS_SYNC) ||
					...
					bsr_info(BSR_LC_TEMP, peer_device, "ended during synchronization and completed resync with other nodes, clearing bitmap UUID and bitmap content (%llu bits)\n",
						(unsigned long long)bsr_bm_total_weight(peer_device));
					bsr_uuid_set_bitmap(peer_device, 0);
					bsr_bm_clear_many_bits(peer_device->device, peer_device->bitmap_index, 0, BSR_END_OF_BITMAP);
				}
				else {
					bsr_info(BSR_LC_TEMP, peer_device, "No resync, but %llu bits in bitmap!\n",
						(unsigned long long)bsr_bm_total_weight(peer_device));
				}
			}
		}
				    ...
				    int *hg, int rule_nr)
{
	...
	if (p)
		bsr_info(BSR_LC_TEMP, device, "Becoming sync %s due to disk states.\n",
			  *hg > 0 ? "source" : "target");
}
						...
						int *hg)
{
		...
		peer_device->dirty_bits == 0)
	{
		...
		bsr_info(BSR_LC_TEMP, peer_device, "FIXME, both nodes are UpToDate, but have inconsistent bits set. clear it without resync\n");
		...
		{
			bsr_info(BSR_LC_TEMP, peer_device, "bitmap will be cleared due to inconsistent out-of-sync\n");
...
out:
	if (*hg)
		bsr_info(BSR_LC_TEMP, device, "Becoming sync %s due to %s.\n",
		*hg > 0 ? "source" : "target",
						...
						enum bsr_disk_state peer_disk_state) __must_hold(local)
{
	...
	if (hg == -1000) {
		bsr_alert(BSR_LC_RESYNC_OV, device, "Unrelated data, aborting!\n");
		return -1;
	}
	if (hg < -1000) {
		bsr_alert(BSR_LC_RESYNC_OV, device, "To resolve this both sides have to support at least protocol %d\n", -hg - 1000);
		return -1;
	}
	...
	if (hg == 100 && (!bsr_device_stable(device, NULL) || !(peer_device->uuid_flags & UUID_FLAG_STABLE))) {
		bsr_warn(device, "Ignore Split-Brain, for now, at least one side unstable\n");
		hg = 0;
	}
	*/
	...
	if (hg == 100 || (hg == -100 && nc->always_asbp)) {
		...
		if (abs(hg) < 100) {
			bsr_warn(BSR_LC_CONNECTION, device, "Split-Brain detected, %d primaries, "
			     "automatically solved. Sync from %s node\n",
			     pcount, (hg < 0) ? "peer" : "this");
			if (forced) {
				bsr_warn(BSR_LC_CONNECTION, device, "Doing a full sync, since"
	...
	if (hg == -100) {
		...
		if (test_bit(DISCARD_MY_DATA, &peer_device->flags) &&
			...
			bsr_err(BSR_LC_TEMP, connection, "incompatible %s settings\n", "discard-my-data");
			rcu_read_unlock();
			return -1;
		}
		...
		if (abs(hg) < 100)
			bsr_warn(BSR_LC_CONNECTION, device, "Split-Brain detected, manually solved. "
			     "Sync from %s node\n",
			     (hg < 0) ? "peer" : "this");
	}
	...
	if (hg == -100) {
		bsr_alert(BSR_LC_CONNECTION, device, "Split-Brain detected but unresolved, dropping connection!\n");
	...
	if (hg <= -2 && /* by intention we do not use disk_state here. */
		...
		case ASB_CALL_HELPER:
			...
			/* fall through */
		case ASB_DISCONNECT:
			bsr_err(BSR_LC_TEMP, device, "I shall become SyncTarget, but I am primary!\n");
			return -1;
		case ASB_VIOLENTLY:
			bsr_warn(BSR_LC_CONNECTION, device, "Becoming SyncTarget, violating the stable-data"
			     "assumption\n");
		}
	}
	...
	if (hg == 3 && device->disk_state[NOW] < D_OUTDATED && 
		...
		bsr_err(BSR_LC_TEMP, device, "I shall become SyncSource, but I am inconsistent!\n");
		return -1;
	}

	if (hg == -3 && peer_device->uuid_flags & UUID_FLAG_INCONSISTENT) {
		bsr_err(BSR_LC_TEMP, device, "I shall become SyncTarget, but peer is inconsistent!\n");
		return -1;
	}	

	if (test_bit(CONN_DRY_RUN, &connection->flags)) {
		if (hg == 0)
			bsr_info(BSR_LC_TEMP, device, "dry-run connect: No resync, would become Connected immediately.\n");
		else
			bsr_info(BSR_LC_TEMP, device, "dry-run connect: Would become %s, doing a %s resync.",
...
static int receive_protocol(struct bsr_connection *connection, struct packet_info *pi)
{
	...
	if (integrity_alg[0]) {
		...
		if (IS_ERR(peer_integrity_tfm)) {
			...
			bsr_err(BSR_LC_TEMP, connection, "peer data-integrity-alg %s not supported\n",
				 integrity_alg);
			goto disconnect;
		}
		...
		if (!(int_dig_in && int_dig_vv)) {
			bsr_err(BSR_LC_TEMP, connection, "Allocation of buffers for data integrity checking failed\n");
			goto disconnect;
		}
	}
	...
	if (!new_net_conf) {
		bsr_err(BSR_LC_TEMP, connection, "Allocation of new net_conf failed\n");
		goto disconnect;
	}

	if (mutex_lock_interruptible(&connection->resource->conf_update)) {
		bsr_err(BSR_LC_TEMP, connection, "Interrupted while waiting for conf_update\n");
	...
	*new_net_conf = *old_net_conf;
	...
	if (strcmp(old_net_conf->integrity_alg, integrity_alg))
		bsr_info(BSR_LC_TEMP, connection, "peer data-integrity-alg: %s\n",
...
#endif
	if (IS_ERR(tfm)) {
		bsr_err(BSR_LC_TEMP, device, "Can not allocate \"%s\" as %s (reason: %ld)\n",
...
{
	bsr_warn(BSR_LC_VOLUME, connection, "%s packet received for volume %d, which is not configured locally\n",
...
static int receive_SyncParam(struct bsr_connection *connection, struct packet_info *pi)
{
	...
	if (pi->size > exp_max_sz) {
		bsr_err(BSR_LC_TEMP, device, "SyncParam packet too long: received %u, expected <= %u bytes\n",
		    pi->size, exp_max_sz);
		return -EIO;
	}
	...
	if (get_ldev(device)) {
		...
		if (!new_peer_device_conf) {
			...
			bsr_err(BSR_LC_TEMP, device, "Allocation of new peer_device_conf failed\n");
			return -ENOMEM;
		}
	...
	if (apv >= 88) {
		if (apv == 88) {
			if (data_size > SHARED_SECRET_MAX || data_size == 0) {
				bsr_err(BSR_LC_TEMP, device, "verify-alg too long, "
		...
		if (strcmp(old_net_conf->verify_alg, p->verify_alg)) {
			if (peer_device->repl_state[NOW] == L_OFF) {
				bsr_err(BSR_LC_TEMP, device, "Different verify-alg settings. me=\"%s\" peer=\"%s\"\n",
		...
		if (apv >= 89 && strcmp(old_net_conf->csums_alg, p->csums_alg)) {
			if (peer_device->repl_state[NOW] == L_OFF) {
				bsr_err(BSR_LC_TEMP, device, "Different csums-alg settings. me=\"%s\" peer=\"%s\"\n",
...
#endif
				if (!new_plan) {
					bsr_err(BSR_LC_TEMP, device, "kmalloc of fifo_buffer failed");
					goto disconnect;
				}
			}
		}
		...
		if (verify_tfm || csums_tfm) {
			...
			if (!new_net_conf) {
				bsr_err(BSR_LC_TEMP, device, "Allocation of new net_conf failed\n");
				goto disconnect;
			}
			...
			*new_net_conf = *old_net_conf;
			...
			if (verify_tfm) {
				...
				bsr_info(BSR_LC_TEMP, device, "using verify-alg: \"%s\"\n", p->verify_alg);
			}
			if (csums_tfm) {
				...
				bsr_info(BSR_LC_TEMP, device, "using csums-alg: \"%s\"\n", p->csums_alg);
			}
#ifdef _WIN
			synchronize_rcu_w32_wlock();
#endif
...
#endif
	if (new_peer_device_conf) {
		bsr_info(BSR_LC_TEMP, peer_device, "sync, resync_rate : %uk, c_plan_ahead : %uk, c_delay_target : %uk, c_fill_target : %us, c_max_rate : %uk, c_min_rate : %uk, ov_req_num : %ub, ov_req_interval : %ums\n",
	...
	const char *s, sector_t a, sector_t b)
{
	...
	d = (a > b) ? (a - b) : (b - a);
	if (d > (a>>3) || d > (b>>3))
		bsr_warn(BSR_LC_VOLUME, peer_device, "Considerable difference in %s: %llu bytes vs. %llu bytes\n", s,
...
static void maybe_trigger_resync(struct bsr_device *device, struct bsr_peer_device *peer_device, bool grew, bool skip)
{
	...
	if (test_and_clear_bit(RESIZE_PENDING, &peer_device->flags) ||
		...
		if (peer_device->disk_state[NOW] >= D_INCONSISTENT &&
			...
			if (skip)
				bsr_info(BSR_LC_TEMP, peer_device, "Resync of new storage suppressed with --assume-clean\n");
			else
...
static int receive_sizes(struct bsr_connection *connection, struct packet_info *pi)
{
    ...
    bsr_info(BSR_LC_TEMP, device, "current_mydisk_size: %llu bytes\n", (unsigned long long)(cur_size<<9));
    bsr_info(BSR_LC_TEMP, peer_device, "peer_current_size: %llu bytes peer_user_size: %llu bytes peer_disk_size: %llu bytes peer_max_size: %llu bytes\n",
	...
	if ((p_size && p_csize > p_size) || (p_usize && p_csize > p_usize)) {
		bsr_warn(BSR_LC_VOLUME, peer_device, "Peer sent bogus sizes, disconnecting\n");
		goto disconnect;
	}
	...
	*/
	...
	if (get_ldev(device)) {
		...
		my_max_size = bsr_get_max_capacity(device->ldev);
		        bsr_info(BSR_LC_TEMP, peer_device, "md_effective_size: %llu my_user_size: %llu my_max_size: %llu\n",
		...
		if (new_size < cur_size &&
            ...
            bsr_err(BSR_LC_TEMP, peer_device, "The peer's disk size is too small! (%llu bytes < %llu bytes)\n",
		...
		if (my_max_size < p_csize && !is_handshake) {
			bsr_err(BSR_LC_TEMP, peer_device, "Peer's size larger than my maximum capacity (%llu < %llu sectors)\n",
		...
		if (my_usize != p_usize) {
			...
			if (!new_disk_conf) {
				bsr_err(BSR_LC_TEMP, device, "Allocation of new disk_conf failed\n");
				err = -ENOMEM;
				goto out;
			}
			...
			*new_disk_conf = *old_disk_conf;
			...
			bsr_info(BSR_LC_TEMP, peer_device, "Peer sets u_size to %llu sectors\n",
	...
	if (have_ldev) {
		...
		bsr_info(BSR_LC_TEMP, peer_device, "calling bsr_determine_dev_size()\n");		
	...
	if (device->device_conf.max_bio_size > protocol_max_bio_size ||
		...
		bsr_err(BSR_LC_TEMP, device, "Peer cannot deal with requests bigger than %u. "
			...
			enum resync_reason reason) __must_hold(local)
{
	...
	if (new_repl_state == -1) {
		bsr_info(BSR_LC_TEMP, peer_device, "Unexpected result of handshake() %d!\n", new_repl_state);
		...
		bitmap_mod_after_handshake(peer_device, hg, peer_node_id);
		bsr_info(BSR_LC_TEMP, peer_device, "Becoming %s %s\n", bsr_repl_str(new_repl_state),
	...
	if (new_repl_state == L_ESTABLISHED && peer_disk_state >= D_CONSISTENT &&
		...
		/* No resync with up-to-date peer -> I should be consistent or up-to-date as well.
		   ...
		   as well. */
		bsr_info(BSR_LC_TEMP, peer_device, "Upgrading local disk to %s after unstable/weak (and no resync).\n",
	...
	if ((rv == SS_NOTHING_TO_DO || rv == SS_RESYNC_RUNNING) &&
		...
		bsr_info(BSR_LC_TEMP, peer_device, "...resync is already in progress.\n");
	}
}
...
static void bsr_resync_authoritative(struct bsr_peer_device *peer_device, enum bsr_repl_state side)
{
	...
	if (new_repl_state == -1) {
		bsr_info(BSR_LC_TEMP, peer_device, "Invalid resync side %s\n", bsr_repl_str(side));
		return;
	}
	...
	if (abs(hg) >= 100)	{
		bsr_err(BSR_LC_TEMP, peer_device, "Can not start resync due to unexpected handshake result(%d)\n", hg);
		// DW-1360 destroy connection for conflicted data.
		change_cstate_ex(peer_device->connection, C_DISCONNECTING, CS_HARD);
		return;
	}

	bsr_info(BSR_LC_TEMP, peer_device, "Becoming %s due to authoritative node changed\n", bsr_repl_str(new_repl_state));
...
static int __receive_uuids(struct bsr_peer_device *peer_device, u64 node_mask)
{
	...
	if (peer_device->connection->agreed_pro_version < 110 && bad_server) {
		bsr_err(BSR_LC_TEMP, device, "Can only connect to data with current UUID=%016llX\n",
	...
	if (get_ldev(device)) {
		...
		if (skip_initial_sync) {
			...
			bsr_info(BSR_LC_TEMP, device, "Accepted new current UUID, preparing to skip initial sync\n");
		...
		if (peer_device->uuid_flags & UUID_FLAG_NEW_DATAGEN) {
			bsr_info(BSR_LC_TEMP, peer_device, "received new current UUID: %016llX\n", peer_device->current_uuid);
			bsr_uuid_received_new_current(peer_device, peer_device->current_uuid, node_mask);
		}
		else {
			bsr_warn(BSR_LC_UUID, peer_device, "receive new current but not update UUID: %016llX\n", peer_device->current_uuid);
		}
		...
		if (resource->state_change_flags) {
			bsr_info(BSR_LC_TEMP, peer_device, "Delaying update of exposed data uuid\n");
...
static int receive_uuids110(struct bsr_connection *connection, struct packet_info *pi)
{
	...
	if ((peer_device->repl_state[NOW] >= L_STARTING_SYNC_S && peer_device->repl_state[NOW] <= L_WF_BITMAP_T) ||
		...
		if (!bsr_inspect_resync_side(peer_device, peer_device->repl_state[NOW], NOW, false)) {
			...
			bsr_info(BSR_LC_TEMP, peer_device, "Resync will be aborted since peer goes unsyncable.\n");
			begin_state_change(device->resource, &irq_flags, CS_VERBOSE);
			  ...
			  enum chg_state_flags flags)
{
	...
	if (mask.i) {
		bsr_info(BSR_LC_TEMP, connection, "Remote state change: request %u/%u not "
		"understood\n", mask.i, val.i & mask.i);
			   ...
			   union bsr_state mask, union bsr_state val)
{
	...
	if (mask.i) {
		bsr_info(BSR_LC_TEMP, peer_device, "Remote state change: request %u/%u not "
		"understood\n", mask.i, val.i & mask.i);
		return SS_NOT_SUPPORTED;
	}
	return SS_SUCCESS;
...
static int receive_req_state(struct bsr_connection *connection, struct packet_info *pi)
{
	...
	if (!expect(connection, connection->agreed_pro_version < 110)) {
		bsr_err(BSR_LC_TEMP, connection, "Packet %s not allowed in protocol version %d\n",
	...
	if (rv != SS_SUCCESS) {
		bsr_info(BSR_LC_TEMP, connection, "Rejecting concurrent remote state change\n");
...
#endif
{
	...
	if (resource->twopc_work.cb == NULL) {
		bsr_err(BSR_LC_TEMP, resource, "Two-phase commit %u timeout\n",
...
check_concurrent_transactions(struct bsr_resource *resource, struct twopc_reply *new_r)
{
	...
	if (new_r->initiator_node_id < ongoing->initiator_node_id) {
		if ((unsigned int)ongoing->initiator_node_id == resource->res_opts.node_id) {
			bsr_info(BSR_LC_TEMP, resource, "[TWOPC] CSC_ABORT_LOCAL! new_r->initiator_node_id (%d) ongoing->initiator_node_id (%d)\n",
					new_r->initiator_node_id, ongoing->initiator_node_id);
			return CSC_ABORT_LOCAL;
		}
		else {
			bsr_info(BSR_LC_TEMP, resource, "[TWOPC] CSC_QUEUE! new_r->initiator_node_id (%d) ongoing->initiator_node_id (%d)\n",
					new_r->initiator_node_id, ongoing->initiator_node_id);
			return CSC_QUEUE;
		}
		...
		if (ongoing->is_disconnect && new_r->tid != ongoing->tid) {
			bsr_info(BSR_LC_TEMP, resource, "[TWOPC] CSC_UPDATE! new_r->initiator_node_id (%d), new_r->tid (%u)\n",
				new_r->initiator_node_id, new_r->tid);
			del_timer(&resource->twopc_timer);
			clear_remote_state_change_without_lock(resource);
			return CSC_UPDATE;
		}
		bsr_info(BSR_LC_TEMP, resource, "[TWOPC] CSC_REJECT! new_r->initiator_node_id (%d) ongoing->initiator_node_id (%d)\n",
					new_r->initiator_node_id, ongoing->initiator_node_id);

		return CSC_REJECT;
	}
	if (new_r->tid != ongoing->tid) {
		bsr_info(BSR_LC_TEMP, resource, "[TWOPC] CSC_TID_MISS! new_r->tid (%u) ongoing->tid (%u)\n",
...
static int queued_twopc_work(struct bsr_work *w, int cancel)
{
	...
	while (true) {
		if (jiffies - q->start_jif >= t || cancel) {
			if (!cancel)
				bsr_info(BSR_LC_TEMP, connection, "Rejecting concurrent "
...
bsr_commit_size_change(struct bsr_device *device, struct resize_parms *rs, u64 nodes_to_reach)
{
	...
	if (my_usize != tr->user_size) {
        ...
        *new_disk_conf = *old_disk_conf;
        ...
        bsr_info(BSR_LC_TEMP, device, "New u_size %llu sectors\n",
             ...
             ULONG_PTR receive_jif)
{
	...
	if (connection == NULL)
		bsr_err(BSR_LC_TEMP, NO_OBJECT, "process_twopc connection is null\n");
	...
	bsr_info(BSR_LC_TEMP, resource, "[TWOPC:%u] target_node_id (%d) csc_rv (%d) primary_nodes (%llu) pi->cmd (%s)\n", 
	...
	if (csc_rv == CSC_CLEAR && pi->cmd != P_TWOPC_ABORT) {
		if (!is_prepare(pi->cmd)) {
			...
			spin_unlock_irq(&resource->req_lock);
			bsr_debug(BSR_LC_TEMP, connection, "Ignoring %s packet %u\n",
				   bsr_packet_name(pi->cmd),
				   reply->tid);
			...
			if(resource->role[NEW] == R_SECONDARY && reply->primary_nodes != 0 ) {
				struct bsr_device *device;
				int vnr;
				bsr_info(BSR_LC_TEMP, resource, "Peer node is Primary. LastPrimary flag set [TWOPC:%u] target_node_id (%d) primary_nodes (%llu) pi->cmd (%s)\n", 
				...
				idr_for_each_entry_ex(struct bsr_device *, &resource->devices, device, vnr) {
					if(get_ldev_if_state(device, D_NEGOTIATING)) {
						...
						bsr_info(BSR_LC_TEMP, resource, "LastPrimary got it! But disk state is diskless or failed device->disk_state:%d\n",device->disk_state[NEW]);
					}
				} 
			}
			...
			if (resource->twopc_type == TWOPC_STATE_CHANGE && pi->cmd == P_TWOPC_COMMIT) {
				...
				if (reply->target_node_id == (int)resource->res_opts.node_id &&
					...
					bsr_err(BSR_LC_TEMP, connection, "[TWOPC:%u] target_node_id (%d) unexpected packet \n",
		...
		if (test_and_set_bit(TWOPC_EXECUTED, &resource->flags)) {
			spin_unlock_irq(&resource->req_lock);
			bsr_info(BSR_LC_TEMP, connection, "Ignoring redundant %s packet %u.\n",
					bsr_packet_name(pi->cmd),
					reply->tid);
			return 0;
		}
		...
		bsr_info(BSR_LC_TEMP, connection, "Aborting local state change %u to yield to remote "
			  "state change %u.\n",
			  resource->twopc_reply.tid,
			  reply->tid);
		...
		if (alt_rv == ALT_MATCH) {
			...
			/* abort_local_transaction() comes back unlocked in this case ... */
			bsr_info(BSR_LC_TEMP, connection, "Aborting local state change %u "
		...
		if (err) {
			...
			if (err)
				bsr_info(BSR_LC_TEMP, connection, "Ignoring %s packet %u.\n",
					  bsr_packet_name(pi->cmd),
					  reply->tid);
		}

		bsr_info(BSR_LC_TEMP, resource, "[TWOPC:%u] target_node_id (%d) abort_starting_twopc \n", 
					reply->tid, reply->target_node_id);
			...
			match: 
				...
				if (!reply_cmd) {
					...
					if(list_add_valid(&connection->twopc_parent_list, &resource->twopc_parents)) {	
						...
						bsr_info(BSR_LC_TEMP, connection, "twopc_parent_list(%p) already added.\n", &connection->twopc_parent_list);
						kref_debug_put(&connection->kref_debug, 9);
						kref_put(&connection->kref, bsr_destroy_connection);
					}
				}
	...
	case P_TWOPC_PREPARE:
		bsr_info(BSR_LC_TEMP, connection, "Preparing remote state change %u "
	...
	case P_TWOPC_PREP_RSZ:
		...
		break;
	case P_TWOPC_ABORT:
		bsr_info(BSR_LC_TEMP, connection, "Aborting remote state change %u\n",
			  reply->tid);
		flags |= CS_ABORT;
		break;
	case P_TWOPC_COMMIT:
		bsr_info(BSR_LC_TEMP, connection, "Committing remote state change %u\n",
			  reply->tid);
		break;
	default:
		BUG();
	...
	if (flags & CS_PREPARE) {
		...
		if( (resource->role[NEW] != R_PRIMARY) && (reply->primary_nodes != 0) ) {
			struct bsr_device *device;
			int vnr;
			bsr_info(BSR_LC_TEMP, resource, "Peer node is Primary. LastPrimary flag set [TWOPC:%u] after clear_remote_state_change target_node_id (%d) primary_nodes (%llu) pi->cmd (%s)\n", 
				reply->tid, reply->target_node_id, reply->primary_nodes, bsr_packet_name(pi->cmd));
			idr_for_each_entry_ex(struct bsr_device *, &resource->devices, device, vnr) {
				if(get_ldev_if_state(device, D_NEGOTIATING)) {
					bsr_md_clear_flag (device, MDF_LAST_PRIMARY );
...
static void try_to_get_resynced(struct bsr_device *device)
{
	...
	for_each_peer_device_rcu(peer_device, device) {
		...
		if (peer_device->disk_state[NOW] == D_UP_TO_DATE) {
			hg = bsr_uuid_compare(peer_device, &rule_nr, &peer_node_id);
			bsr_info(BSR_LC_TEMP, peer_device, "hg = %d\n", hg);
			if (hg <= 0 && hg > best_hg) {
				best_hg = hg;
...
static int receive_state(struct bsr_connection *connection, struct packet_info *pi)
{
	...
	if (peer_state.disk == D_NEGOTIATING) {
		...
		bsr_info(BSR_LC_TEMP, device, "real peer disk state = %s\n", bsr_disk_str(peer_disk_state));
	}
	// DW-2054 if state is received in D_ATTACHING condition, it induces re-connection.
	else if (device->disk_state[NOW] == D_ATTACHING) {
		bsr_info(BSR_LC_TEMP, device, "reconnect because initialization packets were received in D_ATTACHING state");
 ...
 retry:
		 ...
		 * Maybe we should finish it up, too? */
			...
			if (finish_now || old_peer_state.conn == L_SYNC_SOURCE ||
...
#ifdef _DEBUG_OOS
				{
					...
					bsr_info(BSR_LC_TEMP, peer_device, "SyncSource still sees bits set!! FIXME, total(%llu), failed(%llu)\n", bsr_bm_total_weight(peer_device), peer_device->rs_failed);
					if (atomic_read(&g_featurelog_flag) & FEATURELOG_FLAG_OOS) {
						...
						do {
							...
							bsr_oos(BSR_LC_OUT_OF_SYNC, NO_OBJECT, "["OOS_TRACE_STRING"] pnode-id(%d), bitmap_index(%d), out-of-sync for sector(%llu) is remaining\n",
	...
	if (connection->agreed_pro_version >= 110 &&
		...
		bsr_info(BSR_LC_TEMP, peer_device, "peer is SyncSource. change to SyncTarget\n"); // DW-1518
		bsr_start_resync(peer_device, L_SYNC_TARGET);
		return 0;
	}
	...
	if (peer_state.conn == L_AHEAD) {
		if (old_peer_state.conn != L_BEHIND) {
			bsr_info(BSR_LC_TEMP, peer_device, "peer is Ahead. change to Behind mode\n"); // DW-1518
		}
	...
	if (peer_device->uuids_received &&
		...
		if( (peer_state.role == R_PRIMARY) && (device->resource->role[NOW] == R_PRIMARY) ) {
			bsr_err(BSR_LC_TEMP, device, "2 primary is not allowed.\n");
			put_ldev(device);
			goto fail;
		}
		...
		if (new_repl_state == -1) {
			...
			if (device->disk_state[NOW] == D_NEGOTIATING) {
				...
				if (connection->agreed_pro_version < 110) {
					bsr_err(BSR_LC_TEMP, device, "Disk attach process on the peer node was aborted.\n");
		...
		if (peer_device->current_uuid == bsr_current_uuid(device)) {
			bsr_info(BSR_LC_TEMP, peer_device, "Upgrading local disk to D_UP_TO_DATE since current UUID matches.\n");
	...
	bsr_info(BSR_LC_TEMP, peer_device, "receive new repl state \"%s\"\n", bsr_repl_str(new_repl_state));
	...
	if (peer_device->disk_state[NEW] == D_CONSISTENT &&
		...
		bsr_err(BSR_LC_TEMP, device, "Aborting Connect, can not thaw IO with an only Consistent peer\n");
		tl_clear(connection);
	...
	if (rv < SS_SUCCESS) {
		...
		if (connection->cstate[NOW] == C_CONNECTING || rv == SS_NEED_CONNECTION){
			bsr_info(BSR_LC_TEMP, peer_device, "connection->cstate[OLD] == C_CONNECTING, change cstate to NETWORK_FAILURE instead of DISCONNECTING\n");
...
static int receive_sync_uuid(struct bsr_connection *connection, struct packet_info *pi)
{
	...
	if (get_ldev_if_state(device, D_NEGOTIATING)) {
		...
		bsr_err(BSR_LC_TEMP, device, "Ignoring SyncUUID packet!\n");
		 ...
		 unsigned int len)
{
	...
	for (have = bits; have > 0; s += (ULONG_PTR)rl, toggle = !toggle) {
		...
		if (toggle) {
			e = s + (ULONG_PTR)rl -1;
			if (e >= c->bm_bits) {
				bsr_err(BSR_LC_TEMP, peer_device, "bitmap overflow (e:%lu) while decoding bm RLE packet\n", e);
				return -EIO;
			}
		...
		if (have < bits) {
			bsr_err(BSR_LC_TEMP, peer_device, "bitmap decoding error: h:%d b:%d la:0x%08llx l:%u/%u\n",
		...
		unsigned int len)
{
	...
	bsr_err(BSR_LC_TEMP, peer_device, "receive_bitmap_c: unknown encoding %u\n", p->encoding);
		...
		const char *direction, struct bm_xfer_ctx *c)
{
	...
	r = 1000 - r;
	bsr_info(BSR_LC_TEMP, peer_device, "%s bitmap stats [Bytes(packets)]: plain %u(%u), RLE %u(%u), "
...
static int receive_bitmap_finished(struct bsr_connection *connection, struct bsr_peer_device *peer_device, bool *unlocked)
{
	...
	*unlocked = false;
	...
	if (peer_device->repl_state[NOW] == L_WF_BITMAP_T ||
		...
		bsr_info(BSR_LC_TEMP, peer_device, "unexpected repl_state (%s) in receive_bitmap\n",
	...
	if (peer_device->repl_state[NOW] == L_WF_BITMAP_S ||
		...
		if (connection->agreed_pro_version >= 113) {
			...
			bsr_info(BSR_LC_TEMP, peer_device, "send that bitmap exchange has been completed\n");
...
static int receive_bitmap(struct bsr_connection *connection, struct packet_info *pi)
{
	...
	if (peer_device->bitmap_index == -1) {
		bsr_err(BSR_LC_TEMP, peer_device, "No bitmap allocated in receive_bitmap()!\n");
		return -EIO;
	}
	device = peer_device->device;
	...
	if (pi->cmd == P_BITMAP) {
		...
		* and the feature is enabled! */
		...
		if (pi->size > BSR_SOCKET_BUFFER_SIZE - bsr_header_size(connection)) {
			bsr_err(BSR_LC_TEMP, device, "ReportCBitmap packet too large\n");
			err = -EIO;
			goto out;
		}
		if (pi->size <= sizeof(*p)) {
			bsr_err(BSR_LC_TEMP, device, "ReportCBitmap packet too small (l:%u)\n", pi->size);
			err = -EIO;
			goto out;
		}
		...
		bsr_warn(BSR_LC_BITMAP, device, "receive_bitmap: cmd neither ReportBitMap nor ReportCBitMap (is 0x%x)", pi->cmd);
		err = -EIO;
		goto out;
	}
...
{
	bsr_warn(BSR_LC_PROTOCOL, connection, "skipping unknown optional packet type %d, l: %d!\n",
		 pi->cmd, pi->size);
...
static int list_add_resync_pending(struct bsr_device* device, sector_t sst, sector_t est)
{
	...
	if (pending_st) {
		...
		if (!pending_st) {
			bsr_err(BSR_LC_TEMP, device, "failed to resync pending bits allocate, sector : %llu ~ %llu\n", (unsigned long long)sst, (unsigned long long)est);
...
static int receive_out_of_sync(struct bsr_connection *connection, struct packet_info *pi)
{
	...
	case L_SYNC_TARGET: 
		...
		break; 
	default:
		bsr_info(BSR_LC_TEMP, device, "ASSERT FAILED cstate = %s, expected: WFSyncUUID|WFBitMapT|Behind\n",
				bsr_repl_str(peer_device->repl_state[NOW]));
	}
	...
	if (bResetTimer) {
		bsr_info(BSR_LC_TEMP, peer_device, "received out-of-sync has been set after resync timer has been expired, restart timer to send rs request for rest\n");
	...
	if (new_repl_state != L_ESTABLISHED) {
...
#ifdef _TRACE_PEER_DAGTAG
		bsr_info(BSR_LC_TEMP, connection, "Reconciliation resync because \'%s\' disappeared. (o=%d) lost_peer:%p lost_peer->last_dagtag_sector:0x%llx be64_to_cpu(p->dagtag):%llx\n",
			  lost_peer->transport.net_conf->name, (int)dagtag_offset, lost_peer, lost_peer->last_dagtag_sector, be64_to_cpu(p->dagtag));
#else
		bsr_info(BSR_LC_TEMP, connection, "Reconciliation resync because \'%s\' disappeared. (o=%d)\n",
		...
		idr_for_each_entry_ex(struct bsr_peer_device *, &connection->peer_devices, peer_device, vnr) {
			if (new_repl_state == L_WF_BITMAP_S && test_bit(RECONCILIATION_RESYNC, &peer_device->flags)) {
				if (rv != SS_SUCCESS) {
					bsr_debug(BSR_LC_TEMP, peer_device, "Disable RECONCILIATION_RESYNC flag.\n");
...
#ifdef _TRACE_PEER_DAGTAG	
		bsr_info(BSR_LC_TEMP, connection, "No reconciliation resync even though \'%s\' disappeared. (o=%d) lost_peer:%p lost_peer->last_dagtag_sector:0x%llx be64_to_cpu(p->dagtag):%llx\n",
			  lost_peer->transport.net_conf->name, (int)dagtag_offset, lost_peer, lost_peer->last_dagtag_sector, be64_to_cpu(p->dagtag));
#else
		bsr_info(BSR_LC_TEMP, connection, "No reconciliation resync even though \'%s\' disappeared. (o=%d)\n",
			lost_peer->transport.net_conf->name, (int)dagtag_offset);
#endif
		...
		idr_for_each_entry_ex(struct bsr_peer_device *, &connection->peer_devices, peer_device, vnr) {	
			...
			if(peer_device->disk_state[NOW] > D_INCONSISTENT && get_ldev_if_state(peer_device->device, D_OUTDATED)) {
				...
				bsr_info(BSR_LC_TEMP, connection, "No bsr_bm_clear_many_bits, disk_state:%d peer disk_state:%d\n",
...
static int receive_current_uuid(struct bsr_connection *connection, struct packet_info *pi)
{
	...
	if (current_uuid == bsr_current_uuid(device)) {
		...
		if (bsr_current_uuid(device) != 0 && device->ldev) {
			...
			if (peer_md->bitmap_uuid != 0) {
				bsr_info(BSR_LC_TEMP, peer_device, "Clear bitmap_uuid (cur_uuid:%016llX bm_uuid:%016llX)\n", current_uuid, peer_md->bitmap_uuid);
				bsr_uuid_set_bitmap(peer_device, 0);
			}
		}
	...
	if (get_ldev_if_state(device, D_UP_TO_DATE)) {
		if (connection->peer_role[NOW] == R_PRIMARY) {
			bsr_warn(BSR_LC_UUID, peer_device, "received new current UUID: %016llX "
...
static void bsrd(struct bsr_connection *connection)
{
	...
	while (get_t_state(&connection->receiver) == RUNNING) {
		...
		if (unlikely(pi.cmd >= ARRAY_SIZE(bsr_cmd_handler) || !cmd->fn)) {
			bsr_err(BSR_LC_TEMP, connection, "Unexpected data packet %s (0x%04x)",
		...
		if (pi.size > shs && !cmd->expect_payload) {
			bsr_err(BSR_LC_TEMP, connection, "No payload expected %s l:%d\n",
				 bsr_packet_name(pi.cmd), pi.size);
			goto err_out;
		}
		if (pi.size < shs) {
			bsr_err(BSR_LC_TEMP, connection, "%s: unexpected packet size, expected:%d received:%d\n",
		...
		bsr_debug(BSR_LC_TEMP, connection, "receiving %s, size: %u vnr: %d\n", bsr_packet_name(pi.cmd), pi.size, pi.vnr);
		err = cmd->fn(connection, &pi);
		if (err) {
			bsr_err(BSR_LC_TEMP, connection, "error receiving %s, e: %d l: %u!\n",
...
static void cleanup_resync_leftovers(struct bsr_peer_device *peer_device)
{
	...
	*  on the fly. */
	...
	if (peer_device->rs_send_req != peer_device->rs_recv_res ||
		...
		bsr_info(BSR_LC_TEMP, peer_device, "incomplete resync exit, rs_send_req(%llu), rs_recv_res(%llu), rs_written(%lld)\n",
...
void conn_disconnect(struct bsr_connection *connection)
{
	...
	* peer_request queued to the submitter workqueue. */
	...
	if (!list_empty(&connection->active_ee)) {
		list_for_each_entry_ex(struct bsr_peer_request, peer_req, &connection->active_ee, w.list) {
			...
			bsr_info(BSR_LC_TEMP, device, "add, active_ee => inactive_ee(%p), sector(%llu), size(%u)\n", 
	...
	if (!list_empty(&connection->sync_ee)) {
		list_for_each_entry_ex(struct bsr_peer_request, peer_req, &connection->sync_ee, w.list) {
			struct bsr_device *device = peer_req->peer_device->device; 
			bsr_info(BSR_LC_TEMP, device, "add, sync_ee => inactive_ee(%p), sector(%llu), size(%u)\n", 
				peer_req, (unsigned long long)peer_req->i.sector, peer_req->i.size); 
		}
	...
	if (!list_empty(&connection->read_ee)) {
		list_for_each_entry_ex(struct bsr_peer_request, peer_req, &connection->read_ee, w.list) {
			struct bsr_device *device = peer_req->peer_device->device;
			bsr_info(BSR_LC_TEMP, device, "add, read_ee => inactive_ee(%p), sector(%llu), size(%u)\n", 
				peer_req, (unsigned long long)peer_req->i.sector, peer_req->i.size); 
		}
	...
	* to be "canceled" */
	...
	if (i)
		bsr_err(BSR_LC_TEMP, connection, "read_ee not empty, killed %d entries\n", i);
	i = bsr_free_peer_reqs(resource, &connection->active_ee, true);
	if (i)
		bsr_err(BSR_LC_TEMP, connection, "active_ee not empty, killed %d entries\n", i);
	i = bsr_free_peer_reqs(resource, &connection->sync_ee, true);
	if (i)
		bsr_err(BSR_LC_TEMP, connection, "sync_ee not empty, killed %d entries\n", i);
	i = bsr_free_peer_reqs(resource, &connection->net_ee, true);
	if (i)
		bsr_err(BSR_LC_TEMP, connection, "net_ee not empty, killed %d entries\n", i);
	...
	if (i)
		bsr_info(BSR_LC_TEMP, connection, "pp_in_use = %d, expected 0\n", i);
	i = atomic_read(&connection->pp_in_use_by_net);
	if (i)
		bsr_info(BSR_LC_TEMP, connection, "pp_in_use_by_net = %d, expected 0\n", i);

	if (!list_empty(&connection->current_epoch->list)) {
		struct bsr_epoch *epoch;
		bsr_err(BSR_LC_TEMP, connection, "ASSERTION FAILED: connection->current_epoch->list not empty\n");
		...
		list_for_each_entry_ex(struct bsr_epoch, epoch, &connection->current_epoch->list, list) {
			bsr_info(BSR_LC_TEMP, connection, "ASSERTION FAILED: remove epoch barrier_nr : %u, epochs:%u\n", epoch->barrier_nr, connection->epochs);
			list_del(&epoch->list);
			kfree(epoch);
			connection->epochs--;
			if (list_empty(&connection->current_epoch->list))
	...
	bsr_info(BSR_LC_TEMP, connection, "Connection closed\n");
...
int bsr_do_features(struct bsr_connection *connection)
{
	...
	if (err){
		bsr_debug_conn("fail bsr_send_feature err = %d\n", err); 
		return 0;
	}
	...
	if (err) {
		...
		if (err == -EAGAIN)
			bsr_err(BSR_LC_TEMP, connection, "timeout while waiting for feature packet\n");
		return 0;
	}
	bsr_debug_conn("success bsr_recv_header\n");
	if (pi.cmd != P_CONNECTION_FEATURES) {
		bsr_err(BSR_LC_TEMP, connection, "expected ConnectionFeatures packet, received: %s (0x%04x)\n",
			 bsr_packet_name(pi.cmd), pi.cmd);
		return -1;
	}

	if (pi.size != (unsigned int)expect) {
		bsr_err(BSR_LC_TEMP, connection, "expected ConnectionFeatures length: %u, received: %u\n",
		     expect, pi.size);
		return -1;
	}
	...
	if (PRO_VERSION_MAX < p->protocol_min ||
		...
		bsr_err(BSR_LC_TEMP, connection, "incompatible BSR dialects: "
	...
	if (connection->agreed_pro_version < 110) {
		...
		if (multiple) {
			bsr_err(BSR_LC_TEMP, connection, "Peer supports protocols %d-%d, but "
	...
	if (connection->agreed_pro_version >= 110) {
		if (be32_to_cpu(p->sender_node_id) != connection->peer_node_id) {
			bsr_err(BSR_LC_TEMP, connection, "Peer presented a node_id of %d instead of %d\n",
				 be32_to_cpu(p->sender_node_id), connection->peer_node_id);
			return 0;
		}
		if (be32_to_cpu(p->receiver_node_id) != resource->res_opts.node_id) {
			bsr_err(BSR_LC_TEMP, connection, "Peer expects me to have a node_id of %d instead of %d\n",
				 be32_to_cpu(p->receiver_node_id), resource->res_opts.node_id);
			return 0;
		}
	}

	bsr_info(BSR_LC_TEMP, connection, "Handshake to peer %d successful: "
		  "Agreed network protocol version %d\n",
		  connection->peer_node_id,
		  connection->agreed_pro_version);


	bsr_info(BSR_LC_TEMP, connection, "Feature flags enabled on protocol level: 0x%x%s%s%s.\n",
...
int bsr_do_auth(struct bsr_connection *connection)
{
	bsr_err(BSR_LC_TEMP, connection, "This kernel was build without CONFIG_CRYPTO_HMAC.\n");
	bsr_err(BSR_LC_TEMP, connection, "You need to disable 'cram-hmac-alg' in bsr.conf.\n");
	return -1;
}
#else
...
int bsr_do_auth(struct bsr_connection *connection)
{
	...
	if (rv) {
		bsr_err(BSR_LC_TEMP, connection, "crypto_shash_setkey() failed with %d\n", rv);
		rv = -1;
		goto fail;
	}
	...
	if (pi.cmd != P_AUTH_CHALLENGE) {
		bsr_err(BSR_LC_TEMP, connection, "expected AuthChallenge packet, received: %s (0x%04x)\n",
			 bsr_packet_name(pi.cmd), pi.cmd);
		rv = 0;
		goto fail;
	}
	...
	if (pi.size != sizeof(peers_ch->d)) {
		bsr_err(BSR_LC_TEMP, connection, "unexpected AuthChallenge payload.\n");
		rv = -1;
		goto fail;
	}
	...
	if (peers_ch == NULL) {
		bsr_err(BSR_LC_TEMP, connection, "kmalloc of peers_ch failed\n");
		rv = -1;
		goto fail;
	}
	...
	if (!memcmp(my_challenge.d, peers_ch->d, sizeof(my_challenge.d))) {
		bsr_err(BSR_LC_TEMP, connection, "Peer presented the same challenge!\n");
		rv = -1;
		goto fail;
	}
	...
	if (rv) {
		bsr_err(BSR_LC_TEMP, connection, "crypto_shash_digest() failed with %d\n", rv);
		rv = -1;
		goto fail;
	}
	...
	if (pi.cmd != P_AUTH_RESPONSE) {
		bsr_err(BSR_LC_TEMP, connection, "expected AuthResponse packet, received: %s (0x%04x)\n",
			 bsr_packet_name(pi.cmd), pi.cmd);
		rv = 0;
		goto fail;
	}
	...
	if (pi.size != resp_size) {
		bsr_err(BSR_LC_TEMP, connection, "expected AuthResponse payload of %u bytes, received %u\n",
				resp_size, pi.size);
		rv = 0;
		goto fail;
	}
	...
	if (right_response == NULL) {
		bsr_err(BSR_LC_TEMP, connection, "kmalloc of right_response failed\n");
		rv = -1;
		goto fail;
	}
	...
	if (rv)
		bsr_info(BSR_LC_TEMP, connection, "Peer authenticated using %d bytes HMAC\n",
		     resp_size);
	else
		rv = -1;
...
static int got_RqSReply(struct bsr_connection *connection, struct packet_info *pi)
{
	...
	if (retcode >= SS_SUCCESS)
		...
		set_bit(TWOPC_NO, &connection->flags);
		bsr_debug(BSR_LC_TEMP, connection, "Requested state change failed by peer: %s (%d)\n",
...
static int got_twopc_reply(struct bsr_connection *connection, struct packet_info *pi)
{
	...
	if ((unsigned int)resource->twopc_reply.initiator_node_id == be32_to_cpu(p->initiator_node_id) &&
		...
		bsr_debug(BSR_LC_TEMP, connection, "Got a %s reply for state change %u\n",
			   bsr_packet_name(pi->cmd),
			   resource->twopc_reply.tid);
		...
		bsr_debug(BSR_LC_TEMP, connection, "Ignoring %s reply for state change %u\n",
			   bsr_packet_name(pi->cmd),
			   be32_to_cpu(p->tid));
	}
	spin_unlock_irq(&resource->req_lock);
...
#ifdef BSR_TRACE	
	bsr_debug(BSR_LC_TEMP, NO_OBJECT,"(%s) validate_req_change_req_state: before __req_mod! IRQL(%d) \n", current->comm, KeGetCurrentIrql());
#endif
	__req_mod(req, what, peer_device, &m);
	spin_unlock_irq(&device->resource->req_lock);

#ifdef BSR_TRACE	
	bsr_debug(BSR_LC_TEMP, NO_OBJECT,"(%s) validate_req_change_req_state: after __req_mod! IRQL(%d) \n", current->comm, KeGetCurrentIrql());
#endif
...
#ifdef BSR_TRACE
	bsr_debug(BSR_LC_TEMP, NO_OBJECT,"pi-cmd 0x%x(%s) sect:0x%llx sz:%d\n", pi->cmd, bsr_packet_name(pi->cmd), sector, blksize);
#endif
...
static int got_NegAck(struct bsr_connection *connection, struct packet_info *pi)
{
	...
	if (connection->agreed_pro_version >= 113) {
		...
		if (p->block_id == ID_SYNCER_SPLIT || p->block_id == ID_SYNCER_SPLIT_DONE) {
			bsr_debug(BSR_LC_TEMP, connection, "bsr_rs_failed_io sector : %llu, size %d\n", (unsigned long long)sector, size);
...
static int got_NegDReply(struct bsr_connection *connection, struct packet_info *pi)
{
	...
	bsr_err(BSR_LC_TEMP, device, "Got NegDReply; Sector %llus, len %u.\n",
...
#ifdef BSR_TRACE
	bsr_debug(BSR_LC_TEMP, NO_OBJECT,"do tl_release\n");
#endif
...
static int got_peer_ack(struct bsr_connection *connection, struct packet_info *pi)
{
...
#ifdef _TRACE_PEER_DAGTAG    
	bsr_info(BSR_LC_TEMP, NO_OBJECT,"got_peer_ack dagtag:%llx in_sync:%llx\n", dagtag, in_sync);
#endif
	...
	bsr_err(BSR_LC_TEMP, connection, "peer request with dagtag %llu not found\n", dagtag);
	return -EIO;
...
#ifdef _TRACE_PEER_DAGTAG			
			bsr_info(BSR_LC_TEMP, NO_OBJECT,"got_peer_ack bsr_set_sync device:%p, peer_req->i.sector:%llx, peer_req->i.size:%d, in_sync_b:%llx, set_sync_mask:%llx\n", 
...
int bsr_ack_receiver(struct bsr_thread *thi)
{
	...
	if (rv < 0)
		bsr_err(BSR_LC_TEMP, connection, "bsr_ack_receiver: ERROR set priority, ret=%d\n", rv);
#endif
	...
	while (get_t_state(thi) == RUNNING) {
		...
		if (connection->resource->breqbuf_overflow_alarm) {
			bsr_err(BSR_LC_TEMP, connection, "bsr_req overflow alarm\n");
		...
		if (test_and_clear_bit(SEND_PING, &connection->flags)) {
			...
			if (ping_ret) {
				...
				if (ping_ret == -EINTR && current->sig == SIGXCPU) {
					bsr_info(BSR_LC_TEMP, NO_OBJECT,"got SIGXCPU during ping\n");
					flush_signals(current);
				}
#endif
				bsr_err(BSR_LC_TEMP, connection, "bsr_send_ping has failed (%d)\n", ping_ret);
				goto reconnect;
			}
		...
		if (likely(rv > 0)) {
			...
			bsr_err(BSR_LC_TEMP, connection, "meta connection shut down by peer.\n");
			goto reconnect;
		} else if (rv == -EAGAIN) {
			...
			if (ping_timeout_active) {
				bsr_err(BSR_LC_TEMP, connection, "PingAck did not arrive in time.\n");
				goto reconnect;
			}
			...
			if (current->sig == SIGXCPU) {
				//bsr_info(BSR_LC_TEMP, NO_OBJECT,"got SIGXCPU during rx.\n");
			}
#endif
			flush_signals(current);
			continue;
		} else {
			bsr_err(BSR_LC_TEMP, connection, "sock_recvmsg returned %d\n", rv);
			goto reconnect;
		}
		...
		if (received == expect && cmd == NULL) {
			...
			if (pi.cmd >= ARRAY_SIZE(ack_receiver_tbl) || !cmd->fn) {
				bsr_err(BSR_LC_TEMP, connection, "Unexpected meta packet %s (0x%04x)\n",
					 bsr_packet_name(pi.cmd), pi.cmd);
				goto disconnect;
			}
			expect = (int)(header_size + cmd->pkt_size);
			if (pi.size != expect - header_size) {
				bsr_err(BSR_LC_TEMP, connection, "Wrong packet size on meta (c: %d, l: %u)\n",
					pi.cmd, pi.size);
				goto reconnect;
			}
			rflags = 0;
		...
		if (received == expect) {
			...
			pi.data = buffer;
			if (cmd) {
				bsr_debug(BSR_LC_TEMP, connection, "receiving %s, l: %d\n", bsr_packet_name(pi.cmd), pi.size); 
				err = cmd->fn(connection, &pi);
				if (err)
					bsr_debug(BSR_LC_TEMP, connection, "receiving error e: %d\n", err);
			}

			if (err) {
#ifdef _WIN
				if (err == -EINTR && current->sig == SIGXCPU) {
					//bsr_info(BSR_LC_TEMP, NO_OBJECT,"got SIGXCPU during fn(%s)\n", bsr_packet_name(pi.cmd));
					flush_signals(current);
					goto ignore_sig;
				}
#else // _LIN
				bsr_err(BSR_LC_TEMP, connection, "%pf failed\n", cmd->fn);
#endif
				goto reconnect;
			}
#ifdef _WIN
...
disconnect:
	...
	bsr_info(BSR_LC_TEMP, connection, "ack_receiver terminated\n");
...
void bsr_send_peer_ack_wf(struct work_struct *ws)
{
	...
	if (process_peer_ack_list(connection)){
		...
		if (connection->cstate[NOW] != C_DISCONNECTING) {
			bsr_debug(BSR_LC_TEMP, connection, "change the connection state to C_UNCONNECTED\n");
			__change_cstate(connection, C_UNCONNECTED);
		}
	}
}
