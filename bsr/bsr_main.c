...
mempool_t *drbd_ee_mempool;		/* peer requests */
...
static int drbd_create_mempools(void)
{
	...
	// BSR-247 drbd_request_mempool tag
	drbd_request_cache = '48DW';
	// BSR-247 drbd_ee_mempool tag
	drbd_ee_cache = '58DW';
#else
	drbd_pp_pool = NULL;
	drbd_bm_ext_cache = NULL;
				      ...
				      struct device_conf *device_conf, struct drbd_device **p_device)
{
	...
	drbd_info(NO_OBJECT,"device:%p q->max_hw_sectors: %x sectors, device->this_bdev->d_size: %lld bytes\n", device, q->max_hw_sectors, device->this_bdev->d_size);
#endif
