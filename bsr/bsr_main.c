		...
		unsigned int set_size)
{
	...
	if (req == NULL) {
		bsr_err(BSR_LC_TEMP, connection, "BAD! BarrierAck #%u received, but no epoch in tl!?\n",
			 (unsigned int)barrier_nr);
		goto bail;
	}
	if (expect_epoch != barrier_nr) {
		bsr_err(BSR_LC_TEMP, connection, "BAD! BarrierAck #%u received, expected #%u!\n",
			(unsigned int)barrier_nr, (unsigned int)expect_epoch);
		goto bail;
	}

	if (expect_size != set_size) {
		bsr_err(BSR_LC_TEMP, connection, "BAD! BarrierAck #%u received with n_writes=%u, expected n_writes=%u!\n",
...
void _tl_restart(struct bsr_connection *connection, enum bsr_req_event what)
{
	...
	tl_for_each_req_ref(req, r, &resource->transfer_log) {
		...
		if (NULL == req->device) {
			bsr_err(BSR_LC_TEMP, NO_OBJECT,"req->device is null! ignore!"); 
			break; 
		}
		peer_device = conn_peer_device(connection, req->device->vnr);
		...
		// DW-689 temporary patch
		if (NULL == peer_device) {
			bsr_err(BSR_LC_TEMP, NO_OBJECT, "peer_device is null! ignore!");
			break; 
		}
...
#endif
{
	...
	if (!thi->nt) {
		bsr_err(BSR_LC_TEMP, NO_OBJECT,"BSR_PANIC: ct_add_thread faild.\n");
...
restart:
	...
	if (thi->t_state == RESTARTING) {
		if (connection)
			bsr_info(BSR_LC_TEMP, connection, "Restarting %s thread\n", thi->name);
		else
			bsr_info(BSR_LC_TEMP, resource, "Restarting %s thread\n", thi->name);
		thi->t_state = RUNNING;
	...
	if (connection)
		bsr_info(BSR_LC_TEMP, connection, "Terminating %s thread\n", thi->name);
	else
		bsr_info(BSR_LC_TEMP, resource, "Terminating %s thread\n", thi->name);
...
int bsr_thread_start(struct bsr_thread *thi)
{
	...
	case NONE:
		if (connection)
			bsr_info(BSR_LC_TEMP, connection, "Starting %s thread (from %s [%d])\n",
				 thi->name, current->comm, current->pid);
		else
			bsr_info(BSR_LC_TEMP, resource, "Starting %s thread (from %s [%d])\n",
				 thi->name, current->comm, current->pid);
		...
		if (IS_ERR(nt)) {
			if (connection)
				bsr_err(BSR_LC_TEMP, connection, "Couldn't start thread\n");
			else
				bsr_err(BSR_LC_TEMP, resource, "Couldn't start thread\n");
	...
	case EXITING:
		...
		if (connection)
			bsr_info(BSR_LC_TEMP, connection, "Restarting %s thread (from %s [%d])\n",
					thi->name, current->comm, current->pid);
		else
			bsr_info(BSR_LC_TEMP, resource, "Restarting %s thread (from %s [%d])\n",
					thi->name, current->comm, current->pid);
		/* fall through */
	case RUNNING:
	case RESTARTING:
...
void _bsr_thread_stop(struct bsr_thread *thi, int restart, int wait)
{
	...
	//bsr_info(BSR_LC_TEMP, NO_OBJECT,"thi(%s) ns(%s) state(%d) waitflag(%d) event(%d)-------------------!\n", 
	...
	if (thi->t_state != ns) {
		...
		//	bsr_info(BSR_LC_TEMP, NO_OBJECT,"cur=(%s) thi=(%s) stop myself\n", current->comm, thi->name ); 
		}
	}
	spin_unlock_irqrestore(&thi->t_lock, flags);
	...
	if (wait) {
#ifdef _WIN
		// bsr_info(BSR_LC_TEMP, NO_OBJECT,"(%s) wait_for_completion. signaled(%d)\n", current->comm, KeReadStateEvent(&thi->stop.wait.wqh_event));
	...
	// bsr_info(BSR_LC_TEMP, NO_OBJECT,"waitflag(%d) signaled(%d). sent stop sig done.\n", wait, KeReadStateEvent(&thi->stop.wait.wqh_event));
			      ...
			      enum bsr_stream bsr_stream)
{
	...
	if (sbuf->pos - page_start + size > PAGE_SIZE) {
		bsr_debug_rs("(%s) stream(%d)! unsent(%ld) pos(%ld) size(%d)\n", current->comm, bsr_stream, (long)sbuf->unsent, (long)sbuf->pos, size);
				    ...
				    enum bsr_stream bsr_stream)
{
	...
	if (!transport->ops->stream_ok(transport, bsr_stream)) {
		bsr_err(BSR_LC_TEMP, connection, "socket not allocate\n");
		return NULL;
	}
	...
	if(!p) {
		bsr_err(BSR_LC_TEMP, connection, "failed allocate send buffer\n");
	}
	return p;
#else // _LIN
	...
	if (corked && !flush) {
		...
		if (bsr_stream == DATA_STREAM) {
			...
			// DW-1977 last successful protocol may not be correct because it is a transfer to the buffer
				...
				bsr_info(BSR_LC_TEMP, connection, "last successful protocol %s\n", bsr_packet_name(cmd));
		}
	}
...
int __bsr_send_protocol(struct bsr_connection *connection, enum bsr_packet cmd)
{
	...
	if (test_bit(CONN_DRY_RUN, &connection->flags) && connection->agreed_pro_version < 92) {
		...
		bsr_err(BSR_LC_TEMP, connection, "--dry-run is not supported by peer");
		return -EOPNOTSUPP;
	}
...
void bsr_print_uuids(struct bsr_peer_device *peer_device, const char *text, const char *caller)
{
	...
	if (get_ldev_if_state(device, D_NEGOTIATING)) {
		bsr_info(BSR_LC_TEMP, peer_device, "%s, %s %016llX:%016llX:%016llX:%016llX\n",
			caller, text,
		...
		bsr_info(BSR_LC_TEMP, device, "%s, %s effective data uuid: %016llX\n",
			caller, text, 
			    ...
			    struct p_twopc_request *request)
{
	...
	bsr_debug(BSR_LC_TEMP, connection, "Sending %s request for state change %u\n",
		   bsr_packet_name(cmd),
...
int bsr_send_peer_dagtag(struct bsr_connection *connection, struct bsr_connection *lost_peer)
{
	...
	bsr_info(BSR_LC_TEMP, NO_OBJECT,"bsr_send_peer_dagtag lost_peer:%p lost_peer->last_dagtag_sector:%llx lost_peer->peer_node_id:%d\n",lost_peer,lost_peer->last_dagtag_sector,lost_peer->peer_node_id);
#endif	
				...
				struct bm_xfer_ctx *c)
{
	...
	do {
		...
		if (rl == 0) {
			bsr_warn(BSR_LC_BITMAP, peer_device, "unexpected zero runlength while encoding bitmap "
			...
			if (toggle == 0) {
				...
				bsr_err(BSR_LC_TEMP, peer_device, "unexpected out-of-sync has occurred\n");
				return -1;
			}
		}
		...
		if (bits == -ENOBUFS) /* buffer full */
			break;
		if (bits <= 0) {
			bsr_err(BSR_LC_TEMP, peer_device, "error while encoding bitmap: %d\n", bits);
			return 0;
		}
...
send_bitmap_rle_or_plain(struct bsr_peer_device *peer_device, struct bm_xfer_ctx *c)
{
	...
	if (!tpc) {
		bsr_err(BSR_LC_TEMP, peer_device, "allocate failed\n");
		return -ENOMEM;
	}
	...
	if (len < 0) {
		bsr_err(BSR_LC_TEMP, peer_device, "unexpected len : %d \n", len);
		return -EIO;
	}
	...
	if (len) {
		...
		if (err) {
			bsr_err(BSR_LC_TEMP, peer_device, "error sending P_COMPRESSED_BITMAP, e: %d \n", err);
		}
		...
		if (err) {
			bsr_err(BSR_LC_TEMP, peer_device, "error sending P_BITMAP, e: %d \n", err);
		}		
...
void bsr_send_bitmap_source_complete(struct bsr_device *device, struct bsr_peer_device *peer_device, int err)
{
	...
	if (err) {
		bsr_err(BSR_LC_TEMP, peer_device, "syncsource send bitmap failed err(%d)\n", err);
...
{
	if (err) {
		bsr_err(BSR_LC_TEMP, peer_device, "synctarget send bitmap failed err(%d)\n", err);
		change_cstate_ex(peer_device->connection, C_NETWORK_FAILURE, CS_HARD);
	}
			     ...
			     struct bsr_peer_device *peer_device)
{
	...
	if (!expect(device, device->bitmap)) {
		bsr_err(BSR_LC_TEMP, peer_device, "bitmap is NULL!\n");
		return false;
	}

	if (get_ldev(device)) {
		if (bsr_md_test_peer_flag(peer_device, MDF_PEER_FULL_SYNC)) {
			bsr_info(BSR_LC_TEMP, device, "Writing the whole bitmap, MDF_FullSync was set.\n");
			bsr_bm_set_many_bits(peer_device, 0, BSR_END_OF_BITMAP);
			if (bsr_bm_write(device, NULL)) {
				/* write_bm did fail! Leave full sync flag set in Meta P_DATA
				 * but otherwise process as per normal - need to tell other
				 * side that a full resync is required! */
				bsr_err(BSR_LC_TEMP, device, "Failed to write bitmap to disk!\n");
			} else {
...
int bsr_send_bitmap(struct bsr_device *device, struct bsr_peer_device *peer_device)
{
	...
	if (peer_device->bitmap_index == -1) {
		bsr_err(BSR_LC_TEMP, peer_device, "No bitmap allocated in bsr_send_bitmap()!\n");
		return -EIO;
	}
	...
	if (incomp_sync) {
		if (incomp_sync_source != peer_device &&
			...
			if (bb == NULL) {
				bsr_err(BSR_LC_TEMP, peer_device, "bitmap bit buffer allocate failed\n");
				change_cstate_ex(peer_device->connection, C_NETWORK_FAILURE, CS_HARD);
			}
			else {
				memset(bb, 0, sizeof(ULONG_PTR) * allow_size);

				bsr_info(BSR_LC_TEMP, peer_device, "bitmap merge, from index(%d) out of sync(%llu), to bitmap index(%d) out of sync (%llu)\n",
				...
				bsr_info(BSR_LC_TEMP, peer_device, "finished bitmap merge, to index(%d) out of sync (%llu)\n", peer_device->bitmap_index, (unsigned long long)bsr_bm_total_weight(peer_device));
				kfree2(bb);
		       ...
		       sector_t sector, int size, u64 block_id)
{
...
#ifdef BSR_TRACE
	bsr_debug(BSR_LC_TEMP, NO_OBJECT,"sz=%d sector=%lld\n", size, sector);
#endif
	p = bsr_prepare_command(peer_device, sizeof(*p), DATA_STREAM);
	if (!p)
		return -EIO;
    ...
    bsr_debug_rs("size(%d) cmd(%d) sector(0x%llx) block_id(%llu)\n", size, cmd, (u64)sector, block_id);
	return bsr_send_command(peer_device, cmd, DATA_STREAM);
}
			    ...
			    struct bsr_peer_request *peer_req)
{
	...
	page_chain_for_each(page) {
		...
		if (page_chain_offset(page) != 0 ||
			...
			bsr_err(BSR_LC_TEMP, peer_device, "FIXME page %p offset %u len %u\n",
	...
	if (!err) {
		...
		if (digest_size > 0 && digest_size <= 64) {
			...
			if (memcmp(p + 1, digest, digest_size)) {
				bsr_warn(BSR_LC_REQUEST, device,
	...
	if (resource->res_opts.auto_promote) {
		...
		if (mode & FMODE_WRITE) {
			if (resource->role[NOW] == R_SECONDARY) {
				...
				if (rv < SS_SUCCESS)
					bsr_info(BSR_LC_TEMP, resource, "Auto-promote failed: %s\n",
...
#endif
{
	...
	if (resource->res_opts.auto_promote) {
		...
		if (open_rw_cnt == 0 &&
			...
			if (rv < SS_SUCCESS)
				bsr_warn(BSR_LC_DRIVER, resource, "Auto-demote failed: %s\n",
					  bsr_set_st_err_str(rv));
		}
	}
...
void bsr_destroy_device(struct kref *kref)
{
	...
	bsr_debug(BSR_LC_TEMP, NO_OBJECT,"%s\n", __FUNCTION__);
...
static void do_retry(struct work_struct *ws)
{
	...
	list_for_each_entry_safe_ex(struct bsr_request,  req, tmp, &writes, tl_requests) {
		...
		if (!expected)
			bsr_err(BSR_LC_TEMP, device, "req=%p completion_ref=%d rq_state=%x\n",
...
void bsr_restart_request(struct bsr_request *req)
{
	...
	bsr_info(BSR_LC_TEMP, NO_OBJECT,"req(%p) req->nq_ref (%d)\n", req, atomic_read(&req->nq_ref));
...
{
    bsr_info(BSR_LC_TEMP, NO_OBJECT,"Shutdown: IRQL(%d) device(%ws) Name(%wZ)\n",
...
void bsr_flush_workqueue(struct bsr_resource* resource, struct bsr_work_queue *work_queue)
{
	...
	if (get_t_state(&resource->worker) != RUNNING) {
		bsr_info(BSR_LC_TEMP, NO_OBJECT,"bsr_flush_workqueue &resource->worker != RUNNING return resource:%p\n",resource);
		return;
	}
...
void bsr_destroy_connection(struct kref *kref)
{
	...
	bsr_info(BSR_LC_TEMP, connection, "%s\n", __FUNCTION__);

	if (atomic_read(&connection->current_epoch->epoch_size) !=  0)
		bsr_err(BSR_LC_TEMP, connection, "epoch_size:%d\n", atomic_read(&connection->current_epoch->epoch_size));
	kfree(connection->current_epoch);
	...
	if (atomic_read(&connection->inacitve_ee_cnt)) {
		...
		bsr_info(BSR_LC_TEMP, connection, "inactive_ee count not completed:%d\n", atomic_read(&connection->inacitve_ee_cnt));
				      ...
				      struct device_conf *device_conf, struct bsr_device **p_device)
{
	...
	if (!pvext) {
		err = ERR_NO_DISK;
		bsr_err(BSR_LC_TEMP, device, "%d: Device has no disk.\n", err);
		goto out_no_disk;
	}
#endif
	...
	bsr_info(BSR_LC_TEMP, NO_OBJECT,"device:%p q->max_hw_sectors: %llu sectors, device->this_bdev->d_size: %llu bytes\n", device, q->max_hw_sectors, device->this_bdev->d_size);
#endif
...
void bsr_put_connection(struct bsr_connection *connection)
{
	...
	if (rr)
		bsr_err(BSR_LC_TEMP, connection, "%d EEs in done list found!\n", rr);

	rr = bsr_free_peer_reqs(connection->resource, &connection->net_ee, true);
	if (rr)
		bsr_err(BSR_LC_TEMP, connection, "%d EEs in net list found!\n", rr);
...
NTSTATUS bsr_log_rolling_file_clean_up(WCHAR* filePath)
{
	...
	if (!NT_SUCCESS(status)) {
		bsr_err(BSR_LC_TEMP, NO_OBJECT, "failed to open log directory(%x)\n", status);
		return status;
	}
	...
	if (!pFileBothDirInfo){
		bsr_err(BSR_LC_TEMP, NO_OBJECT, "failed to allocation query buffer (%u)\n", currentSize);
	...
	while (TRUE) {
		...
		if (STATUS_BUFFER_OVERFLOW == status) {
			...
			if (MAX_PATH < (currentSize / 2)) {
				bsr_err(BSR_LC_TEMP, NO_OBJECT, "failed to long path (%u)\n", (currentSize / 2));
		...
		{
			bsr_err(BSR_LC_TEMP, NO_OBJECT, "failed to query (%x)\n", status);
			goto out2;
		}
	...
	if (log_file_max_count >= atomic_read(&g_log_file_max_count)) {
		...
		list_for_each_entry_ex(struct log_rolling_file_list, t, &rlist.list, list) {
			...
			if (!NT_SUCCESS(status)) {
				bsr_err(BSR_LC_TEMP, NO_OBJECT, "failed to open file %ws(%x)\n", fileFullPath, status);
				continue;
			}

			status = ZwSetInformationFile(hFile, &ioStatus, buf, 1, FileDispositionInformation);
			if (!NT_SUCCESS(status)) {
				bsr_err(BSR_LC_TEMP, NO_OBJECT, "failed to FileDispositionInformation %ws(%x)\n", fileFullPath, status);
			}
			ZwClose(hFile);
...
void wait_for_add_device(WCHAR *path) 
{
	...
	while (wait_device_add) {
		...
		MVOL_UNLOCK();
		if (wait_device_add)
			bsr_info(BSR_LC_TEMP, NO_OBJECT, "wait for device to be connected for log file generation.(%ws)\n", path);
...
#endif
{
	...
	if (!NT_SUCCESS(status)) {
		...
		g_consumer_state = EXITING;
		bsr_info(BSR_LC_TEMP, NO_OBJECT, "failed to get bsr path status(%x)\n", status);
		return;
	}
	...
	if (!NT_SUCCESS(status)) {
		bsr_info(BSR_LC_TEMP, NO_OBJECT, "failed to create log file status(%x)\n", status);
	}
#else 
	// BSR-581
	...
	if (hFile == NULL || IS_ERR(hFile)) {
		...
		while (g_consumer_state == RUNNING) {
			...
			if (!NT_SUCCESS(status)) {
				bsr_info(BSR_LC_TEMP, NO_OBJECT, "failed to write log status(%x)\n", status);
				break;
			}
#else
			...
			if (atomic_read(&idx) == (LOGBUF_MAXCNT - 1) || logFileSize > (MAX_BSRLOG_BUF * LOGBUF_MAXCNT)) {
				...
				if (!NT_SUCCESS(status)) {
					bsr_info(BSR_LC_TEMP, NO_OBJECT, "failed to rename log file status(%x)\n", status);
					break;
				}
				status = ZwCreateFile(&hFile,
										FILE_APPEND_DATA,
				...
				if (!NT_SUCCESS(status)) {
					bsr_info(BSR_LC_TEMP, NO_OBJECT, "failed to new log file status(%x)\n", status);
					break;
				}
#else // _LIN
	...
	if (g_consumer_state == RUNNING) {
		...
		bsr_info(BSR_LC_TEMP, NO_OBJECT, "log consumer thread has been terminated.");
...
void bsr_cleanup(void)
{
	...
	bsr_info(BSR_LC_TEMP, NO_OBJECT, "module cleanup done.\n");
}
...
#endif
{
	...
	if (minor_count < BSR_MINOR_COUNT_MIN || minor_count > BSR_MINOR_COUNT_MAX) {
		bsr_err(BSR_LC_TEMP, NO_OBJECT, "invalid minor_count (%u)\n", minor_count);
#ifdef MODULE
		return -EINVAL;
#else
	...
	if (err) {
		bsr_err(BSR_LC_TEMP, NO_OBJECT, "unable to register block device major %d\n", BSR_MAJOR);
		return err;
	}
	...
	if (err) {
		bsr_err(BSR_LC_TEMP, NO_OBJECT, "unable to register bsr-control device\n");
		return err;
	}
	...
	*/
	...
	if (err) {
		bsr_err(BSR_LC_TEMP, NO_OBJECT, "unable to register generic netlink family\n");
		goto fail;
	}
#endif
	...
	if (!bsr_proc)	{
		bsr_err(BSR_LC_TEMP, NO_OBJECT, "unable to register proc file\n");
		goto fail;
	}
#endif
	...
	if (!retry.wq) {
		bsr_err(BSR_LC_TEMP, NO_OBJECT, "unable to create retry workqueue\n");
		goto fail;
	}
	...
	if (!NT_SUCCESS(start_mnt_monitor())) {
		bsr_err(BSR_LC_TEMP, NO_OBJECT,"could not start mount monitor\n");
		goto fail;
	}
#endif
#endif
	...
	bsr_info(BSR_LC_TEMP, NO_OBJECT, "initialized. "
	       "Version: " REL_VERSION " (api:%d/proto:%d-%d)\n",
	       GENL_MAGIC_VERSION, PRO_VERSION_MIN, PRO_VERSION_MAX);
	bsr_info(BSR_LC_TEMP, NO_OBJECT, "%s\n", bsr_buildtag());
	bsr_info(BSR_LC_TEMP, NO_OBJECT, "registered as block device major %d\n", BSR_MAJOR);
...
fail:
	bsr_cleanup();
	if (err == -ENOMEM)
		bsr_err(BSR_LC_TEMP, NO_OBJECT, "ran out of memory\n");
	else
		bsr_err(BSR_LC_TEMP, NO_OBJECT, "initialization failure\n");
#ifdef _LIN
	// BSR-581
	clean_logging();
#endif
...
void bsr_md_write(struct bsr_device *device, void *b)
{
	...
	if (bsr_md_sync_page_io(device, device->ldev, sector, REQ_OP_WRITE)) {
		...
		bsr_err(BSR_LC_TEMP, device, "meta data update failed!\n");
		...
		struct bsr_md *in_core)
{
	...
	return 0;
err:
	bsr_err(BSR_LC_TEMP, device, "invalid activity log striping: al_stripes=%u, al_stripe_size_4k=%u\n",
		...
		struct bsr_backing_dev *bdev)
{
	...
	if (max_peers > BSR_PEERS_MAX) {
		bsr_err(BSR_LC_TEMP, device, "bm_max_peers too high\n");
		goto err;
	}
...
err:
	bsr_err(BSR_LC_TEMP, device, "meta data offsets don't make sense: idx=%d "
...
int bsr_md_read(struct bsr_device *device, struct bsr_backing_dev *bdev)
{
	...
	if (bsr_md_sync_page_io(device, bdev, bdev->md.md_offset,
		...
		bsr_err(BSR_LC_TEMP, device, "Error while reading metadata.\n");
		rv = ERR_IO_MD_DISK;
		goto err;
	}
	...
	if (magic == BSR_MD_MAGIC_09 && !(flags & MDF_AL_CLEAN)) {
		...
		bsr_err(BSR_LC_TEMP, device, "Found unclean meta data. Did you \"bsradm apply-al\"?\n");
		rv = ERR_MD_UNCLEAN;
		goto err;
	}
	rv = ERR_MD_INVALID;
	if (magic != BSR_MD_MAGIC_09) {
		if (magic == BSR_MD_MAGIC_07 ||
			...
			bsr_err(BSR_LC_TEMP, device, "Found old meta data magic. Did you \"bsradm create-md\"?\n");
		else
			bsr_err(BSR_LC_TEMP, device, "Meta data magic not found. Did you \"bsradm create-md\"?\n");
		goto err;
	}

	if (be32_to_cpu(buffer->bm_bytes_per_bit) != BM_BLOCK_SIZE) {
		bsr_err(BSR_LC_TEMP, device, "unexpected bm_bytes_per_bit: %u (expected %u)\n",
	...
	if (bdev->md.node_id != -1 && bdev->md.node_id != my_node_id) {
		bsr_err(BSR_LC_TEMP, device, "ambiguous node id: meta-data: %d, config: %d\n",
	...
	for (i = 0; i < BSR_NODE_ID_MAX; i++) {
		...
		if (peer_md->bitmap_index == -1)
			continue;
		if (i == my_node_id) {
			bsr_warn(BSR_LC_IO, device, "my own node id (%d) should not have a bitmap index (%d)\n",
				my_node_id, peer_md->bitmap_index);
			goto err;
		}

		if (peer_md->bitmap_index < -1 || peer_md->bitmap_index >= (int)max_peers) {
			bsr_warn(BSR_LC_IO, device, "peer node id %d: bitmap index (%d) exceeds allocated bitmap slots (%d)\n",
...
static void __bsr_uuid_new_current(struct bsr_device *device, bool forced, bool send, const char* caller) __must_hold(local)
{
	...
	bsr_info(BSR_LC_TEMP, device, "%s, new current UUID: %016llX weak: %016llX\n", caller,
...
void bsr_uuid_new_current(struct bsr_device *device, bool forced, const char* caller)
{
	if (get_ldev_if_state(device, D_UP_TO_DATE)) {
		...
		bsr_info(BSR_LC_TEMP, device, "%s, sending new current UUID: %016llX\n", caller, current_uuid);
...
static u64 __set_bitmap_slots(struct bsr_device *device, struct bsr_peer_device *peer_device, u64 do_nodes) __must_hold(local)
{
	...
	for (node_id = 0; node_id < BSR_NODE_ID_MAX; node_id++) {
		...
		if (peer_md[node_id].bitmap_uuid != bitmap_uuid) {
			...
			/* bsr_info(BSR_LC_TEMP, device, "bitmap[node_id=%d] = %llX\n", node_id, bitmap_uuid); */
...
void forget_bitmap(struct bsr_device *device, int node_id) __must_hold(local) // DW-955
{
	...
	bsr_info(BSR_LC_TEMP, device, "clearing bitmap UUID and content (%llu bits) for node %d (%s)(slot %d)\n",
...
static void copy_bitmap(struct bsr_device *device, int from_id, int to_id) __must_hold(local)
{
	...
	bsr_info(BSR_LC_TEMP, device, "Node %d (%s) synced up to node %d (%s). copying bitmap slot %d to %d.\n",
...
found:
	...
	if (from_id == -1) {
		if (peer_md[node_id1].bitmap_uuid == 0 && peer_md[node_id2].bitmap_uuid == 0)
			return false;
		bsr_err(BSR_LC_TEMP, peer_device, "unexpected\n");
		bsr_err(BSR_LC_TEMP, peer_device, "In UUIDs from node %d found equal UUID (%llX) for nodes %d %d\n",
			 peer_device->node_id, peer_bm_uuid, node_id1, node_id2);
		bsr_err(BSR_LC_TEMP, peer_device, "I have %llX for node_id=%d\n",
			...
			if (peer_current_uuid == (bsr_current_uuid(device) & ~UUID_PRIMARY)) {
				...
				if (node_id == peer_device->node_id)
					...
					if (found_peer &&
						...
						bsr_info(BSR_LC_TEMP, device, "bitmap will be cleared due to other resync, pdisk(%d), prepl(%d), peerdirty(%llu), pdvflag(%llx)\n",
					...
					bsr_info(BSR_LC_TEMP, device, "Clearing bitmap UUID for node %d\n",
						  node_id);
				bsr_md_mark_dirty(device);
// DW-979
// DW-980
clear_flag:
			...
			if (from_node_id != -1 && node_id != from_node_id &&
				...
				if (peer_md[node_id].bitmap_index != -1 &&
					...
					bsr_info(BSR_LC_TEMP, device, "Node %d synced up to node %d.\n",
		...
		(bsr_current_uuid(device) & ~UUID_PRIMARY))
	{
		...
		if (peer_md[peer_node_id].bitmap_index != -1
				&& !bsr_md_test_peer_flag(peer_device, MDF_PEER_PRIMARY_IO_ERROR)) {
			bsr_info(BSR_LC_TEMP, peer_device, "bitmap will be cleared due to inconsistent out-of-sync, disk(%d)\n", device->disk_state[NOW]);
		...
		(bsr_current_uuid(device) & ~UUID_PRIMARY))
	{
		...
		if (peer_md[peer_node_id].bitmap_index != -1 
				&& !bsr_md_test_peer_flag(peer_device, MDF_PEER_PRIMARY_IO_ERROR)) {
			bsr_info(BSR_LC_TEMP, peer_device, "bitmap will be cleared because peer has consistent disk with primary's\n");
			forget_bitmap(device, peer_node_id);
		}
		bsr_md_mark_dirty(device);
...
int bsr_bmio_set_all_or_fast(struct bsr_device *device, struct bsr_peer_device *peer_device) __must_hold(local)
{
	...
	if (peer_device->repl_state[NOW] == L_STARTING_SYNC_S) {
		...
		{
			bsr_warn(BSR_LC_RESYNC_OV, peer_device, "can not perform fast invalidate(remote), protocol ver(%d), fastSyncOpt(%d)\n", peer_device->connection->agreed_pro_version, isFastInitialSync());
			if (dec_bm_work_n) {
		...
		bsr_warn(BSR_LC_RESYNC_OV, peer_device, "unexpected repl state: %s\n", bsr_repl_str(peer_device->repl_state[NOW]));
	}
			      ...
			      struct bsr_peer_device *peer_device) __must_hold(local)
{
	...
	for_each_peer_device_rcu(p, device) {
		if (!update_sync_bits(p, 0, bsr_bm_bits(device), SET_OUT_OF_SYNC, true)) {
			bsr_err(BSR_LC_TEMP, device, "no sync bit has been set for peer(%d), set whole bits without updating resync extent instead.\n", p->node_id);
			  ...
			  struct bsr_peer_device *peer_device) __must_hold(local)
{
	...
	if (!update_sync_bits(peer_device, 0, bsr_bm_bits(device), SET_OUT_OF_SYNC, false)) {
		bsr_err(BSR_LC_TEMP, peer_device, "no sync bit has been set, set whole bits without updating resync extent instead.\n");
		bsr_bm_set_many_bits(peer_device, 0, BSR_END_OF_BITMAP);
	}
...
ULONG_PTR SetOOSFromBitmap(PVOLUME_BITMAP_BUFFER pBitmap, struct bsr_peer_device *peer_device)
{
	...
	if (NULL == pBitmap ||
		...
		NULL == peer_device)
	{
		bsr_err(BSR_LC_TEMP, peer_device, "Invalid parameter, pBitmap(0x%p), pBitmap->Buffer(0x%p) peer_device(0x%p)\n", pBitmap, pBitmap ? pBitmap->Buffer : NULL, peer_device);
#ifdef _WIN
		return UINT64_MAX;
#else	// _LIN
		return -1;
...
#endif
#endif
	bsr_info(BSR_LC_TEMP, NO_OBJECT, "Fast sync %s\n", bRet ? "enabled" : "disabled");
...
bool ConvertVolumeBitmap(PVOLUME_BITMAP_BUFFER pVbb, PCHAR pConverted, ULONG bytesPerCluster, ULONG ulBsrBitmapUnit)
{
	...
	if (NULL == pVbb ||
		...
		NULL == pConverted)
	{
		bsr_err(BSR_LC_TEMP, NO_OBJECT,"Invalid parameter, pVbb(0x%p), pVbb->Buffer(0x%p), pConverted(0x%p)\n", pVbb, pVbb ? pVbb->Buffer : NULL, pConverted);
		return false;
	}
...
PVOLUME_BITMAP_BUFFER GetVolumeBitmapForBsr(struct bsr_device *device, ULONG ulBsrBitmapUnit)
{
	...
	do {
		...
		if (NULL == pVbb) {
			bsr_err(BSR_LC_TEMP, device, "Could not get volume bitmap, minor(%u)\n", device->minor);
			break;
		}
		...
		if (ulBytesPerCluster == ulBsrBitmapUnit) {
...
#endif
			if (NULL == pBsrBitmap) {
				bsr_err(BSR_LC_TEMP, device, "pConvertedBitmap allocation failed\n");
				break;
			}
			...
			if (!ConvertVolumeBitmap(pVbb, (char *)pBsrBitmap->Buffer, ulBytesPerCluster, ulBsrBitmapUnit)) {
				bsr_err(BSR_LC_TEMP, device, "Could not convert bitmap, ulBytesPerCluster(%u), ulBsrBitmapUnit(%u)\n", ulBytesPerCluster, ulBsrBitmapUnit);
				kfree(pBsrBitmap);
				pBsrBitmap = NULL;
				break;
			}
...
bool SetOOSAllocatedCluster(struct bsr_device *device, struct bsr_peer_device *peer_device, enum bsr_repl_state side, bool bitmap_lock)
{
	...
	if (NULL == device ||
		...
		if (peer_device)
			bsr_err(BSR_LC_TEMP, peer_device,"Invalid parameter side(%s)\n", bsr_repl_str(side));
		else
			bsr_err(BSR_LC_TEMP, NO_OBJECT, "Invalid parameter side(%s)\n", bsr_repl_str(side));
	...
	if (!bsr_inspect_resync_side(peer_device, side, NOW, false)) {
		bsr_warn(BSR_LC_RESYNC_OV, peer_device, "can't be %s\n", bsr_repl_str(side));
		if (!bitmap_lock)
			mutex_unlock(&device->resource->vol_ctl_mutex);
		goto out;
	}
	...
	if (device->resource->role[NOW] == R_SECONDARY) {
		...
		if (side == L_SYNC_SOURCE) {
			bsr_info(BSR_LC_TEMP, peer_device,"I am a secondary sync source, will mount volume for temporary to get allocated clusters.\n");
			bSecondary = true;
		}
		else if (side == L_SYNC_TARGET) {
			bsr_info(BSR_LC_TEMP, peer_device,"I am a sync target, wait to receive source's bitmap\n");
			bRet = true;
			mutex_unlock(&device->resource->vol_ctl_mutex);
			goto out;
		}
	...
	bsr_info(BSR_LC_TEMP, peer_device, "Writing the bitmap for allocated clusters.\n");
	...
	do {
		if (bSecondary) {
			...
			if (!ChangeVolumeReadonly(device->minor, true)) {
				bsr_err(BSR_LC_TEMP, peer_device, "Could not change volume read-only attribute\n");
		...
		if (NULL == pBitmap) {
			bsr_err(BSR_LC_TEMP, peer_device, "Could not get bitmap for bsr\n");
		}
		...
		if (bSecondary) {
			...
			if (!ChangeVolumeReadonly(device->minor, false)) {
				bsr_err(BSR_LC_TEMP, peer_device, "Read-only attribute for volume(minor: %d) had been set, but can't be reverted. force detach bsr disk\n", device->minor);
				if (device &&
	...
	bsr_info(BSR_LC_TEMP, peer_device, "%llu bits(%llu KB) have been set as out-of-sync\n", 
	...
	if (count == -1) {
		bsr_err(BSR_LC_TEMP, peer_device, "Could not set bits from gotten bitmap\n");
		bRet = false;
	}
	else{
		bsr_info(BSR_LC_TEMP, peer_device, "%llu bits(%llu KB) are set as new out-of-sync\n", 
...
int w_fast_ov_get_bm(struct bsr_work *w, int cancel) {
	...
	if (atomic_read(&device->pending_bitmap_work.n)) {
		bsr_info(BSR_LC_TEMP, peer_device, "fast_ov canceled due to pending bitmap work.\n");
		if (side == L_VERIFY_S) {
	...
	if (device->resource->role[NOW] == R_SECONDARY) {
		// DW-1317 set read-only attribute and mount for temporary.
		if (side == L_VERIFY_S) {
			bsr_info(BSR_LC_TEMP, peer_device,"I am a secondary verify source, will mount volume for temporary to get allocated clusters.\n");
			bSecondary = true;
		}
		else {
			bsr_warn(BSR_LC_RESYNC_OV, peer_device, "unexpected repl state: %s\n", bsr_repl_str(peer_device->repl_state[NOW]));
			  ...
			  struct bsr_peer_device *peer_device)
{
	...
	if (current == device->resource->worker.task)
		bsr_info(BSR_LC_TEMP, device, "%s, worker.task(%p), current(%p)\n", why ? why : "?", device->resource->worker.task, current);

	bm_io_work = kmalloc(sizeof(*bm_io_work), GFP_NOIO, '21DW');
	if (!bm_io_work) {
		bsr_err(BSR_LC_TEMP, device, "Could not allocate bm io work.\n");
...
{
	if (!device->ldev) {
		bsr_warn(BSR_LC_STATE, device, "ldev is null.\n");
		return;
	}
			   ...
			   enum mdf_peer_flag flag) __must_hold(local)
{
	...
	struct bsr_device *device = peer_device->device;
	if (!device->ldev) {
		bsr_warn(BSR_LC_STATE, peer_device, "ldev is null.\n");
		return;
	}
...
void unlock_all_resources(void)
{
	...
	local_irq_enable();
#ifdef _WIN
	bsr_debug_req_lock("local_irq_enable : CurrentIrql(%d)\n", KeGetCurrentIrql());
#endif
...
_bsr_insert_fault(struct bsr_device *device, unsigned int type)
{
	...
	if (ret) {
		...
		if (bsr_ratelimit())
			bsr_warn(BSR_LC_IO, device, "***Simulating %s failure\n",
				_bsr_fault_str(type));
	}
