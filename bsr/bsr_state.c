					      ...
					      enum bsr_state_rv rv, bool locked, const char* caller)
{
	...
	if (rv < SS_SUCCESS) {
		if (flags & CS_VERBOSE) {
			bsr_err(0, BSR_LC_TEMP, resource, "State change failed: %s\n", bsr_set_st_err_str(rv));
...
#ifdef _WIN
		bsr_warn(0, BSR_LC_STATE, NO_OBJECT,"worker should not initiate state changes with CS_SERIALIZE current:%p resource->worker.task:%p\n", current , resource->worker.task);
#else // _LIN
	...
	if (val) {
		for_each_peer_device(p, device) {
			...
			if (p->resync_susp_other_c[NOW] != p->resync_susp_other_c[NEW])
				bsr_info(0, BSR_LC_TEMP, peer_device, "%s => node_id(%d), resync_susp_other_c : true\n", caller, p->node_id);
		...
		for_each_peer_device(p, device) {
			...
			if (p->resync_susp_other_c[NOW] != p->resync_susp_other_c[NEW])
				bsr_info(0, BSR_LC_TEMP, peer_device, "%s => node_id(%d), resync_susp_other_c : false\n", caller, p->node_id);
...
static void print_state_change(struct bsr_resource *resource, const char *prefix, bool locked, const char *caller)
{
	...
	if (b != buffer) {
		*(b-1) = 0;
		bsr_info(0, BSR_LC_TEMP, resource, "%s, %s%s\n", caller, prefix, buffer);
	}
	...
	for_each_connection(connection, resource) {
		...
		if (b != buffer) {
			*(b-1) = 0;
			bsr_info(0, BSR_LC_TEMP, connection, "%s, %s%s\n", caller, prefix, buffer);
		}
	}
	...
	idr_for_each_entry_ex(struct bsr_device *, &resource->devices, device, vnr) {
		...
		if (disk_state[OLD] != disk_state[NEW])
			bsr_info(0, BSR_LC_TEMP, device, "%s, %sdisk( %s -> %s )\n",
				  caller,
				  prefix,
		...
		for_each_peer_device(peer_device, device) {
			...
			if (b != buffer) {
				*(b-1) = 0;
				bsr_info(0, BSR_LC_TEMP, peer_device, "%s, %s%s\n", caller, prefix, buffer);
			}
		}
	}
}
...
#endif
{
	...
	if (context->flags & CS_VERBOSE)
		bsr_err(0, BSR_LC_TEMP, resource, "%s\n", err_str);
	if (context->err_str)
	...
	for_each_connection(connection, resource) {
		...
		idr_for_each_entry_ex(struct bsr_peer_device *, &connection->peer_devices, peer_device, vnr) {
			    ...
			    peer_device->repl_state[NEW] >= L_ESTABLISHED)
			{
				...
				bsr_info(0, BSR_LC_TEMP, connection, "return SS_NEED_CONNECTION!!! cs=%d repl=%d \n",
		...
		if (disk_state[OLD] == D_NEGOTIATING && disk_state[NEW] == D_NEGOTIATING) {
			...
			for_each_peer_device(peer_device, device) {
				...
				if (nr == L_NEG_NO_RESULT)
					...
					bsr_err(0, BSR_LC_TEMP, peer_device, "Unexpected nr = %s\n", bsr_repl_str(nr));
			}
			...
			for_each_peer_device(peer_device, device) {
				...
				if (nr == L_WF_BITMAP_S && disk_state[NEW] == D_INCONSISTENT) {
					...
					nr = L_PAUSED_SYNC_S;
					__change_resync_susp_other_c(peer_device, true, NULL);
					bsr_warn(0, BSR_LC_STATE, peer_device, "Finish me\n");
				}
		...
		for_each_peer_device(peer_device, device) {
			...
			if ((repl_state[NEW] >= L_STARTING_SYNC_S && repl_state[NEW] <= L_PAUSED_SYNC_T)) {
				if (((repl_state[NEW] != L_STARTING_SYNC_S && repl_state[NEW] != L_STARTING_SYNC_T) ||
					...
					bsr_warn(0, BSR_LC_STATE, peer_device, "force it to be L_ESTABLISHED due to unsyncable stability\n");
					__change_repl_state(peer_device, L_ESTABLISHED, __FUNCTION__);
					  ...
					  struct completion *done)
{
	...
	if (work && work->state_change) {
		...
		kfree(work);
		bsr_err(0, BSR_LC_TEMP, resource, "Could not allocate after state change work\n");
		if (done)
			complete(done);
	}
}
...
static void finish_state_change(struct bsr_resource *resource, struct completion *done, bool locked, const char* caller)
{
	...
	idr_for_each_entry_ex(struct bsr_device *, &resource->devices, device, vnr) {
		...
		if (disk_state[OLD] == D_ATTACHING && disk_state[NEW] >= D_NEGOTIATING)
			bsr_info(0, BSR_LC_TEMP, device, "attached to current UUID: %016llX\n", device->ldev->md.current_uuid);
		...
		for (n_connection = 0; n_connection < state_change->n_connections; n_connection++) {
			...
			if (connection->cstate[NOW] == C_CONNECTED && !test_bit(INITIAL_STATE_SENT, &peer_device->flags)) {
				...
				if (!res) {
					...
					bsr_err(0, BSR_LC_TEMP, peer_device, "state initial send timeout!\n");
				}
			}
 ...
 * RESTART_FROZEN_DISK_IO may need a (temporary?) dedicated kernel thread */
			...
			if (repl_state[NEW] == L_WF_BITMAP_T &&
				...
				// DW-2026 if the status is not L_STARTING_SYNC_T, the status is not send.
					...
					bsr_info(0, BSR_LC_TEMP, peer_device, "not sending state because of old repl_state(%s)\n", bsr_repl_str(repl_state[OLD]));
			}
		...
		if (effective_disk_size_determined && get_ldev(device)) {
			sector_t size = bsr_get_capacity(device->this_bdev);
	...
	if (resource->remote_state_change) {
		...
		for(;;) {
			...
			if (when_done_lock(resource, irq_flags)) {
				bsr_info(0, BSR_LC_TEMP, resource, "Two-phase commit: "
						...
						struct change_context *context, const char* caller)
{
	...
	bsr_info(0, BSR_LC_TEMP, resource, "Preparing cluster-wide state change %u (%u->%d %u/%u)\n",
	...
	if (have_peers) {
        ...
        if (t) {
			...
			bsr_info(0, BSR_LC_TEMP, resource, "[TWOPC:%u] target_node_id(%d) get_cluster_wide_reply (%d) \n", 
						reply->tid,
						context->target_node_id, 
						rv);
		}
		...
		if (rv == SS_CW_SUCCESS) {
			...
			bsr_info(0, BSR_LC_TEMP, resource, "State change %u: primary_nodes=%lX, weak_nodes=%lX\n",
			...
			if (!(context->mask.conn == conn_MASK && context->val.conn == C_DISCONNECTING) ||
				...
				for_each_connection_rcu(connection, resource) {
					...
					if (node_id == context->target_node_id) {
						bsr_info(0, BSR_LC_TEMP, connection, "Cluster is now split\n");
						break;
					}
				}
				rcu_read_unlock();
	...
	{
		bsr_warn(0, BSR_LC_TWOPC, resource, "twopc timeout, no more retry\n");
	...
	if (rv >= SS_SUCCESS)
		bsr_info(0, BSR_LC_TEMP, resource, "Committing cluster-wide state change %u (%ums) (%u->%d)\n",
			  be32_to_cpu(request.tid),
		...
		bsr_info(0, BSR_LC_TEMP, resource, "Aborting cluster-wide state change %u (%ums) rv = %d (%u->%d)\n",
			  be32_to_cpu(request.tid),
			  jiffies_to_msecs(jiffies - start_time),
			  rv,
			  resource->res_opts.node_id,
...
retry:
	...
	bsr_info(0, BSR_LC_TEMP, resource, "Preparing cluster-wide state change %u "
	...
	if (have_peers) {
		...
		if (rv == SS_TIMEOUT || rv == SS_CONCURRENT_ST_CHG) {
			...
			bsr_info(0, BSR_LC_TEMP, resource, "Retrying cluster-wide state change after %ums\n",
	...
	if (rv >= SS_SUCCESS) {
		...
		commit_it = new_size != bsr_get_capacity(device->this_bdev);

		if (commit_it) {
			request.exposed_size = cpu_to_be64(new_size);
			request.diskful_primary_nodes = cpu_to_be64(reply->diskful_primary_nodes);
			bsr_info(0, BSR_LC_TEMP, resource, "Committing cluster-wide state change %u (%ums)\n",
				be32_to_cpu(request.tid),
				jiffies_to_msecs(jiffies - start_time));
		}
		else {
			bsr_info(0, BSR_LC_TEMP, resource, "Aborting cluster-wide state change %u (%ums) size unchanged\n",
				be32_to_cpu(request.tid),
				jiffies_to_msecs(jiffies - start_time));
		}
	}
		...
		commit_it = false;
		bsr_info(0, BSR_LC_TEMP, resource, "Aborting cluster-wide state change %u (%ums) rv = %d\n",
			be32_to_cpu(request.tid),
			jiffies_to_msecs(jiffies - start_time),
			rv);
	}
...
void twopc_end_nested(struct bsr_resource *resource, enum bsr_packet cmd, bool as_work)
{
	...
	if (!twopc_reply.tid){
		bsr_info(0, BSR_LC_TEMP, resource, "!twopc_reply.tid = %u result: %s\n", twopc_reply.tid, bsr_packet_name(cmd));
		// DW-1414
		spin_unlock_irq(&resource->req_lock);
	...
	list_for_each_entry_safe_ex(struct bsr_connection, twopc_parent, tmp, &parents, twopc_parent_list) {
		if (&twopc_parent->twopc_parent_list == twopc_parent->twopc_parent_list.next) {
			bsr_err(0, BSR_LC_TEMP, resource, "twopc_parent_list is invalid\n");
			// DW-1480
	...
	if (connections == NULL) {
		spin_unlock_irq(&resource->req_lock);
		bsr_err(0, BSR_LC_TEMP, resource, "failed to allocate memory for connections\n");
		return;
	}
