			...
			if (peer_device->connection->agreed_pro_version >= 113) {
				...
				if ((repl_state[OLD] == L_SYNC_TARGET || repl_state[OLD] == L_PAUSED_SYNC_T) && repl_state[OLD] != repl_state[NEW]) {
					...
					if (peer_device->sent_rs_req_size) {
						BSR_VERIFY_DATA("send request to complete bitmap exchange since status has changed, sector(%llu) size(%u), bitmap(%llu ~ %llu)\n",
						...
						_bsr_send_ack(peer_device, P_RS_WRITE_ACK, cpu_to_be64(peer_device->sent_rs_req_sector), cpu_to_be32(peer_device->sent_rs_req_size), ID_SYNCER_SPLIT_DONE);
