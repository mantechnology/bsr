...
static int bsr_adm_finish(struct bsr_config_context *adm_ctx, struct genl_info *info, int retcode)
{
	if (retcode < SS_SUCCESS) {
		...
		bsr_err(2, BSR_LC_GENL, resource, "cmd(%u) error: %s", info->genlhdr->cmd, bsr_set_st_err_str(retcode));
	}
...
bool conn_try_outdate_peer(struct bsr_connection *connection)
{
	...
	if (connection->cstate[NOW] >= C_CONNECTED) {
		bsr_err(4, BSR_LC_GENL, connection, "Not connected. cstate(%s)", bsr_conn_str(connection->cstate[NOW]));
		...
		enum dds_flags flags, struct resize_parms *rs) __must_hold(local)
{
	...
	if (bsr_get_capacity(device->this_bdev) != size ||
		...
		if (effective_disk_size_determined(device)) {
			...
			bsr_info(13, BSR_LC_GENL, device, "Update disk size %s (%llu KB)", ppsize(ppb, sizeof(ppb), size >> 1),
...
static bool get_max_agreeable_size(struct bsr_device *device, uint64_t *max, uint64_t min) __must_hold(local)
{
	...
	for (node_id = 0; node_id < BSR_NODE_ID_MAX; node_id++) {
		...
		if (device->ldev->md.node_id == node_id) {
			bsr_info(16, BSR_LC_GENL, device, "local node_id: %u", node_id);
...
static int bsr_check_al_size(struct bsr_device *device, struct disk_conf *dc)
{
	...
	if (t) {
		for (i = 0; i < t->nr_elements; i++) {
			...
			if (e->refcnt)
				bsr_err(22, BSR_LC_GENL, device, "Reference count has non-zero element(%u), reference count(%u)", e->lc_number, e->refcnt);
			in_use += e->refcnt;
		}
	}
	if (!in_use)
...
static int adm_detach(struct bsr_device *device, int force, struct sk_buff *reply_skb)
{
	...
	// TODO: When passing the result of timeout to ret, it should be verified that there is no problem.
	 wait_event_interruptible_timeout_ex(device->misc_wait,
						 ...
						 timeo, timeo);
	bsr_info(42, BSR_LC_GENL, NO_OBJECT,"wait event interruptible timeout. time out(%ld) disk state(%s)", timeo, bsr_disk_str(device->disk_state[NOW]));
	if (retcode >= SS_SUCCESS) {
		int res;
...
#endif
		if (!new_plan) {
			bsr_err(43, BSR_LC_GENL, peer_device, "Failed to allocate %d size memory in kmalloc", fifo_size);
			return -ENOMEM;
		}
...
adm_add_path(struct bsr_config_context *adm_ctx,  struct genl_info *info)
{
	...
	if (err) {
		...
		bsr_err(47, BSR_LC_GENL, connection, "Failed to get an listener. err(%d)", err);
...
adm_del_path(struct bsr_config_context *adm_ctx,  struct genl_info *info)
{
	...
	bsr_err(48, BSR_LC_GENL, connection, "Failed to put an listener. err(%d)", err);
	bsr_msg_put_info(adm_ctx->reply_skb,
...
void del_connection(struct bsr_connection *connection)
{
	...
	if (rv2 < SS_SUCCESS)
		bsr_err(49, BSR_LC_GENL, connection, "Failed to change status to STANDALONE. state(%d)", rv2);
...
int bsr_adm_suspend_io(struct sk_buff *skb, struct genl_info *info)
{
	...
	bsr_err(52, BSR_LC_GENL, resource, "command suspend I/O(%d) is not supported.", info->genlhdr->cmd);
...
int bsr_adm_resume_io(struct sk_buff *skb, struct genl_info *info)
{
	...
	resource = adm_ctx.device->resource;
	bsr_err(53, BSR_LC_GENL, resource, "command resume I/O(%d) is not supported.", info->genlhdr->cmd);;
	bsr_adm_finish(&adm_ctx, info, -ENOMSG);
	return -ENOMSG;
#else // _LIN
...
int bsr_adm_new_c_uuid(struct sk_buff *skb, struct genl_info *info)
{
	...
	if (args.clear_bm) {
		...
		if (err) {
			bsr_err(55, BSR_LC_GENL, device, "Writing bitmap failed with %d",err);
			retcode = ERR_IO_MD_DISK;
		}
		for_each_peer_device(peer_device, device) {
...
#endif
		bsr_err(56, BSR_LC_GENL, NO_OBJECT, "invalid node id (%d)", res_opts.node_id);
		retcode = ERR_INVALID_REQUEST;
		goto out;
	}
#ifdef _LIN
...
int bsr_adm_down(struct sk_buff *skb, struct genl_info *info)
{
	...
	for_each_connection_ref(connection, im, resource) {
		...
		if (retcode >= SS_SUCCESS) {
			...
			bsr_info(58, BSR_LC_GENL, connection, "conn_try_disconnect retcode : %d", retcode);
...
nla_put_failure:
	nlmsg_free(skb);
	bsr_err(65, BSR_LC_GENL, NO_OBJECT, "Error %d sending event. Event seq:%u", err, seq);
}
