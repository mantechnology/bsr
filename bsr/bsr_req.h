...
#ifdef DRBD_TRACE
	drbd_debug(NO_DEVICE,"(%s) _req_mod: call __req_mod! IRQL(%d) \n", current->comm, KeGetCurrentIrql());
#endif
...
#ifdef DRBD_TRACE	
	drbd_debug(NO_DEVICE,"(%s) req_mod: before __req_mod! IRQL(%d) \n", current->comm, KeGetCurrentIrql());
#endif
	rv = __req_mod(req, what, peer_device, &m);
	spin_unlock_irq(&device->resource->req_lock);
#ifdef DRBD_TRACE	
	drbd_debug(NO_DEVICE,"(%s) req_mod: after __req_mod! IRQL(%d) \n", current->comm, KeGetCurrentIrql());
#endif
	if (m.bio)
#ifdef _WIN32
		complete_master_bio(device, &m, __FUNCTION__, __LINE__); 
