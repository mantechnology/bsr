...
#ifdef BSR_TRACE
	bsr_debug(34, NO_REQUEST,"(%s) _req_mod: call __req_mod! IRQL(%d) \n", current->comm, KeGetCurrentIrql());
#endif
...
#ifdef BSR_TRACE	
	bsr_debug(35, BSR_LC_REQUEST, NO_OBJECT,"(%s) req_mod: before __req_mod! IRQL(%d) \n", current->comm, KeGetCurrentIrql());
#endif
	rv = __req_mod(req, what, peer_device, &m);
	spin_unlock_irq(&device->resource->req_lock);
#ifdef BSR_TRACE	
	bsr_debug(36, BSR_LC_REQUEST, NO_OBJECT,"(%s) req_mod: after __req_mod! IRQL(%d) \n", current->comm, KeGetCurrentIrql());
#endif
	if (m.bio)
		complete_master_bio(device, &m);
