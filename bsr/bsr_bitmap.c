...
static void __bm_print_lock_info(struct bsr_device *device, const char *func)
{
	...
	bsr_err(0, BSR_LC_TEMP, device, "FIXME %s[%d] in %s, bitmap locked for '%s' by %s[%d]\n",
		current->comm,
		task_pid_nr(current),
		func,
		b->bm_why ? b->bm_why : "?",
	      ...
	      char *why, enum bm_flag flags)
{
	...
	if (!b) {
		bsr_err(0, BSR_LC_TEMP, device, "FIXME no bitmap in bsr_bm_lock!?\n");
		return;
	}
	...
	if (trylock_failed) {		
		...
		bsr_warn(0, BSR_LC_BITMAP, device, "%s[%d] going to '%s' but bitmap already locked for '%s' by %s[%d]\n",
			current->comm, 
			task_pid_nr(current),
			why, 
			b->bm_why ? b->bm_why : "?",
	...
	if (b->bm_flags & BM_LOCK_ALL)
		bsr_err(0, BSR_LC_TEMP, device, "FIXME bitmap already locked in bm_lock\n");
	// DW-1979
...
void bsr_bm_unlock(struct bsr_device *device)
{
	struct bsr_bitmap *b = device->bitmap;
	if (!b) {
		bsr_err(0, BSR_LC_TEMP, device, "FIXME no bitmap in bsr_bm_unlock!?\n");
		return;
	}

	if (!(device->bitmap->bm_flags & BM_LOCK_ALL))
		bsr_err(0, BSR_LC_TEMP, device, "FIXME bitmap not locked in bm_unlock\n");
...
static void bm_free_pages(struct page **pages, ULONG_PTR number)
{
	...
	for (i = 0; i < number; i++) {
		if (!pages[i]) {
			bsr_alert(0, BSR_LC_BITMAP, NO_OBJECT, "bm_free_pages tried to free a NULL pointer; i=%lu n=%lu\n",
				 i, number);
			continue;
		}
		__free_page(pages[i]);
	...
	enum bitmap_operations op, __le32 *buffer)
{
	...
	{
#ifdef _DEBUG_OOS
		bsr_err(0, BSR_LC_TEMP, device, "unexpected error, could not get bitmap, start(%llu)\n", (unsigned long long)start);
#endif
		return 1;
	}
...
#ifdef _DEBUG_OOS
		// DW-1153 add error log
		bsr_err(0, BSR_LC_TEMP, device, "unexpected error, could not get bitmap->bm_pages, start(%llu)\n", (unsigned long long)start);
#endif
		return 0;
	}
...
#ifdef _DEBUG_OOS
		// DW-1153 add error log
		bsr_err(0, BSR_LC_TEMP, device, "unexpected error, bitmap->bm_bits is 0, start(%llu)\n", (unsigned long long)start);
#endif
		return 0;
	}
...
#define ___bm_op(device, bitmap_index, start, end, op, buffer, km_type) \
	({ unsigned long ret; \
	   bsr_info(0, BSR_LC_TEMP, device, "%s: ___bm_op(..., %u, %lu, %lu, %u, %p)\n", \
		     __func__, bitmap_index, start, end, op, buffer); \
...
int bsr_bm_resize(struct bsr_device *device, sector_t capacity, int set_new_bits)
{
	...
	bsr_info(0, BSR_LC_TEMP, device, "bsr_bm_resize called with capacity == %llu\n",
	...
	if (get_ldev(device)) {
		...
		if (bits > bits_on_disk) {
			bsr_err(0, BSR_LC_TEMP, device, "Not enough space for bitmap: %llu > %llu\n",
	...
	bsr_info(0, BSR_LC_TEMP, device, "resync bitmap: bits=%llu words=%llu pages=%llu\n", (unsigned long long)bits, (unsigned long long)words, (unsigned long long)want);
...
void check_and_clear_io_error_in_primary(struct bsr_device *device)
{
	...
	if (total_count == 0 && !all_disconnected) {
		...
		bsr_info(0, BSR_LC_TEMP, device, "io-error has been cleared.\n");
...
void check_and_clear_io_error_in_secondary(struct bsr_peer_device *peer_device)
{
	...
	if (count == 0) {
		bsr_md_clear_peer_flag(peer_device, MDF_PEER_PRIMARY_IO_ERROR);
		bsr_md_clear_flag(device, MDF_IO_ERROR);
		bsr_info(0, BSR_LC_TEMP, peer_device, "io-error has been cleared.\n");
		atomic_set(&device->io_error_count, 0);
		bsr_queue_notify_io_error_cleared(device);
	}
...
#endif
{
	...
	if ((ULONG_PTR)DeviceObject != FAULT_TEST_FLAG) {
		...
		if(gSimulDiskIoError.ErrorFlag && gSimulDiskIoError.ErrorType == SIMUL_DISK_IO_ERROR_TYPE4) {
			if(IsDiskError()) {
				bsr_err(0, BSR_LC_TEMP, NO_OBJECT,"SimulDiskIoError: Bitmap I/O Error type4.....ErrorFlag:%lu ErrorCount:%lu\n",gSimulDiskIoError.ErrorFlag, gSimulDiskIoError.ErrorCount);
	...
	if ((ctx->flags & BM_AIO_COPY_PAGES) == 0 &&
		...
		bsr_warn(0, BSR_LC_BITMAP, device, "bitmap page idx %llu changed during IO!\n", (unsigned long long)idx);
	...
	if (error) {
		...
		if (bsr_ratelimit())
			bsr_err(0, BSR_LC_TEMP, device, "IO ERROR %d on bitmap page idx %llu\n",
					error, (unsigned long long)idx);
	} else {
		bm_clear_page_io_err(b->bm_pages[idx]);
		dynamic_bsr_dbg(device, "bitmap page idx %llu completed\n", (unsigned long long)idx);
	}
...
static int bm_page_io_async(struct bsr_bm_aio_ctx *ctx, int page_nr) __must_hold(local)
{
	...
	if (len == 0){
		bsr_warn(0, BSR_LC_BITMAP, device, "If len is 0, change it to PAGE_SIZE.\n"); 
		len = PAGE_SIZE; 
	}
...
no_memory :
	bsr_err(0, BSR_LC_TEMP, NO_OBJECT,"Unexpected logic: No memory!\n");
	return -ENOMEM;
}
	...
	unsigned flags) __must_hold(local)
{
	...
	*ctx = (struct bsr_bm_aio_ctx) {
	...
	if (flags & BM_AIO_READ) {
		...
		for (i = start_page; i <= end_page; i++) {
			...
			if (!(flags & BM_AIO_WRITE_ALL_PAGES) &&
				...
				//dynamic_bsr_dbg(device, "skipped bm write for idx %u\n", i);
				continue;
			}
			...
			if ((flags & BM_AIO_WRITE_LAZY) &&
				...
				//dynamic_bsr_dbg(device, "skipped bm lazy write for idx %u\n", i);
				continue;
			}
			atomic_inc(&ctx->in_flight);
			// DW-938
	...
	if (flags == 0 && count) {
		...
		if (ms > 5) {
			bsr_info(0, BSR_LC_TEMP, device, "bitmap %s of %llu pages took %u ms\n",
	...
	if (ctx->error) {
		bsr_alert(0, BSR_LC_BITMAP, device, "we had at least one MD IO ERROR during bitmap IO\n");
	...
	if (flags & BM_AIO_READ) {
		now = jiffies;
		bm_count_bits(device);
		bsr_info(0, BSR_LC_TEMP, device, "recounting of set bits took additional %ums\n",
