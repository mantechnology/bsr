	...
	enum bitmap_operations op, __le32 *buffer)
{
	...
	{
#ifdef _DEBUG_OOS
		bsr_err(14, BSR_LC_BITMAP, device, "unexpected error, could not get bitmap, start(%llu)\n", (unsigned long long)start);
#endif
		return 1;
	}
...
#define bm_op(device, bitmap_index, start, end, op, buffer) \
	({ unsigned long ret; \
	   ...
	   bsr_info(18, BSR_LC_BITMAP, device, "= %lu\n", ret); \
	   ret; })
...
#define __bm_op(device, bitmap_index, start, end, op, buffer) \
	({ unsigned long ret; \
	   ...
	   bsr_info(20, BSR_LC_BITMAP, device, "= %lu\n", ret); \
	   ret; })
#endif
...
#define ___bm_op(device, bitmap_index, start, end, op, buffer, km_type) \
	({ unsigned long ret; \
	   ...
	   bsr_info(22, BSR_LC_BITMAP, device, "= %lu\n", ret); \
	   ret; })
#else
#define ___bm_op(device, bitmap_index, start, end, op, buffer, km_type) \
...
#endif
{
	...
	if ((ULONG_PTR)DeviceObject != FAULT_TEST_FLAG) {
		...
		if(gSimulDiskIoError.ErrorFlag && gSimulDiskIoError.ErrorType == SIMUL_DISK_IO_ERROR_TYPE4) {
			if(IsDiskError()) {
				bsr_err(23, BSR_LC_IO, NO_OBJECT,"SimulDiskIoError: Bitmap I/O Error type4.....ErrorFlag:%lu ErrorCount:%lu\n",gSimulDiskIoError.ErrorFlag, gSimulDiskIoError.ErrorCount);
...
no_memory :
	bsr_err(26, BSR_LC_BITMAP, NO_OBJECT,"Unexpected logic: No memory!\n");
	return -ENOMEM;
}
