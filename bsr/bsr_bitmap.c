	...
	enum bitmap_operations op, __le32 *buffer)
{
	...
	{
#ifdef _DEBUG_OOS
		bsr_err(device, "unexpected error, could not get bitmap, start(%llu)", (unsigned long long)start);
#endif
		return 1;
	}
...
#ifdef _DEBUG_OOS
		// DW-1153 add error log
		bsr_err(device, "unexpected error, could not get bitmap->bm_pages, start(%llu)", (unsigned long long)start);
#endif
		return 0;
	}
...
#ifdef _DEBUG_OOS
		// DW-1153 add error log
		bsr_err(device, "unexpected error, bitmap->bm_bits is 0, start(%llu)", (unsigned long long)start);
#endif
		return 0;
	}
...
int bsr_bm_resize(struct bsr_device *device, sector_t capacity, int set_new_bits)
{
	...
	bsr_info(device, "resync bitmap: bits=%llu words=%llu pages=%llu", (unsigned long long)bits, (unsigned long long)words, (unsigned long long)want);
...
#endif
{
	...
	if ((ULONG_PTR)DeviceObject != FAULT_TEST_FLAG) {
		...
		if(gSimulDiskIoError.ErrorFlag && gSimulDiskIoError.ErrorType == SIMUL_DISK_IO_ERROR_TYPE4) {
			if(IsDiskError()) {
				bsr_err(NO_OBJECT,"SimulDiskIoError: Bitmap I/O Error type4.....ErrorFlag:%lu ErrorCount:%lu",gSimulDiskIoError.ErrorFlag, gSimulDiskIoError.ErrorCount);
	...
	if ((ctx->flags & BM_AIO_COPY_PAGES) == 0 &&
		...
		bsr_warn(device, "bitmap page idx %llu changed during IO!", (unsigned long long)idx);
	...
	{
		static int cnt = 0;
		bsr_debug(NO_OBJECT,"bm_async_io_complete done.(%d).................!!!", cnt++);
	}
#endif
