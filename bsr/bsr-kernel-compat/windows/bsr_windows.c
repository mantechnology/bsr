...
{
    bsr_err(NO_OBJECT,"breakpoint: msg(%s)", msg);
}
...
struct bio *bio_alloc(gfp_t gfp_mask, int nr_iovecs, ULONG Tag)
{
	...
	if (nr_iovecs > 256) {
		bsr_err(NO_OBJECT,"BSR_PANIC: bio_alloc: nr_iovecs too big = %d. check over 1MB.", nr_iovecs);
		BUG();
	}
	return bio;
}
...
void bio_endio(struct bio *bio, int error)
{
	if (bio->bi_end_io) {
		if(error) {
			bio->bi_bdev = NULL;
			bsr_info(NO_OBJECT,"thread(%s) bio_endio error with err=%d.", current->comm, error);
        	bio->bi_end_io((void*)FAULT_TEST_FLAG, (void*) bio, (void*) error);
		} else { // if bio_endio is called with success(just in case)
			//bsr_info(NO_OBJECT,"thread(%s) bio_endio with err=%d.", current->comm, error);
			bio->bi_bdev = NULL;
...
int bio_add_page(struct bio *bio, struct page *page, unsigned int len,unsigned int offset)
{
	...
	if (bio->bi_vcnt > 1) {
		bsr_err(NO_OBJECT,"BSR_PANIC: bio->bi_vcn=%d. multi page occured!", bio->bi_vcnt);
        BUG();
	}
...
long schedule_ex(wait_queue_head_t *q, long timeout, char *func, int line, bool auto_reset_event) 
{
	...
	if ((q == NULL) || (q == (wait_queue_head_t *)SCHED_Q_INTERRUPTIBLE)) {
        ...
        while (true) {
            ...
            default:
                bsr_err(NO_OBJECT,"BSR_PANIC: KeWaitForMultipleObjects done! default status=0x%x", status);
                BUG();
                break;
            }
            break;
...
struct workqueue_struct *create_singlethread_workqueue(void * name)
{
	...
	if (!NT_SUCCESS(status)) {
		bsr_err(NO_OBJECT,"PsCreateSystemThread failed with status 0x%08X", status);
		kfree(wq);
		return NULL;
	}
	...
	ZwClose(hThread);
	if (!NT_SUCCESS(status)) {
		bsr_err(NO_OBJECT,"ObReferenceObjectByHandle failed with status 0x%08X", status);
		kfree(wq);
		return NULL;
	}
...
unsigned long _spin_lock_irqsave(spinlock_t *lock)
{
	...
	if( curthread == lock->OwnerThread) { 
		bsr_warn(NO_OBJECT,"thread:%p spinlock recursion is happened! function:%s line:%d", curthread, __FUNCTION__, __LINE__);
	} else {
...
void kobject_put(struct kobject *kobj)
{
    ...
    {
        if (kobj->name == NULL) {
            //bsr_warn(NO_OBJECT,"%p name is null.", kobj);
            return;
        }
 ...
 void sock_release(struct socket *sock)
{
	...
	if (!NT_SUCCESS(status)) 
	{
		bsr_err(NO_OBJECT,"error=0x%x", status);
		return;
	}
#endif
	...
	if (sock->sk){
		...
		bsr_debug(NO_OBJECT,"WSK_EVENT_DISABLE (sock = 0x%p)", sock);
		if (!NT_SUCCESS(status)) {
			bsr_debug(NO_OBJECT,"WSK_EVENT_DISABLE failed (sock = 0x%p)", sock);
		}
	}
...
static void __delete_thread(struct task_struct *t)
{
    ...
    // logic check
    if (ct_thread_num < 0) {
        bsr_err(NO_OBJECT,"BSR_PANIC:unexpected ct_thread_num(%d)", ct_thread_num);
        BUG();
    }
}
...
struct task_struct * ct_add_thread(int id, const char *name, BOOLEAN event, ULONG Tag)
{
	...
	if (++ct_thread_num > CT_MAX_THREAD_LIST) {
		bsr_warn(NO_OBJECT,"ct_thread too big(%s, %d)", name, ct_thread_num);
    }
...
void *crypto_alloc_tfm(char *name, u32 mask)
{
	...
	bsr_info(NO_OBJECT,"request crypto name(%s) --> supported crc32c only.", name);
	return (void *)1;
}
...
int generic_make_request(struct bio *bio)
{
	...
	if (KeGetCurrentIrql() <= DISPATCH_LEVEL) {
		...
		if (bio && bio->bi_bdev && bio->bi_bdev->bd_disk && bio->bi_bdev->bd_disk->pDeviceExtension) {
			...
			if (!NT_SUCCESS(status)) {
				bsr_err(NO_OBJECT,"IoAcquireRemoveLock bio->bi_bdev->bd_disk->pDeviceExtension:%p fail. status(0x%x)", bio->bi_bdev->bd_disk->pDeviceExtension, status);
				return -EIO;
			}
		}
		else {
			bsr_warn(NO_OBJECT,"IRQL(%d), bio->bi_bdev->bd_disk->pDeviceExtension null", KeGetCurrentIrql());
			return -EIO;
		}
	}
	else {
		bsr_warn(NO_OBJECT,"IoAcquireRemoveLock IRQL(%d) is too high, bio->pVolExt:%p fail", KeGetCurrentIrql(), bio->bi_bdev->bd_disk->pDeviceExtension);
		return -EIO;
	}
...
retry:
	...
	if (!newIrp) {
		...
		if (retry < 3) {
			...
			retry++;
			bsr_warn(NO_OBJECT, "IoBuildAsynchronousFsdRequest: cannot alloc new IRP, try again (%d/3)\n", retry);
			goto retry;
		}
	...
	if (gSimulDiskIoError.ErrorFlag && gSimulDiskIoError.ErrorType == SIMUL_DISK_IO_ERROR_TYPE0) {
		if (IsDiskError()) {
			bsr_err(NO_OBJECT,"SimulDiskIoError: type0...............ErrorFlag:%d ErrorCount:%d",gSimulDiskIoError.ErrorFlag, gSimulDiskIoError.ErrorCount);
			// DW-1831 check whether bio->bi_bdev and bio->bi_bdev->bd_disk are null.
			if (bio && bio->bi_bdev && bio->bi_bdev->bd_disk && bio->bi_bdev->bd_disk->pDeviceExtension)
				IoReleaseRemoveLock(&bio->bi_bdev->bd_disk->pDeviceExtension->RemoveLock, NULL);
...
int _BSR_ratelimit(struct ratelimit_state *rs, const char * func, const char * __FILE, const int __LINE)
{
	...
	if (time_is_before_jiffies(rs->begin + rs->interval)){
		if (rs->missed)
			bsr_warn(NO_OBJECT,"%s(%s@%d): %d callbacks suppressed", func, __FILE, __LINE, rs->missed);
		rs->begin = jiffies;
		rs->printed = 0;
		rs->missed = 0;
	}
...
void update_targetdev(PVOLUME_EXTENSION pvext, bool bMountPointUpdate)
{
	...
	if(bMountPointUpdate) {
		if(!IsEmptyUnicodeString(&pvext->MountPoint)) {
			...
			if (!IsEmptyUnicodeString(&old_mount_point))
				bsr_debug(NO_OBJECT,"old_mount_point:%wZ", &old_mount_point);
		}
		...
		if(NT_SUCCESS(status)) {
			...
			if (!IsEmptyUnicodeString(&pvext->MountPoint))
				bsr_debug(NO_OBJECT,"new mount point:%wZ", &pvext->MountPoint);
	...
	bsr_debug(NO_OBJECT,"d_size: %lld bytes bd_contains->d_size: %lld bytes max_hw_sectors: %lld sectors", d_size, pvext->dev->bd_contains ? pvext->dev->bd_contains->d_size : 0, pvext->dev->bd_disk->queue->max_hw_sectors);
}
...
void monitor_mnt_change(PVOID pParam)
{
	...
	do {
		...
		if (!NT_SUCCESS(status)) {
			bsr_err(NO_OBJECT,"could not open mount manager, status : 0x%x", status);
			break;
		}

		status = ZwCreateEvent(&hEvent, GENERIC_ALL, 0, NotificationEvent, FALSE);
		if (!NT_SUCCESS(status)) {
			bsr_err(NO_OBJECT,"could not create event, status : 0x%x", status);
			break;
		}
		...
		while (TRUE == atomic_read(&g_monitor_mnt_working)) {
			...
			if (!NT_SUCCESS(status)) {
				bsr_err(NO_OBJECT,"ZwDeviceIoControl with IOCTL_MOUNTMGR_CHANGE_NOTIFY has been failed, status : 0x%x", status);
				break;
			...
			if (!NT_SUCCESS(status)) {
				bsr_err(NO_OBJECT,"PsCreateSystemThread for adjust_changes_to_volume failed, status : 0x%x", status);
				break;
			}
...
NTSTATUS start_mnt_monitor()
{
	...
	if (!NT_SUCCESS(status)) {
		bsr_err(NO_OBJECT,"PsCreateSystemThread for monitor_mnt_change failed with status 0x%08X", status);
		return status;
	}
...
LONGLONG get_targetdev_volsize(PVOLUME_EXTENSION VolumeExtension)
{
	...
	if (!NT_SUCCESS(status)) {
		bsr_warn(NO_OBJECT,"get volume size error = 0x%x", status);
...
struct bsr_device *get_device_with_vol_ext(PVOLUME_EXTENSION pvext, bool bCheckRemoveLock)
{
	...
	if (bCheckRemoveLock) {
		...
		if (!NT_SUCCESS(status)) {
			bsr_info(NO_OBJECT,"failed to acquire remove lock with status:0x%x, return NULL", status);
			return NULL;
		}
	}
...
BOOLEAN do_add_minor(unsigned int minor)
{
    ...
    if (!new_reg_buf) {
        bsr_err(NO_OBJECT,"Failed to ExAllocatePoolWithTag new_reg_buf", 0);
        return FALSE;
    }
    ...
    if (!keyInfo) {
        ...
        bsr_err(NO_OBJECT,"Failed to ExAllocatePoolWithTag() size(%u)", size);
        goto cleanup;
    }
    ...
    if (!valueInfo) {
        status = STATUS_INSUFFICIENT_RESOURCES;
        bsr_err(NO_OBJECT,"Failed to ExAllocatePoolWithTag() valueInfoSize(%d)", valueInfoSize);
        goto cleanup;
    }
...
struct block_device *blkdev_get_by_path(const char *path, fmode_t mode, void *holder, bool bUpdatetargetdev)
{
	...
	if (!NT_SUCCESS(status)) {
		bsr_warn(NO_OBJECT,"Wrong path = %s", path);
...
void dumpHex(const void *aBuffer, const size_t aBufferSize, size_t aWidth)
{
	...
	*(sLine + sLineSize - 1) = '\0';
	...
	bsr_info(NO_OBJECT,"DUMP: addr=0x%p, sz=%d. width=%d", aBuffer, aBufferSize, aWidth);
...
int call_usermodehelper(char *path, char **argv, char **envp, unsigned int wait)
{
	...
	if (!cmd_line) {
		bsr_err(NO_OBJECT,"malloc(%d) failed", leng);
		if(pSock) {
			kfree(pSock);
		}
		return -1;
    ...
    bsr_info(NO_OBJECT,"malloc len(%d) cmd_line(%s)", leng, cmd_line);
	...
	if (!NT_SUCCESS(Status)) {
		...
		bsr_info(NO_OBJECT,"Connect() timeout. IRQL(%d)", KeGetCurrentIrql());
		goto error;
	}
...

	{
		...
		if ((readcount = Receive(pSock, &hello, (long)strlen(BSR_DAEMON_SOCKET_STRING), 0, g_handler_timeout)) == (long)strlen(BSR_DAEMON_SOCKET_STRING)) {
			bsr_debug(NO_OBJECT,"recv HI!!! ");
		} else {
			if (readcount == -EAGAIN) {
				bsr_info(NO_OBJECT,"error rx hi timeout(%d) g_handler_retry(%d) !!!!", g_handler_timeout, g_handler_retry);
			} else {
				bsr_err(NO_OBJECT, "error recv status=0x%x", readcount);
			}
			ret = -1;
		...
		if (0 != memcmp(hello, BSR_DAEMON_SOCKET_STRING, strlen(BSR_DAEMON_SOCKET_STRING))) {
			bsr_err(NO_OBJECT, "this is not a daemon connection.\n");
			goto error;
		}
		...
		if ((Status = SendLocal(pSock, BSR_DAEMON_SOCKET_STRING, (unsigned int)strlen(BSR_DAEMON_SOCKET_STRING), 0, g_handler_timeout)) != (long)strlen(BSR_DAEMON_SOCKET_STRING)) {
			bsr_err(NO_OBJECT, "send socket string fail stat=0x%x\n", Status);
			ret = -1;
			goto error;
		}

		if ((Status = SendLocal(pSock, cmd_line, (unsigned int)strlen(cmd_line), 0, g_handler_timeout)) != (long) strlen(cmd_line)) {
			bsr_err(NO_OBJECT,"send command fail stat=0x%x", Status);
			ret = -1;
			goto error;
		}

		if ((readcount = Receive(pSock, &ret, 1, 0, g_handler_timeout)) > 0) {
			bsr_debug(NO_OBJECT,"recv val=0x%x", ret);
		} else {
			if (readcount == -EAGAIN) {
				bsr_info(NO_OBJECT,"recv retval timeout(%d)!", g_handler_timeout);
			} else {
				bsr_info(NO_OBJECT,"recv status=0x%x", readcount);
			}
			ret = -1;
			goto error;
		}
		...
		if ((Status = SendLocal(pSock, "BYE", 3, 0, g_handler_timeout)) != 3) {
			bsr_err(NO_OBJECT,"send bye fail stat=0x%x", Status); // ignore!
		}

		bsr_debug(NO_OBJECT,"Disconnect:shutdown...", Status);
		Disconnect(pSock);

#if 0
		if ((readcount = Receive(Socket, &ret, 1, 0, 0)) > 0) {
			bsr_info(NO_OBJECT,"recv dummy  val=0x%x", ret);// ignore!
		} else {
			bsr_info(NO_OBJECT,"recv dummy  status=%d", readcount);// ignore!
		}
#endif
	}
...
int bsr_backing_bdev_events(struct bsr_device *device)
{
	...
	if (!NT_SUCCESS(status)) {
		bsr_err(NO_OBJECT,"mvolGetDiskPerf status=0x%x", status);
