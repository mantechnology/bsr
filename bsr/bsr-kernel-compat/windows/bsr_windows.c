...
struct page  *alloc_page(int flag)
{
	...
	if (!p)	{
		drbd_info(,"alloc_page struct page failed\n");
		return NULL;
	}	
	...
	if (!p->addr)	{
		kfree(p); 
		drbd_info(,"alloc_page PAGE_SIZE failed\n");
		return NULL;
	}
...
struct bio *bio_alloc(gfp_t gfp_mask, int nr_iovecs, ULONG Tag)
{
	...
	if (nr_iovecs > 256) {
		drbd_err(,"DRBD_PANIC: bio_alloc: nr_iovecs too big = %d. check over 1MB.\n", nr_iovecs);
		BUG();
	}
	return bio;
}
...
int bio_add_page(struct bio *bio, struct page *page, unsigned int len,unsigned int offset)
{
	...
	if (bio->bi_vcnt > 1) {
		drbd_err(,"DRBD_PANIC: bio->bi_vcn=%d. multi page occured!\n", bio->bi_vcnt);
        BUG();
	}
...
long schedule_ex(wait_queue_head_t *q, long timeout, char *func, int line, bool auto_reset_event) 
{
	...
	if ((q == NULL) || (q == (wait_queue_head_t *)SCHED_Q_INTERRUPTIBLE)) {
        ...
        while (true) {
            ...
            default:
                drbd_err(,"DRBD_PANIC: KeWaitForMultipleObjects done! default status=0x%x\n", status);
                BUG();
                break;
            }
            break;
...
int mutex_lock_interruptible(struct mutex *m)
{
	...
	default:
		err = -EIO;
		drbd_err(,"KeWaitForMultipleObjects returned unexpected status(0x%x)", status);
		break;
	}
...
unsigned long _spin_lock_irqsave(spinlock_t *lock)
{
	...
	if( curthread == lock->OwnerThread) { 
		drbd_warn(,"thread:%p spinlock recursion is happened! function:%s line:%d\n", curthread, __FUNCTION__, __LINE__);
	} else {
 ...
 void sock_release(struct socket *sock)
{
	...
	if (sock->sk){
		...
		drbd_debug(,"WSK_EVENT_DISABLE (sock = 0x%p)\n", sock);
		if (!NT_SUCCESS(status)) {
			drbd_debug(,"WSK_EVENT_DISABLE failed (sock = 0x%p)\n", sock);
		}
	}
...
static void __delete_thread(struct task_struct *t)
{
    ...
    if (ct_thread_num < 0)
    {
        drbd_err(,"DRBD_PANIC:unexpected ct_thread_num(%d)\n", ct_thread_num);
        BUG();
    }
}
...
struct task_struct * ct_add_thread(PKTHREAD id, const char *name, BOOLEAN event, ULONG Tag)
{
	...
	if (++ct_thread_num > CT_MAX_THREAD_LIST) {
		drbd_warn(,"ct_thread too big(%s, %d)\n", name, ct_thread_num);
    }
...
void *crypto_alloc_tfm(char *name, u32 mask)
{
	...
	drbd_info(,"request crypto name(%s) --> supported crc32c only.\n", name);
	return (void *)1;
}
...
int generic_make_request(struct bio *bio)
{
	...
	if (KeGetCurrentIrql() <= DISPATCH_LEVEL) {
		...
		if (bio && bio->bi_bdev && bio->bi_bdev->bd_disk && bio->bi_bdev->bd_disk->pDeviceExtension) {
			...
			if (!NT_SUCCESS(status)) {
				drbd_err(,"IoAcquireRemoveLock bio->bi_bdev->bd_disk->pDeviceExtension:%p fail. status(0x%x)\n", bio->bi_bdev->bd_disk->pDeviceExtension, status);
				return -EIO;
			}
		}
		else {
			drbd_warn(,"IRQL(%d), bio->bi_bdev->bd_disk->pDeviceExtension null\n", KeGetCurrentIrql());
			return -EIO;
		}
	}
	else {
		drbd_warn(,"IoAcquireRemoveLock IRQL(%d) is too high, bio->pVolExt:%p fail\n", KeGetCurrentIrql(), bio->bi_bdev->bd_disk->pDeviceExtension);
		return -EIO;
	}
...
#ifdef DRBD_TRACE
    drbd_debug(,"(%s)Local I/O(%s): sect=0x%llx sz=%d IRQL=%d buf=0x%p, off&=0x%llx target=%c:\n", 
	...
	if (!newIrp) {
		drbd_err(,"IoBuildAsynchronousFsdRequest: cannot alloc new IRP\n");
		//DW-1831 check whether bio->bi_bdev and bio->bi_bdev->bd_disk are null.
		if (bio && bio->bi_bdev && bio->bi_bdev->bd_disk && bio->bi_bdev->bd_disk->pDeviceExtension)
	...
	if (gSimulDiskIoError.ErrorFlag && gSimulDiskIoError.ErrorType == SIMUL_DISK_IO_ERROR_TYPE0) {
		if (IsDiskError()) {
			drbd_err(,"SimulDiskIoError: type0...............ErrorFlag:%d ErrorCount:%d\n",gSimulDiskIoError.ErrorFlag, gSimulDiskIoError.ErrorCount);
			//DW-1831 check whether bio->bi_bdev and bio->bi_bdev->bd_disk are null.
			if (bio && bio->bi_bdev && bio->bi_bdev->bd_disk && bio->bi_bdev->bd_disk->pDeviceExtension)
				IoReleaseRemoveLock(&bio->bi_bdev->bd_disk->pDeviceExtension->RemoveLock, NULL);
...
#else
		drbd_err(,"drbd:skb_put: skb_over_panic\n");
#endif
	}
...
int _DRBD_ratelimit(struct ratelimit_state *rs, const char * func, const char * __FILE, const int __LINE)
{
	...
	if (time_is_before_jiffies(rs->begin + rs->interval)){
		if (rs->missed)
			drbd_warn(,"%s(%s@%d): %d callbacks suppressed\n", func, __FILE, __LINE, rs->missed);
		rs->begin = jiffies;
		rs->printed = 0;
		rs->missed = 0;
	}
...
bool _expect(long exp, struct drbd_conf *mdev, char *file, int line)
{
	if (!exp)
	{
		drbd_err(,"minor(%d) ASSERTION FAILED in file:%s line:%d\n", mdev->minor, file, line);
        BUG();
	}
	return exp;
}
...
void update_targetdev(PVOLUME_EXTENSION pvext, bool bMountPointUpdate)
{
	...
	if (!pvext) {
		drbd_warn(,"update_targetdev fail pvext is NULL\n");
		return;
	}
			...
			if (pvext->Active && bWasExist) {
				if(IsEmptyUnicodeString(&pvext->MountPoint) || 
					...
					if (device && get_ldev_if_state(device, D_NEGOTIATING)) {
						drbd_warn(,"replicating volume letter is changed, detaching\n");
...
void monitor_mnt_change(PVOID pParam)
{
	...
	do {
		...
		if (!NT_SUCCESS(status)) {
			drbd_err(,"could not open mount manager, status : 0x%x\n", status);
			break;
		}

		status = ZwCreateEvent(&hEvent, GENERIC_ALL, 0, NotificationEvent, FALSE);
		if (!NT_SUCCESS(status)) {
			drbd_err(,"could not create event, status : 0x%x\n", status);
			break;
		}
		...
		while (TRUE == atomic_read(&g_monitor_mnt_working)) {
			...
			if (!NT_SUCCESS(status)) {
				drbd_err(,"ZwDeviceIoControl with IOCTL_MOUNTMGR_CHANGE_NOTIFY has been failed, status : 0x%x\n", status);
				break;
			...
			if (!NT_SUCCESS(status)) {
				drbd_err(,"PsCreateSystemThread for adjust_changes_to_volume failed, status : 0x%x\n", status);
				break;
			}
...
NTSTATUS start_mnt_monitor()
{
	...
	if (!NT_SUCCESS(status)) {
		drbd_err(,"PsCreateSystemThread for monitor_mnt_change failed with status 0x%08X\n", status);
		return status;
	}
...
LONGLONG get_targetdev_volsize(PVOLUME_EXTENSION VolumeExtension)
{
	...
	if (VolumeExtension->TargetDeviceObject == NULL) {
		drbd_err(,"TargetDeviceObject is null!\n");
		return (LONGLONG)0;
	}
	status = mvolGetVolumeSize(VolumeExtension->TargetDeviceObject, &volumeSize);
	if (!NT_SUCCESS(status)) {
		drbd_warn(,"get volume size error = 0x%x\n", status);
...
struct block_device * create_drbd_block_device(IN OUT PVOLUME_EXTENSION pvext)
{
    ...
    if (!dev) {
        drbd_err(,"Failed to allocate block_device NonPagedMemory\n");
        return NULL;
    }
	...
	if (!dev->bd_disk)
	{
		drbd_err(,"Failed to allocate gendisk NonPagedMemory\n");
		goto gendisk_failed;
	}
	...
	{
		drbd_err(,"Failed to allocate request_queue NonPagedMemory\n");
...
struct drbd_device *get_device_with_vol_ext(PVOLUME_EXTENSION pvext, bool bCheckRemoveLock)
{
	...
	if (!pvext->dev)
	{
		drbd_err(,"failed to get drbd device since pvext->dev is NULL\n");
		return NULL;		
	}
	...
	if (bCheckRemoveLock)
	{
		...
		if (!NT_SUCCESS(status))
		{
			drbd_info(,"failed to acquire remove lock with status:0x%x, return NULL\n", status);
			return NULL;
		}
	}
...
BOOLEAN do_add_minor(unsigned int minor)
{
    ...
    if (!new_reg_buf) {
        drbd_err(,"Failed to ExAllocatePoolWithTag new_reg_buf\n", 0);
        return FALSE;
    }
    ...
    if (!keyInfo) {
        ...
        drbd_err(,"Failed to ExAllocatePoolWithTag() size(%d)\n", size);
        goto cleanup;
    }
    ...
    if (!valueInfo) {
        status = STATUS_INSUFFICIENT_RESOURCES;
        drbd_err(,"Failed to ExAllocatePoolWithTag() valueInfoSize(%d)\n", valueInfoSize);
        goto cleanup;
    }
...
void dumpHex(const void *aBuffer, const size_t aBufferSize, size_t aWidth)
{
	...
	if (!sLine)
	{
		drbd_err(,"sLine:kzalloc failed\n");
		return;
	}

	*(sLine + sLineSize - 1) = '\0';

	drbd_info(,"DUMP: addr=0x%p, sz=%d. width=%d\n", aBuffer, aBufferSize, aWidth);
...
int call_usermodehelper(char *path, char **argv, char **envp, unsigned int wait)
{
	...
	if (!pSock) {
		drbd_err(,"call_usermodehelper kzalloc failed\n");
		return -1;
	}
#ifdef _WIN64
	...
	if (!NT_SUCCESS(Status)) {
		...
		drbd_info(,"Connect() timeout. IRQL(%d)\n", KeGetCurrentIrql());
		goto error;
	}

	drbd_info(,"Connected to the %u.%u.%u.%u:%u  status:0x%08X IRQL(%d)\n", 
...

	{
		...
		if ((readcount = Receive(pSock, &hello, 2, 0, g_handler_timeout)) == 2) {
			...
			if (readcount == -EAGAIN) {
				drbd_info(,"error rx hi timeout(%d) g_handler_retry(%d) !!!!\n", g_handler_timeout, g_handler_retry);
			} else {
				drbd_info(,"error recv status=0x%x\n", readcount);
			}
			ret = -1;
		...
		if ((Status = SendLocal(pSock, cmd_line, (unsigned int)strlen(cmd_line), 0, g_handler_timeout)) != (long) strlen(cmd_line)) {
			drbd_err(,"send command fail stat=0x%x\n", Status);
			ret = -1;
			goto error;
		}

		if ((readcount = Receive(pSock, &ret, 1, 0, g_handler_timeout)) > 0) {
			drbd_debug(,"recv val=0x%x\n", ret);
		} else {
			if (readcount == -EAGAIN) {
				drbd_info(,"recv retval timeout(%d)!\n", g_handler_timeout);
			} else {
				drbd_info(,"recv status=0x%x\n", readcount);
			}
			ret = -1;
			goto error;
		}
		...
		if ((Status = SendLocal(pSock, "BYE", 3, 0, g_handler_timeout)) != 3) {
			drbd_err(,"send bye fail stat=0x%x\n", Status); // ignore!
		}

		drbd_debug(,"Disconnect:shutdown...\n", Status);
		Disconnect(pSock);

#if 0
		if ((readcount = Receive(Socket, &ret, 1, 0, 0)) > 0) {
			drbd_info(,"recv dummy  val=0x%x\n", ret);// ignore!
		} else {
			drbd_info(,"recv dummy  status=%d\n", readcount);// ignore!
		}
#endif
	}
...
int drbd_backing_bdev_events(struct drbd_device *device)
{
	...
	if (!NT_SUCCESS(status))
	{
		drbd_err(,"mvolGetDiskPerf status=0x%x\n", status);
		return mdev->writ_cnt + mdev->read_cnt;
	}
	// drbd_info(,"mdev: %d + %d = %d, diskPerf: %lld + %lld = %lld\n",
...
VOID RetryAsyncWriteRequest(struct bio* bio, PIRP Irp, NTSTATUS error, char* ctx)
{
	...
	if (write_log)
		drbd_warn(,"ctx:%s error:%x bio->io_retry:%d\n", ctx, error, bio->io_retry);
