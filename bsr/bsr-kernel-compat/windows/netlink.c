...
static bool push_msocket_entry(void * ptr)
{
    ...
    //drbd_debug(NO_DEVICE,"Added entry(0x%p), slink(0x%p), socket(0x%p)\n", entry, entry->slink, entry->ptr);
...
static void pop_msocket_entry(void * ptr)
{
    ...
    while (iter) {
        ...
        if (socket_entry && socket_entry->ptr == ptr) {
            //drbd_debug(NO_DEVICE,"socket_entry(0x%p), slink(0x%p), socket(0x%p) found in list\n", socket_entry, socket_entry->slink, socket_entry->ptr);
...
static int _genl_dump(struct genl_ops * pops, struct sk_buff * skb, struct netlink_callback * cb, struct genl_info * info)
{
    ...
    if (0 == err) {
        ...
        drbd_info(NO_DEVICE,"drbd_adm_get_status_all err = %d\n", err);
    }
    ...
    drbd_debug(NO_DEVICE,"send_reply(%d) seq(%d)\n", err, cb->nlh->nlmsg_seq);
...
int genlmsg_unicast(struct sk_buff *skb, struct genl_info *info)
{
	...
	if ((sent = SendLocal(info->pSock, skb->data, skb->len, 0, (DRBD_TIMEOUT_DEF*100))) == (skb->len)) {
        ...
        drbd_info(NO_DEVICE,"sent Error=0x%x. sock=%p, data=%p sz=%d\n", sent, info->pSock->sk, skb->data, skb->len);
...
struct genl_info * genl_info_new(struct nlmsghdr * nlh, struct socket* sock, struct nlattr **attrs)
{
    ...
    if (!pinfo) {
        drbd_err(NO_DEVICE,"Failed to allocate (struct genl_info) memory. size(%d)\n",
...
InitWskNetlink(void * pctx)
{
    ...
    if (!NT_SUCCESS(status)) {
        drbd_err(NO_DEVICE,"Failed to init. status(0x%x)\n", status);
        return;
    }
    ...
    drbd_info(NO_DEVICE,"Netlink Server Start\n");
	
	gpNetlinkServerSocket = kzalloc(sizeof(struct socket), 0, '42DW');
	if(!gpNetlinkServerSocket) {
		drbd_err(NO_DEVICE,"Failed to alloc Netlink server struct socket\n");
		return;
	}
    ...
    if (!netlink_socket) {
        drbd_err(NO_DEVICE,"Failed to create Netlink server socket\n");
        goto end;
    }
    ...
    if (!NT_SUCCESS(status)) {
        drbd_err(NO_DEVICE,"Failed to bind. status(0x%x)\n", status);
...
static int w_connect(struct drbd_work *w, int cancel)
{
	...
	drbd_info(NO_DEVICE,"w_connect:\n");

	status = KeWaitForSingleObject(&resource->workerdone, Executive, KernelMode, FALSE, &timeout);
	if (status == STATUS_TIMEOUT) {
		drbd_info(NO_DEVICE,"w_connect:KeWaitForSingleObject timeout\n");
	}
    ...
    if (pinfo->nlhdr->nlmsg_flags && NLM_F_DUMP)
    {
        ...
        if (skb)
        {
            ...
            while (ret > 0) {
				...
				if(cnt++ > 512) {
					drbd_info(NO_DEVICE,"_genl_dump exceed process break;\n");
					break;
				}
            }
...
NetlinkWorkThread(PVOID context)
{
    ...
    if (!psock_buf) {
        drbd_err(NO_DEVICE,"Failed to allocate NP memory. size(%d)\n", NLMSG_GOODSIZE);
        goto cleanup;
    }
    ...
    while (true) {
        ...
        if (readcount == 0) {
            drbd_debug(NO_DEVICE,"Receive done...\n");
            goto cleanup;
        } else if(readcount < 0) {
            drbd_info(NO_DEVICE,"Receive error = 0x%x\n", readcount);
            goto cleanup;
        }
        ...
        if (strstr(psock_buf, DRBD_EVENT_SOCKET_STRING)) {
			drbd_debug(NO_DEVICE,"DRBD_EVENT_SOCKET_STRING received. socket(0x%p)\n", socket);
		...
		if( ((unsigned int)readcount != nlh->nlmsg_len) 
			...
			drbd_warn(NO_DEVICE,"Unrecognizable netlink command arrives and doesn't process...\n");
			drbd_debug(NO_DEVICE,"rx(%d), len(%d), flags(0x%x), type(0x%x), seq(%d), magic(%x)\n",
        ...
        if (!nlmsg_ok(nlh, readcount)) {
            drbd_err(NO_DEVICE,"rx message(%d) crashed!\n", readcount);
            goto cleanup;
        }

        drbd_debug(NO_DEVICE,"rx readcount(%d), headerlen(%d), cmd(%d), flags(0x%x), type(0x%x), seq(%d), magic(%x)\n",
        ...
        if (gmh) {
            ...
            if (mdev && drbd_suspended(mdev)) {
                ...
                drbd_warn(NO_DEVICE,"minor(%d) suspended\n", gmh->minor);
                goto cleanup;
            }
        }
        ...
        if (pops) {
            ...
            if( (DRBD_ADM_GET_RESOURCES <= cmd)  && (cmd <= DRBD_ADM_GET_PEER_DEVICES) ) {
				drbd_debug(NO_DEVICE,"drbd netlink cmd(%s:%u) begin ->\n", pops->str, cmd);
            } else {
            	drbd_info(NO_DEVICE,"drbd netlink cmd(%s:%u) begin ->\n", pops->str, cmd);
            }
			...
			if (STATUS_SUCCESS == status) {
				...
				if (err) {
					drbd_err(NO_DEVICE,"netlink cmd failed while operating. cmd(%u), error(%d)\n", cmd, err);
					errcnt++;
				}
				if( (DRBD_ADM_GET_RESOURCES <= cmd)  && (cmd <= DRBD_ADM_GET_PEER_DEVICES) ) {
					drbd_debug(NO_DEVICE,"drbd netlink cmd(%s:%u) done <-\n", pops->str, cmd);
				} else {
					drbd_info(NO_DEVICE,"drbd netlink cmd(%s:%u) done <-\n", pops->str, cmd);
				}
			} else {
				drbd_info(NO_DEVICE,"drbd netlink cmd(%s:%u) Failed to acquire the mutex status: 0x%x\n", pops->str, cmd, status);
			}

        } else {
            drbd_info(NO_DEVICE,"Not validated cmd(%d)\n", cmd);
        }
    }
...
cleanup:
    ...
    if (errcnt) {
        drbd_err(NO_DEVICE,"NetlinkWorkThread:%p done. error occured %d times\n",KeGetCurrentThread(), errcnt);
    } else {
		drbd_debug(NO_DEVICE,"NetlinkWorkThread:%p done...\n",KeGetCurrentThread());
    }
}
...
)
{
    ...
    drbd_debug(NO_DEVICE,"%u.%u.%u.%u:%u -> %u.%u.%u.%u:%u connected\n",
	...
	if(pRemote->sin_addr.S_un.S_un_b.s_b1 != 0x7f) {
		drbd_debug(NO_DEVICE,"External connection attempt was made and blocked.\n");		
		return STATUS_REQUEST_NOT_ACCEPTED;
	}
