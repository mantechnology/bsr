...
static int _genl_dump(struct genl_ops * pops, struct sk_buff * skb, struct netlink_callback * cb, struct genl_info * info)
{
    ...
    if (0 == err) {
		...
		bsr_warn(2, BSR_LC_NETLINK, NO_OBJECT, "Failed to get the state of all objects. err(%d)\n", err);
    }
...
int genlmsg_unicast(struct sk_buff *skb, struct genl_info *info)
{
	...
	if ((sent = SendLocal(info->pSock, skb->data, skb->len, 0, (BSR_TIMEOUT_DEF*100))) == (skb->len)) {
		...
		bsr_warn(3, BSR_LC_NETLINK, NO_OBJECT, "Failed to send. status(0x%x) socket(%p) data(%p) size=%d\n", sent, info->pSock->sk, skb->data, skb->len);
...
InitWskNetlink(void * pctx)
{
    ...
    if (!NT_SUCCESS(status)) {
		bsr_err(5, BSR_LC_NETLINK, NO_OBJECT, "Failed to init wsk. status(0x%x)\n", status);
        return;
    }
...
NetlinkWorkThread(PVOID context)
{
    ...
    while (true) {
        ...
        if (!nlmsg_ok(nlh, readcount)) {
			bsr_err(17, BSR_LC_NETLINK, NO_OBJECT, "message does not match the rest of the data. read count(%d)\n", readcount);
            goto cleanup;
        }
        ...
        if (pops) {
			...
			if (STATUS_SUCCESS == status) {
				...
				if (err) {
					bsr_err(19, BSR_LC_NETLINK, NO_OBJECT, "command failed while operating. cmd(%u), error(%d)\n", cmd, err);
					errcnt++;
				}
				if( (BSR_ADM_GET_RESOURCES <= cmd)  && (cmd <= BSR_ADM_GET_PEER_DEVICES) ) {
