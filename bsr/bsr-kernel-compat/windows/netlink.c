...
static bool push_msocket_entry(void * ptr)
{
    ...
    //bsr_debug(NO_OBJECT,"Added entry(0x%p), slink(0x%p), socket(0x%p)", entry, entry->slink, entry->ptr);
...
static void pop_msocket_entry(void * ptr)
{
    ...
    while (iter) {
        ...
        if (socket_entry && socket_entry->ptr == ptr) {
            //bsr_debug(NO_OBJECT,"socket_entry(0x%p), slink(0x%p), socket(0x%p) found in list", socket_entry, socket_entry->slink, socket_entry->ptr);
...
int bsr_genl_multicast_events(struct sk_buff * skb, const struct sib_info *sib)
{
    ...
    while (iter) {
        ...
        if (socket_entry) {
            //bsr_debug(NO_OBJECT,"send socket(0x%p), data(0x%p), len(%d)", socket_entry->ptr, skb->data, skb->len);
			int sent = SendLocal(socket_entry->ptr, skb->data, skb->len, 0, (BSR_TIMEOUT_DEF*100));
            if (sent != skb->len) {
                bsr_info(NO_OBJECT,"Failed to send socket(0x%x)", socket_entry->ptr);
            }
        }
        iter = iter->Next;
    }
...
static int _genl_dump(struct genl_ops * pops, struct sk_buff * skb, struct netlink_callback * cb, struct genl_info * info)
{
    ...
    if (0 == err) {
        ...
        bsr_info(NO_OBJECT,"bsr_adm_get_status_all err = %d", err);
    }
    ...
    bsr_debug(NO_OBJECT,"send_reply(%d) seq(%d)", err, cb->nlh->nlmsg_seq);
...
int genlmsg_unicast(struct sk_buff *skb, struct genl_info *info)
{
	...
	if ((sent = SendLocal(info->pSock, skb->data, skb->len, 0, (BSR_TIMEOUT_DEF*100))) == (skb->len)) {
        ...
        bsr_info(NO_OBJECT,"sent Error=0x%x. sock=%p, data=%p sz=%d", sent, info->pSock->sk, skb->data, skb->len);
...
InitWskNetlink(void * pctx)
{
    ...
    if (!NT_SUCCESS(status)) {
        bsr_err(NO_OBJECT,"Failed to init. status(0x%x)", status);
        return;
    }
    ...
    if (!NT_SUCCESS(status)) {
        bsr_err(NO_OBJECT,"Failed to bind. status(0x%x)", status);
...
NetlinkWorkThread(PVOID context)
{
	...
	bsr_debug(NO_OBJECT,"NetlinkWorkThread:%p begin...accept socket:%p remote port:%d",KeGetCurrentThread(),socket, HTON_SHORT(((PNETLINK_WORK_ITEM)context)->RemotePort));
    
    netlink_work_thread_cnt++;

	pSock = kzalloc(sizeof(struct socket), 0, '42DW'); 
	if(!pSock) {
		bsr_err(NO_OBJECT,"Failed to allocate struct socket memory. size(%d)", sizeof(struct socket));
        goto cleanup;
	}
    ...
    if (!psock_buf) {
        bsr_err(NO_OBJECT,"Failed to allocate NP memory. size(%d)", NLMSG_GOODSIZE);
        goto cleanup;
    }
    ...
    while (true) {
        ...
        if (readcount == 0) {
            ...
            bsr_info(NO_OBJECT,"Receive error = 0x%x", readcount);
            goto cleanup;
        }
        ...
        if (strstr(psock_buf, BSR_EVENT_SOCKET_STRING)) {
			bsr_debug(NO_OBJECT,"BSR_EVENT_SOCKET_STRING received. socket(0x%p)", socket);
        ...
        if (!pinfo) {
            bsr_err(NO_OBJECT,"Failed to allocate (struct genl_info) memory. size(%d)", sizeof(struct genl_info));
            goto cleanup;
        }

        bsr_tla_parse(nlh, local_attrs);
        if (!nlmsg_ok(nlh, readcount)) {
            bsr_err(NO_OBJECT,"rx message(%d) crashed!", readcount);
            goto cleanup;
        }
        ...
        if (gmh) {
            ...
            if (mdev && bsr_suspended(mdev)) {
                ...
                bsr_warn(NO_OBJECT,"minor(%d) suspended", gmh->minor);
                goto cleanup;
            }
        }
        ...
        if (pops) {
            ...
            if( (BSR_ADM_GET_RESOURCES <= cmd)  && (cmd <= BSR_ADM_GET_PEER_DEVICES) ) {
				bsr_debug(NO_OBJECT,"bsr netlink cmd(%s:%u) begin ->", pops->str, cmd);
            } else {
            	bsr_info(NO_OBJECT,"bsr netlink cmd(%s:%u) begin ->", pops->str, cmd);
            }
			...
			if (STATUS_SUCCESS == status) {
				...
				if (err) {
					bsr_err(NO_OBJECT,"netlink cmd failed while operating. cmd(%u), error(%d)", cmd, err);
					errcnt++;
				}
				if( (BSR_ADM_GET_RESOURCES <= cmd)  && (cmd <= BSR_ADM_GET_PEER_DEVICES) ) {
					bsr_debug(NO_OBJECT,"bsr netlink cmd(%s:%u) done (cmd_pending:%d) <-", pops->str, cmd, netlink_work_thread_cnt-1);
				} else {
					bsr_info(NO_OBJECT,"bsr netlink cmd(%s:%u) done (cmd_pending:%d) <-", pops->str, cmd, netlink_work_thread_cnt-1);
				}
			} else {
                mutex_unlock(&g_genl_run_cmd_mutex);
				bsr_info(NO_OBJECT,"bsr netlink cmd(%s:%u) Failed to acquire the mutex status: 0x%x", pops->str, cmd, status);
			}

        } else {
            bsr_info(NO_OBJECT,"Not validated cmd(%d)", cmd);
        }
    }
...
cleanup:
    ...
    if (errcnt) {
        bsr_err(NO_OBJECT,"NetlinkWorkThread:%p done. error occured %d times",KeGetCurrentThread(), errcnt);
    } else {
		bsr_debug(NO_OBJECT,"NetlinkWorkThread:%p done...",KeGetCurrentThread());
    }
}
...
)
{
    ...
    if (!netlinkWorkItem) {
        bsr_err(NO_OBJECT,"Failed to allocate NP memory. size(%d)", sizeof(NETLINK_WORK_ITEM));
        return STATUS_REQUEST_NOT_ACCEPTED;
    }
