    ...
    while (true) {
        ...
        if (pops) {
            ...
            if((BSR_ADM_GET_RESOURCES <= cmd) && (cmd <= BSR_ADM_GET_PEER_DEVICES)) {
				// BSR-1746 allow up handlers to query their resource configuration.
				if(BSR_ADM_PRIMARY == g_genl_run_cmd ||
				   BSR_ADM_SECONDARY == g_genl_run_cmd ||
				   BSR_ADM_APPLY_PERSIST_ROLE == g_genl_run_cmd ||
				   BSR_ADM_DOWN == g_genl_run_cmd ||
				   BSR_ADM_NEW_RESOURCE == g_genl_run_cmd ||
				   BSR_ADM_CONNECT == g_genl_run_cmd) {
