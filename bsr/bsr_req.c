			...
			if (set_bits) {
				for_each_peer_device(peer_device, device) {
						...
						peer_device->connection->cstate[NOW] >= C_CONNECTED)
					{
						...
						bsr_info(peer_device,"found disappeared out-of-sync, need to send new one(sector(%llu), size(%u))", (unsigned long long)req->i.sector, req->i.size);
	...
	if (m->bio->pMasterIrp) {
		...
		if (!master_bio->splitInfo) {
			if (master_bio->bi_size <= 0 || master_bio->bi_size > (1024 * 1024)) {
				bsr_err(NO_OBJECT, "size 0x%x ERROR!", master_bio->bi_size);
				BUG();
			}
...
#ifdef BSR_TRACE
	if (put > 1) {
        bsr_debug(NO_OBJECT,"(%s) completion_ref: put=%d !!!", current->comm, put);
	}
#endif
	if (!atomic_sub_and_test(put, &req->completion_ref))
	{
#ifdef BSR_TRACE
		bsr_debug(NO_OBJECT,"(%s) completion_ref=%d. No complete req yet! sect=0x%llx sz=%d", current->comm, req->completion_ref, req->i.sector, req->i.size);
#endif
		return 0;
	}
...
#ifdef BSR_TRACE
	bsr_debug(NO_OBJECT,"sect=0x%llx sz=%d done!!!", req->i.sector, req->i.size);
#endif
	return 1;
}
		...
		struct bio_and_error *m)
{
	...
	switch (what) {
	default:
		bsr_err(device, "LOGIC BUG in %s:%u", __FILE__ , __LINE__);
		break;
...
static void __maybe_pull_ahead(struct bsr_device *device, struct bsr_connection *connection)
{
	...
	if (nc->cong_fill) {
		...
		if (total_in_flight >= nc->cong_fill) {
			bsr_info(device, "Congestion-fill threshold reached %lluKB", total_in_flight >> 10 );
			congested = true;
		}
	}
...
static int bsr_process_write_request(struct bsr_request *req)
{
	...
	for_each_peer_device(peer_device, device) {
		...
		_printk(__FUNCTION__, KERN_OOS, ""KERN_OOS"["OOS_TRACE_STRING"] pnode-id(%d), bitmap_index(%d) req(%p), remote(%d), send_oos(%d), sector(%lu ~ %lu)",
...
#endif
{
	...
	if (IS_ERR_OR_NULL(req)) {
		if (req)
			bsr_err(device, "FIXME!!, bug!? failed to local request prepare, bio(%p), sector(%llu), size(%u)", bio, bio->bi_sector, bio->bi_size);
...
void do_submit(struct work_struct *ws)
{
	...
	for (;;) {
		...
		for (;;) {
			...
			*/
			...
			if (!wfa_lists_empty(&wfa, pending)) {
				if(al_wait_count)
					bsr_debug(device, "al_wait retry count : %llu", (unsigned long long)al_wait_count);
				al_wait_count = 0;
				break;
			}
			al_wait_count += 1;
...
#endif
				struct bsr_peer_device *peer_device;
				bsr_err(device, "al_wait timeout... disconnect, retry %llu", (unsigned long long)al_wait_count);
		...
		if (device->resource->role[NOW] == R_SECONDARY && ts != 0) {
			...
			if (ts > ((3 * 1000) * HZ)) {
				bsr_warn(device, "actlog commit takes a long time(%lldus)", ts);
			}
		}
