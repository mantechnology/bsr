...
#endif
{
	...
	bsr_debug(BSR_LC_TEMP, NO_OBJECT,"BIO_ENDIO_FN_START:Thread(%s) bsr_md_io_complete IRQL(%d) .............\n", current->comm, KeGetCurrentIrql());
	...
	if ((ULONG_PTR)DeviceObject != FAULT_TEST_FLAG) {
		...
		if(gSimulDiskIoError.ErrorFlag && gSimulDiskIoError.ErrorType == SIMUL_DISK_IO_ERROR_TYPE3) {
			if(IsDiskError()) {
				bsr_err(BSR_LC_TEMP, NO_OBJECT,"SimulDiskIoError: Meta Data I/O Error type3.....ErrorFlag:%d ErrorCount:%d\n", gSimulDiskIoError.ErrorFlag, gSimulDiskIoError.ErrorCount);
	...
	if (!bio) {
		bsr_debug(BSR_LC_TEMP, NO_OBJECT,"null bio\n");
		BIO_ENDIO_FN_RETURN;
	}
	...
	*/
	...
	if (device->ldev) /* special case: bsr_md_read() during bsr_adm_attach() */
		...
		bsr_debug(BSR_LC_TEMP, device, "ldev null\n");
...
static void bsr_endio_read_sec_final(struct bsr_peer_request *peer_req) __releases(local)
{
	...
	if (test_bit(__EE_WAS_INACTIVE_REQ, &peer_req->flags)) {
		if (test_bit(__EE_WAS_LOST_REQ, &peer_req->flags)) {
			bsr_info(BSR_LC_TEMP, NO_OBJECT, "destroy, read lost inactive_ee(%p), sector(%llu), size(%u)\n", peer_req, (unsigned long long)peer_req->i.sector, peer_req->i.size);
			...
			list_for_each_entry_safe_ex(struct bsr_peer_request, p_req, t_inative, &connection->inactive_ee, w.list) {
				if (peer_req == p_req) {
					bsr_info(BSR_LC_TEMP, device, "destroy, read inactive_ee(%p), sector(%llu), size(%u)\n", peer_req, (unsigned long long)peer_req->i.sector, peer_req->i.size);
...
void bsr_endio_write_sec_final(struct bsr_peer_request *peer_req) __releases(local)
{
	...
	if (test_bit(__EE_WAS_INACTIVE_REQ, &peer_req->flags)) {
		if (test_bit(__EE_WAS_LOST_REQ, &peer_req->flags)) {
			bsr_info(BSR_LC_TEMP, NO_OBJECT, "destroy, wrtie inactive_ee(%p), sector(%llu), size(%u)\n", peer_req, (unsigned long long)peer_req->i.sector, peer_req->i.size);
			bsr_free_peer_req(peer_req); 
		}
		else {
			struct bsr_peer_request *p_req, *t_inative;
			...
			list_for_each_entry_safe_ex(struct bsr_peer_request, p_req, t_inative, &connection->inactive_ee, w.list) {
				if (peer_req == p_req) {
					if (peer_req->block_id != ID_SYNCER) {
						...
						bsr_info(BSR_LC_TEMP, device, "destroy, active_ee => inactive_ee(%p), sector(%llu), size(%u)\n", peer_req, (unsigned long long)peer_req->i.sector, peer_req->i.size);
					}
					else {
						...
						bsr_info(BSR_LC_TEMP, device, "destroy, sync_ee => inactive_ee(%p), sector(%llu), size(%u)\n", peer_req, (unsigned long long)peer_req->i.sector, peer_req->i.size);
					}
...
#endif
{
	...
	//bsr_debug(BSR_LC_TEMP, NO_OBJECT,"BIO_ENDIO_FN_START:Thread(%s) bsr_peer_request_endio: IRQL(%d) ..............\n",  current->comm, KeGetCurrentIrql());
	...
	if ((ULONG_PTR)DeviceObject != FAULT_TEST_FLAG) {
		...
		if(gSimulDiskIoError.ErrorFlag && gSimulDiskIoError.ErrorType == SIMUL_DISK_IO_ERROR_TYPE2) {
			if(IsDiskError()) {
				bsr_err(BSR_LC_TEMP, NO_OBJECT,"SimulDiskIoError: Peer Request I/O Error type2.....ErrorFlag:%d ErrorCount:%d\n", gSimulDiskIoError.ErrorFlag, gSimulDiskIoError.ErrorCount);
				error = STATUS_UNSUCCESSFUL;
			}
		}
	} else {
	...
	//bsr_debug(BSR_LC_TEMP, NO_OBJECT,"bsr_peer_request_endio done.(%d).............!!!\n", peer_request_endio_cnt++);
...
{
#ifdef _WIN
	bsr_err(BSR_LC_TEMP, NO_OBJECT,"bsr%u %s / %u", device->minor, device->resource->name, device->vnr);
...
#endif
{
	...
	//bsr_debug(BSR_LC_TEMP, NO_OBJECT,"BIO_ENDIO_FN_START:Thread(%s) bsr_request_endio: IRQL(%d) ................\n", current->comm, KeGetCurrentIrql());
	...
	if ((ULONG_PTR)DeviceObject != FAULT_TEST_FLAG) {
		...
		if(gSimulDiskIoError.ErrorFlag && gSimulDiskIoError.ErrorType == SIMUL_DISK_IO_ERROR_TYPE1) {
			if(IsDiskError()) {
				bsr_err(BSR_LC_TEMP, NO_OBJECT,"SimulDiskIoError: Local I/O Error type1.....ErrorFlag:%d ErrorCount:%d\n",gSimulDiskIoError.ErrorFlag,gSimulDiskIoError.ErrorCount);
				error = STATUS_UNSUCCESSFUL;
			}
		}
	} else {
	...
	*/
	...
	{
		static int cnt = 0;
		bsr_debug(BSR_LC_TEMP, NO_OBJECT,"bsr_request_endio done.(%d).................IRQL(%d)!!!\n", cnt++, KeGetCurrentIrql());
	}
#endif
#endif
...
#ifdef _WIN
#ifdef BSR_TRACE	
	bsr_debug(BSR_LC_TEMP, NO_OBJECT,"(%s) bsr_request_endio: before __req_mod! IRQL(%d) \n", current->comm, KeGetCurrentIrql());
#endif
#endif
...
static int w_e_send_csum(struct bsr_work *w, int cancel)
{
	...
	if (digest) {
		...
		bsr_err(BSR_LC_TEMP, peer_device, "kmalloc() of digest failed.\n");
		err = -ENOMEM;
	}
...
out:
	...
	if (unlikely(err))
		bsr_err(BSR_LC_TEMP, peer_device, "bsr_send_drequest(..., csum) failed\n");
	return err;
}
...
static int read_for_csum(struct bsr_peer_device *peer_device, sector_t sector, int size)
{
	...
	if (!peer_req) {
		bsr_err(BSR_LC_TEMP, peer_device, "failed to allocate peer request\n");
		goto defer;
	}
	...
	if (size) {
		...
		if (!peer_req->page_chain.head) {
			bsr_err(BSR_LC_TEMP, peer_device, "failed to allocate page chain\n");
			goto defer2;
		}
#ifdef _WIN
	...
	bsr_err(BSR_LC_TEMP, peer_device, "failed to submit peer request\n");
...
int w_resync_timer(struct bsr_work *w, int cancel)
{
	...
	default:
		// DW-1977
		bsr_info(BSR_LC_TEMP, peer_device, "completed because it is not in the VERIFY_S or SYNC_TARGET replication state.\n");
		break;
	}
...
static int make_resync_request(struct bsr_peer_device *peer_device, int cancel)
{
...
#ifdef _WIN
	bsr_debug_tm("timer callback jiffies(%llu)\n", jiffies);
#endif

	if (unlikely(cancel)) {
		bsr_info(BSR_LC_TEMP, peer_device, "resync cacnel.\n");
		return 0;
	}

	if (peer_device->rs_total == 0) {
		/* empty resync? */
		bsr_info(BSR_LC_TEMP, peer_device, "finished because it's rs_total empty\n");
	...
	if (!get_ldev(device)) {
		/* Since we only need to access device->rsync a
		   ...
		   all */
		bsr_err(BSR_LC_TEMP, device, "Disk broke down during resync!\n");
		return 0;
	}
...
#ifdef _WIN
			bsr_debug_tr("make_resync_request: %d/%d: queued=%lld sndbuf=%lld\n", i, number, queued, sndbuf);
#endif
			if (queued > sndbuf / 2) {
				requeue = 1;
...
next_sector:
		...
		for (;;) {
			...
			if (bit >= bsr_bm_bits(device)) {
				...
				bsr_info(BSR_LC_TEMP, peer_device, "BSR_END_OF_BITMAP(%llu), device->bm_resync_fo : %llu, bm_set : %llu\n", 
		...
		if (peer_device->use_csums) {
			...
			if (err) {
				bsr_err(BSR_LC_TEMP, peer_device, "bsr_send_drequest() failed, aborting...\n");
	...
	if (device->bm_resync_fo >= bsr_bm_bits(device)) {
		 ...
		 */
		bsr_info(BSR_LC_TEMP, peer_device, "P_RS_DATA_REPLY not received??,  device->bm_resync_fo : %llu, bm_set : %llu\n", (unsigned long long)device->bm_resync_fo, (unsigned long long)bsr_bm_total_weight(peer_device));
		put_ldev(device);
		return 0;
	}
...
static int make_ov_request(struct bsr_peer_device *peer_device, int cancel)
{
	...
	for (i = 0; i < number; i++) {
		...
		for (;;) {
			...
			if (bit >= bsr_bm_bits(device)) {
				bsr_info(BSR_LC_TEMP, peer_device, "BSR_END_OF_OV_BITMAP(%llu), peer_device->ov_bm_position : %llu\n", 
		...
		if (peer_device->repl_state[NEW] == L_PAUSED_SYNC_T) {
			...
			if (peer_md[peer_node_id].bitmap_index != -1
			 && !bsr_md_test_peer_flag(peer_device, MDF_PEER_PRIMARY_IO_ERROR)) {
				bsr_info(BSR_LC_TEMP, peer_device, "bitmap will be cleared due to resync cancelation\n");
			 ...
			 enum bsr_disk_state new_peer_disk_state)
{
	...
	queue_on_sender_workq:
		...
		bsr_err(BSR_LC_TEMP, peer_device, "Warn failed to kmalloc(dw).\n");
	}
	...
	if (repl_state[NOW] == L_AHEAD) {
		bsr_info(BSR_LC_TEMP, peer_device, "I am ahead, do not finish resync.\n"); // DW-1518
		put_ldev(device);
		spin_unlock_irq(&device->resource->req_lock);	
		return 1;
	}
...
#ifdef SPLIT_REQUEST_RESYNC
	bsr_info(BSR_LC_TEMP, peer_device, "%s done%s (total %llu sec; paused %llu sec; %llu K/sec), hit bit (in sync %llu; marked rl %llu)\n",
...
#else // _LIN
	bsr_info(BSR_LC_TEMP, peer_device, "%s done (total %llu sec; paused %llu sec; %llu K/sec)\n",
	...
	if (repl_state[NOW] == L_VERIFY_S || repl_state[NOW] == L_VERIFY_T) {
		...
		if (peer_device->use_csums && peer_device->rs_total) {
			...
			bsr_info(BSR_LC_TEMP, peer_device, "%llu %% had equal checksums, eliminated: %lluK; "
	...
	if (peer_device->rs_failed) {
		bsr_info(BSR_LC_TEMP, peer_device, "            %llu failed blocks (out of sync :%llu)\n", (unsigned long long)peer_device->rs_failed, (unsigned long long)n_oos);
		...
		if (repl_state[NOW] == L_SYNC_TARGET || repl_state[NOW] == L_PAUSED_SYNC_T) {
			...
			if (stable_resync &&
				...
				if (!peer_device->uuids_received)
					bsr_err(BSR_LC_TEMP, peer_device, "BUG: uuids were not received!\n");

				if (test_bit(UNSTABLE_RESYNC, &peer_device->flags))
					bsr_info(BSR_LC_TEMP, peer_device, "Peer was unstable during resync\n");
			}
...
int w_e_end_data_req(struct bsr_work *w, int cancel)
{
	...
	if (likely((peer_req->flags & EE_WAS_ERROR) == 0)) {
		...
		if (bsr_ratelimit())
			bsr_err(BSR_LC_TEMP, peer_device, "Sending NegDReply. sector=%llus.\n",
	...
	if (unlikely(err))
		bsr_err(BSR_LC_TEMP, peer_device, "bsr_send_block() failed\n");
	return err;
}
	...
	if (peer_device->repl_state[NOW] == L_AHEAD) {
		...
		if (!is_sync_source(peer_device) && device->resource->role[NOW] != R_PRIMARY) {
			...
			if (likely(peer_device->disk_state[NOW] >= D_INCONSISTENT)) {
				...
				if (bsr_ratelimit())
					bsr_err(BSR_LC_TEMP, peer_device, "Not sending RSDataReply, "
					"partner DISKLESS!\n");
				err = 0;
			}
		}
		...
		if (bsr_ratelimit())
			bsr_err(BSR_LC_TEMP, peer_device, "Sending NegRSDReply. sector %llus.\n",
			    (unsigned long long)peer_req->i.sector);
...
int w_e_end_csum_rs_req(struct bsr_work *w, int cancel)
{
	...
	if (unlikely(cancel)) {
		bsr_info(BSR_LC_TEMP, peer_device, "cancel csum rs req, sector : %llu\n", (unsigned long long)peer_req->i.sector);
		bsr_free_peer_req(peer_req);
		dec_unacked(peer_device);
		return 0;
	}
	...
	if (likely((peer_req->flags & EE_WAS_ERROR) == 0)) {
		...
		err = bsr_send_ack(peer_device, P_NEG_RS_DREPLY, peer_req);
		if (bsr_ratelimit())
			bsr_err(BSR_LC_TEMP, device, "Sending NegRSDReply. I guess it gets messy.\n");
	...
	if (unlikely(err))
		bsr_err(BSR_LC_TEMP, device, "bsr_send_block/ack() failed\n");
	return err;
}
...
static bool __bsr_may_sync_now(struct bsr_peer_device *peer_device)
{
	...
	while (1) {
		...
		if ((other_peer_device->repl_state[NOW] >= L_SYNC_SOURCE &&
			...
			bsr_info(BSR_LC_TEMP, peer_device, "another(node_id:%d) peer device is in progress for resync\n", other_peer_device->node_id);
			ret = false;
			break;
		}
	}
...
#endif
{
	...
	bsr_info(BSR_LC_TEMP, peer_device, "post RS_START to the peer_device work\n"); // DW-1518
...
static void do_start_resync(struct bsr_peer_device *peer_device)
{
	...
	if (test_bit(AHEAD_TO_SYNC_SOURCE, &peer_device->flags) && atomic_read(&peer_device->rq_pending_oos_cnt)) {
		bsr_debug(BSR_LC_TEMP, peer_device, "postponing start_resync ... pending oos : %d\n", atomic_read(&peer_device->rq_pending_oos_cnt)); 
		retry_resync = true;
	}
	...
	bsr_info(BSR_LC_TEMP, peer_device, "starting resync ...\n"); // DW-1518
...
bool bsr_inspect_resync_side(struct bsr_peer_device *peer_device, enum bsr_repl_state replState, enum which_state which, bool locked)
{
	...
	if (!peer_device->uuids_received) {
		bsr_info(BSR_LC_TEMP, peer_device, "I have not yet received uuid from peer, can not be %s\n", bsr_repl_str(replState));
		return false;
	}
	...
	switch (replState) {
		...
		case L_VERIFY_T:
			...
			break;
		default:
			bsr_info(BSR_LC_TEMP, peer_device, "unexpected repl_state (%s)\n", bsr_repl_str(replState));
			return false;
	}
	
	if (side == L_SYNC_TARGET || side == L_VERIFY_T) {
		if (!(peer_device->uuid_flags & UUID_FLAG_STABLE)) {
			bsr_info(BSR_LC_TEMP, peer_device, "Sync source is unstable, can not be %s, uuid_flags(%llx), authoritative(%llx)\n",
		...
		if (!bsr_device_stable_ex(device, &authoritative, which, locked) &&
			...
			bsr_info(BSR_LC_TEMP, peer_device, "I am unstable and sync source is not my authoritative node, can not be %s, authoritative(%llx)\n",
		...
		if (!bsr_device_stable_ex(device, &authoritative, which, locked)) {
			bsr_info(BSR_LC_TEMP, peer_device, "I am unstable, can not be %s, authoritative(%llx)\n", bsr_repl_str(replState), authoritative);
			return false;
		}

		if (!(peer_device->uuid_flags & UUID_FLAG_STABLE) &&
			!(NODE_MASK(device->resource->res_opts.node_id) & peer_device->uuid_authoritative_nodes)) {
			bsr_info(BSR_LC_TEMP, peer_device, "Sync target is unstable and I am not its authoritative node, can not be %s, uuid_flags(%llx), authoritative(%llx)\n",
				bsr_repl_str(replState), peer_device->uuid_flags, peer_device->uuid_authoritative_nodes);
			return false;			
		}
	}
...
void bsr_start_resync(struct bsr_peer_device *peer_device, enum bsr_repl_state side)
{
	...
	if (repl_state < L_ESTABLISHED) {
		...
		bsr_err(BSR_LC_TEMP, peer_device, "Unable to start resync since it is not connected\n"); // DW-1518
		return;
	}
	if (repl_state >= L_SYNC_SOURCE && repl_state < L_AHEAD) {
		bsr_err(BSR_LC_TEMP, peer_device, "Resync already running!\n");
		return;
	}
	...
	if (!test_bit(B_RS_H_DONE, &peer_device->flags)) {
		if (side == L_SYNC_TARGET) {
...
#endif
			if (r > 0) {
				bsr_info(BSR_LC_TEMP, device, "before-resync-target handler returned %d, "
					 "dropping connection.\n", r);
			...
			if (r > 0) {
				if (r == 3) {
					bsr_info(BSR_LC_TEMP, device, "before-resync-source handler returned %d, "
	...
	if (down_trylock(&device->resource->state_sem)) {
		...
		bsr_info(BSR_LC_TEMP, peer_device, "Retry later\n"); // DW-1518
	...
	if (r == SS_SUCCESS) {
		...
		bsr_info(BSR_LC_TEMP, peer_device, "Began resync as %s (will sync %llu KB [%llu bits set]).\n",
		...
		bsr_err(BSR_LC_TEMP, peer_device, "Unable to start resync as %s (err = %d)\n", bsr_repl_str(repl_state), r); // DW-1518
	}
		...
		if (s & RQ_EXP_BARR_ACK) {
			...
			if (peer_device->connection->agreed_pro_version >= 113) {
				if (atomic_read(&peer_device->rq_pending_oos_cnt) == 0 &&
					...
					bsr_info(BSR_LC_TEMP, peer_device, "start resync again because there is out of sync(%llu) in L_ESTABLISHED state\n", (unsigned long long)bsr_bm_total_weight(peer_device)); 
