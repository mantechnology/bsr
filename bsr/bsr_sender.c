...
#endif
{
	...
	bsr_debug(NO_OBJECT,"BIO_ENDIO_FN_START:Thread(%s) bsr_md_io_complete IRQL(%d) .............", current->comm, KeGetCurrentIrql());
	...
	if ((ULONG_PTR)DeviceObject != FAULT_TEST_FLAG) {
		...
		if(gSimulDiskIoError.ErrorFlag && gSimulDiskIoError.ErrorType == SIMUL_DISK_IO_ERROR_TYPE3) {
			if(IsDiskError()) {
				bsr_err(NO_OBJECT,"SimulDiskIoError: Meta Data I/O Error type3.....ErrorFlag:%d ErrorCount:%d", gSimulDiskIoError.ErrorFlag, gSimulDiskIoError.ErrorCount);
...
static void bsr_endio_read_sec_final(struct bsr_peer_request *peer_req) __releases(local)
{
	...
	if (test_bit(__EE_WAS_INACTIVE_REQ, &peer_req->flags)) {
		if (test_bit(__EE_WAS_LOST_REQ, &peer_req->flags)) {
			bsr_info(NO_OBJECT, "destroy, read lost inactive_ee(%p), sector(%llu), size(%u)", peer_req, (unsigned long long)peer_req->i.sector, peer_req->i.size);
			...
			list_for_each_entry_safe_ex(struct bsr_peer_request, p_req, t_inative, &connection->inactive_ee, w.list) {
				if (peer_req == p_req) {
					bsr_info(device, "destroy, read inactive_ee(%p), sector(%llu), size(%u)", peer_req, (unsigned long long)peer_req->i.sector, peer_req->i.size);
...
void bsr_endio_write_sec_final(struct bsr_peer_request *peer_req) __releases(local)
{
	...
	if (test_bit(__EE_WAS_INACTIVE_REQ, &peer_req->flags)) {
		if (test_bit(__EE_WAS_LOST_REQ, &peer_req->flags)) {
			bsr_info(NO_OBJECT, "destroy, wrtie inactive_ee(%p), sector(%llu), size(%u)", peer_req, (unsigned long long)peer_req->i.sector, peer_req->i.size);
			bsr_free_peer_req(peer_req); 
		}
		else {
			struct bsr_peer_request *p_req, *t_inative;
			...
			list_for_each_entry_safe_ex(struct bsr_peer_request, p_req, t_inative, &connection->inactive_ee, w.list) {
				if (peer_req == p_req) {
					if (peer_req->block_id != ID_SYNCER) {
						...
						bsr_info(device, "destroy, active_ee => inactive_ee(%p), sector(%llu), size(%u)", peer_req, (unsigned long long)peer_req->i.sector, peer_req->i.size);
					}
					else {
						...
						bsr_info(device, "destroy, sync_ee => inactive_ee(%p), sector(%llu), size(%u)", peer_req, (unsigned long long)peer_req->i.sector, peer_req->i.size);
					}
...
#endif
{
	...
	//bsr_debug(NO_OBJECT,"BIO_ENDIO_FN_START:Thread(%s) bsr_peer_request_endio: IRQL(%d) ..............",  current->comm, KeGetCurrentIrql());
	...
	if ((ULONG_PTR)DeviceObject != FAULT_TEST_FLAG) {
		...
		if(gSimulDiskIoError.ErrorFlag && gSimulDiskIoError.ErrorType == SIMUL_DISK_IO_ERROR_TYPE2) {
			if(IsDiskError()) {
				bsr_err(NO_OBJECT,"SimulDiskIoError: Peer Request I/O Error type2.....ErrorFlag:%d ErrorCount:%d", gSimulDiskIoError.ErrorFlag, gSimulDiskIoError.ErrorCount);
				error = STATUS_UNSUCCESSFUL;
			}
		}
	} else {
	...
	//bsr_debug(NO_OBJECT,"bsr_peer_request_endio done.(%d).............!!!", peer_request_endio_cnt++);
...
{
#ifdef _WIN
	bsr_err(NO_OBJECT,"bsr%u %s / %u", device->minor, device->resource->name, device->vnr);
...
#endif
{
	...
	//bsr_debug(NO_OBJECT,"BIO_ENDIO_FN_START:Thread(%s) bsr_request_endio: IRQL(%d) ................", current->comm, KeGetCurrentIrql());
	...
	if ((ULONG_PTR)DeviceObject != FAULT_TEST_FLAG) {
		...
		if(gSimulDiskIoError.ErrorFlag && gSimulDiskIoError.ErrorType == SIMUL_DISK_IO_ERROR_TYPE1) {
			if(IsDiskError()) {
				bsr_err(NO_OBJECT,"SimulDiskIoError: Local I/O Error type1.....ErrorFlag:%d ErrorCount:%d",gSimulDiskIoError.ErrorFlag,gSimulDiskIoError.ErrorCount);
				error = STATUS_UNSUCCESSFUL;
			}
		}
	} else {
	...
	*/
	...
	{
		static int cnt = 0;
		bsr_debug(NO_OBJECT,"bsr_request_endio done.(%d).................IRQL(%d)!!!", cnt++, KeGetCurrentIrql());
	}
#endif
#endif
...
#ifdef _WIN
#ifdef BSR_TRACE	
	bsr_debug(NO_OBJECT,"(%s) bsr_request_endio: before __req_mod! IRQL(%d) ", current->comm, KeGetCurrentIrql());
#endif
#endif
...
static int w_e_send_csum(struct bsr_work *w, int cancel)
{
	...
	if (digest) {
		...
		bsr_err(peer_device, "kmalloc() of digest failed.");
		err = -ENOMEM;
	}
...
out:
	...
	if (unlikely(err))
		bsr_err(peer_device, "bsr_send_drequest(..., csum) failed");
	return err;
}
...
static int read_for_csum(struct bsr_peer_device *peer_device, sector_t sector, int size)
{
	...
	if (!peer_req) {
		bsr_err(peer_device, "failed to allocate peer request");
		goto defer;
	}
	...
	if (size) {
		...
		if (!peer_req->page_chain.head) {
			bsr_err(peer_device, "failed to allocate page chain");
			goto defer2;
		}
#ifdef _WIN
	...
	bsr_err(peer_device, "failed to submit peer request");
...
int w_resync_timer(struct bsr_work *w, int cancel)
{
	...
	case L_SYNC_TARGET:
		...
		if (mutex_trylock(&device->resource->vol_ctl_mutex)) {
			...
			if (ts > ((3 * 1000) * HZ)) {
				bsr_warn(peer_device, "resync request takes a long time(%lldus)", ts);
			}
		}
		else		
	...
	default:
		// DW-1977
		bsr_info(peer_device, "completed because it is not in the VERIFY_S or SYNC_TARGET replication state.");
		break;
	}
...
#endif
	/*
	bsr_warn(device, "si=%u if=%d wa=%u co=%d st=%d cps=%d pl=%d cc=%d rs=%d",
...
static int make_resync_request(struct bsr_peer_device *peer_device, int cancel)
{
	...
	if (unlikely(cancel)) {
		bsr_info(peer_device, "resync cacnel.");
		return 0;
	}

	if (peer_device->rs_total == 0) {
		/* empty resync? */
		bsr_info(peer_device, "finished because it's rs_total empty");
	...
	if (!get_ldev(device)) {
		/* Since we only need to access device->rsync a
		   ...
		   all */
		bsr_err(device, "Disk broke down during resync!");
		return 0;
	}
...
next_sector:
		...
		for (;;) {
			...
			if (bit >= bsr_bm_bits(device)) {
				...
				bsr_info(peer_device, "BSR_END_OF_BITMAP(%llu), device->bm_resync_fo : %llu, bm_set : %llu", 
		...
		if (peer_device->use_csums) {
			...
			if (err) {
				bsr_err(peer_device, "bsr_send_drequest() failed, aborting...");
	...
	if (device->bm_resync_fo >= bsr_bm_bits(device)) {
		 ...
		 */
		bsr_info(peer_device, "P_RS_DATA_REPLY not received??,  device->bm_resync_fo : %llu, bm_set : %llu", (unsigned long long)device->bm_resync_fo, (unsigned long long)bsr_bm_total_weight(peer_device));
		put_ldev(device);
		return 0;
	}
		...
		if (peer_device->repl_state[NEW] == L_PAUSED_SYNC_T) {
			...
			if (peer_md[peer_node_id].bitmap_index != -1
			 && !bsr_md_test_peer_flag(peer_device, MDF_PEER_PRIMARY_IO_ERROR)) {
				bsr_info(peer_device, "bitmap will be cleared due to resync cancelation");
			 ...
			 enum bsr_disk_state new_peer_disk_state)
{
	...
	queue_on_sender_workq:
		...
		bsr_err(peer_device, "Warn failed to kmalloc(dw).");
	}
	...
	if (repl_state[NOW] == L_AHEAD) {
		bsr_info(peer_device, "I am ahead, do not finish resync."); // DW-1518
		put_ldev(device);
		spin_unlock_irq(&device->resource->req_lock);	
		return 1;
	}
	...
	if (repl_state[NOW] == L_VERIFY_S || repl_state[NOW] == L_VERIFY_T) {
		...
		if (peer_device->use_csums && peer_device->rs_total) {
			...
			bsr_info(peer_device, "%llu %% had equal checksums, eliminated: %lluK; "
	...
	if (peer_device->rs_failed) {
		bsr_info(peer_device, "            %llu failed blocks (out of sync :%llu)", (unsigned long long)peer_device->rs_failed, (unsigned long long)n_oos);
		...
		if (repl_state[NOW] == L_SYNC_TARGET || repl_state[NOW] == L_PAUSED_SYNC_T) {
			...
			if (stable_resync &&
				...
				if (!peer_device->uuids_received)
					bsr_err(peer_device, "BUG: uuids were not received!");

				if (test_bit(UNSTABLE_RESYNC, &peer_device->flags))
					bsr_info(peer_device, "Peer was unstable during resync");
			}
...
int w_e_end_data_req(struct bsr_work *w, int cancel)
{
	...
	if (likely((peer_req->flags & EE_WAS_ERROR) == 0)) {
		...
		if (bsr_ratelimit())
			bsr_err(peer_device, "Sending NegDReply. sector=%llus.",
	...
	if (unlikely(err))
		bsr_err(peer_device, "bsr_send_block() failed");
	return err;
}
	...
	if (peer_device->repl_state[NOW] == L_AHEAD) {
		...
		if (!is_sync_source(peer_device) && device->resource->role[NOW] != R_PRIMARY) {
			...
			if (likely(peer_device->disk_state[NOW] >= D_INCONSISTENT)) {
				...
				if (bsr_ratelimit())
					bsr_err(peer_device, "Not sending RSDataReply, "
					"partner DISKLESS!");
				err = 0;
			}
		}
		...
		if (bsr_ratelimit())
			bsr_err(peer_device, "Sending NegRSDReply. sector %llus.",
			    (unsigned long long)peer_req->i.sector);
...
int w_e_end_csum_rs_req(struct bsr_work *w, int cancel)
{
	...
	if (unlikely(cancel)) {
		bsr_info(peer_device, "cancel csum rs req, sector : %llu", (unsigned long long)peer_req->i.sector);
		bsr_free_peer_req(peer_req);
		dec_unacked(peer_device);
		return 0;
	}
	...
	if (unlikely(err))
		bsr_err(device, "bsr_send_block/ack() failed");
	return err;
}
...
static bool __bsr_may_sync_now(struct bsr_peer_device *peer_device)
{
	...
	while (1) {
		...
		if ((other_peer_device->repl_state[NOW] >= L_SYNC_SOURCE &&
			...
			bsr_info(peer_device, "another(node_id:%d) peer device is in progress for resync", other_peer_device->node_id);
			ret = false;
			break;
		}
	}
...
#endif
{
	...
	bsr_info(peer_device, "post RS_START to the peer_device work"); // DW-1518
...
static void do_start_resync(struct bsr_peer_device *peer_device)
{
	...
	if (atomic_read(&peer_device->unacked_cnt) || 
		...
		bsr_warn(peer_device, "postponing start_resync ... unacked : %d, pending : %d", atomic_read(&peer_device->unacked_cnt), atomic_read(&peer_device->rs_pending_cnt));
		retry_resync = true;
	}

	// DW-2076
	if (test_bit(AHEAD_TO_SYNC_SOURCE, &peer_device->flags) && atomic_read(&peer_device->rq_pending_oos_cnt)) {
		bsr_debug(peer_device, "postponing start_resync ... pending oos : %d", atomic_read(&peer_device->rq_pending_oos_cnt)); 
		retry_resync = true;
	}
	...
	bsr_info(peer_device, "starting resync ..."); // DW-1518
...
bool bsr_inspect_resync_side(struct bsr_peer_device *peer_device, enum bsr_repl_state replState, enum which_state which, bool locked)
{
	...
	if (!peer_device->uuids_received) {
		bsr_info(peer_device, "I have not yet received uuid from peer, can not be %s", bsr_repl_str(replState));
		return false;
	}
	...
	switch (replState) {
		...
		case L_VERIFY_T:
			...
			break;
		default:
			bsr_info(peer_device, "unexpected repl_state (%s)", bsr_repl_str(replState));
			return false;
	}
	...
	if (side == L_SYNC_TARGET || side == L_VERIFY_T) {
		...
		if (!bsr_device_stable_ex(device, &authoritative, which, locked) &&
			...
			bsr_info(peer_device, "I am unstable and sync source is not my authoritative node, can not be %s, authoritative(%llx)",
		...
		if (!bsr_device_stable_ex(device, &authoritative, which, locked)) {
			bsr_info(peer_device, "I am unstable, can not be %s, authoritative(%llx)", bsr_repl_str(replState), authoritative);
			return false;
		}

		if (!(peer_device->uuid_flags & UUID_FLAG_STABLE) &&
			!(NODE_MASK(device->resource->res_opts.node_id) & peer_device->uuid_authoritative_nodes)) {
			bsr_info(peer_device, "Sync target is unstable and I am not its authoritative node, can not be %s, uuid_flags(%llx), authoritative(%llx)",
				bsr_repl_str(replState), peer_device->uuid_flags, peer_device->uuid_authoritative_nodes);
			return false;			
		}
	}
...
void bsr_start_resync(struct bsr_peer_device *peer_device, enum bsr_repl_state side)
{
	...
	if (repl_state < L_ESTABLISHED) {
		...
		bsr_err(peer_device, "Unable to start resync since it is not connected"); // DW-1518
		return;
	}
	if (repl_state >= L_SYNC_SOURCE && repl_state < L_AHEAD) {
		bsr_err(peer_device, "Resync already running!");
		return;
	}
	...
	if (!test_bit(B_RS_H_DONE, &peer_device->flags)) {
		if (side == L_SYNC_TARGET) {
...
#endif
			if (r > 0) {
				bsr_info(device, "before-resync-target handler returned %d, "
			...
			if (r > 0) {
				if (r == 3) {
					bsr_info(device, "before-resync-source handler returned %d, "
	...
	if (down_trylock(&device->resource->state_sem)) {
		...
		bsr_info(peer_device, "Retry later"); // DW-1518
	...
	if (r == SS_SUCCESS) {
		...
		bsr_info(peer_device, "Began resync as %s (will sync %llu KB [%llu bits set]).",
		...
		bsr_err(peer_device, "Unable to start resync as %s (err = %d)", bsr_repl_str(repl_state), r); // DW-1518
	}
...
static void do_peer_device_work(struct bsr_peer_device *peer_device, const ULONG_PTR todo)
{
		...
		if (s & RQ_EXP_BARR_ACK) {
			...
			if (peer_device->connection->agreed_pro_version >= 113) {
				if (atomic_read(&peer_device->rq_pending_oos_cnt) == 0 &&
					...
					bsr_info(peer_device, "start resync again because there is out of sync(%llu) in L_ESTABLISHED state", (unsigned long long)bsr_bm_total_weight(peer_device)); 
		...
		while (!list_empty(&work_list)) {
			...
			if (w->cb != NULL) {
				...
				if (is_null_callback_print == false) {
					...
					bsr_warn(resource, "worker got an null-callback list. resource name (%s), twopc_work(%p) : w(%p)", resource->name, &(resource->twopc_work), w);
