...
bool alloc_bab(struct bsr_connection* connection, struct net_conf* nconf) 
{
	...
	do {
		if(nconf->sndbuf_size < BSR_SNDBUF_SIZE_MIN ) {
			bsr_info(NO_OBJECT,"alloc bab fail nconf->sndbuf_size < BSR_SNDBUF_SIZE_MIN connection->peer_node_id:%u nconf->sndbuf_size:%llu", connection->peer_node_id, nconf->sndbuf_size);
			goto $ALLOC_FAIL;
		}
#ifdef _WIN_SEND_BUF
		__try {
...
#endif
			if(!ring) {
				bsr_info(NO_OBJECT,"alloc data bab fail connection->peer_node_id:%u nconf->sndbuf_size:%llu", connection->peer_node_id, nconf->sndbuf_size);
				goto $ALLOC_FAIL;
			}
			...
			bsr_info(NO_OBJECT,"EXCEPTION_EXECUTE_HANDLER alloc data bab fail connection->peer_node_id:%u nconf->sndbuf_size:%llu", connection->peer_node_id, nconf->sndbuf_size);
			if(ring) {
				ExFreePool(ring);
			}
			goto $ALLOC_FAIL;
		...
		__try {
...
#endif
			if(!ring) {
				bsr_info(NO_OBJECT,"alloc meta bab fail connection->peer_node_id:%u nconf->sndbuf_size:%llu", connection->peer_node_id, nconf->sndbuf_size);
			...
			bsr_info(NO_OBJECT,"EXCEPTION_EXECUTE_HANDLER alloc meta bab fail connection->peer_node_id:%u nconf->sndbuf_size:%llu", connection->peer_node_id, nconf->sndbuf_size);
			if(ring) {
				ExFreePool(ring);
			}
			goto $ALLOC_FAIL;
	...
	bsr_info(NO_OBJECT,"alloc_bab ok connection->peer_node_id:%d nconf->sndbuf_size:%lld", connection->peer_node_id, nconf->sndbuf_size);
	return TRUE;
...
ring_buffer *create_ring_buffer(struct bsr_connection* connection, char *name, signed long long length, enum bsr_stream stream)
{
	...
	if (length == 0 || length > BSR_SNDBUF_SIZE_MAX) {
		bsr_err(NO_OBJECT,"bab(%s) size(%lld) is bad. max(%ld)", name, length, BSR_SNDBUF_SIZE_MAX);
		return NULL;
	}
	...
	if (ring) {
		...
		//bsr_info(NO_OBJECT,"bab(%s) size(%d)", name, length);
#ifdef SENDBUF_TRACE
		...
		if (!ring->static_big_buf) {
			//ExFreePool(ring);
			//kfree2(ring);
			bsr_err(NO_OBJECT,"bab(%s): static_big_buf alloc(%d) failed.  ", name, MAX_ONETIME_SEND_BUF);
			return NULL;
		}
	} else {
		bsr_err(NO_OBJECT,"bab(%s):alloc(%lld) failed", name, sz);
	}
	return ring;
}
...
int do_send(struct socket *socket, struct ring_buffer *bab, int timeout, KEVENT *send_buf_kill_event)
{
	...
	while (true) {
		...
		if (ret != tx_sz) {
			if (ret < 0) {
				if (ret != -EINTR) {
					bsr_info(NO_OBJECT,"Send Error(%d)", ret);
					ret = 0;
				}
				break;
			} else {
				bsr_info(NO_OBJECT,"Tx mismatch. req(%d) sent(%d)", tx_sz, ret);
...
#define MAX_EVT		2
	...
	while (true) {
		...
		default:
			bsr_err(NO_OBJECT,"unexpected wakeup case(0x%x). ignore.", status);
			goto done;
		}
	}
	...
	bsr_info(NO_OBJECT,"sendbuf thread[%p] terminate!!",KeGetCurrentThread());
