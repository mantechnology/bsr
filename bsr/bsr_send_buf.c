...
ring_buffer *create_ring_buffer(struct bsr_connection* connection, char *name, signed long long length, enum bsr_stream stream)
{
	...
	if (length == 0 || length > BSR_SNDBUF_SIZE_MAX) {
		bsr_err(9, BSR_LC_SEND_BUFFER, NO_OBJECT, "Failed to create incorrect size. name(%s), max(%ld), size(%lld)\n", name, length, BSR_SNDBUF_SIZE_MAX);
		return NULL;
	}
	...
	if (ring) {
		...
		//bsr_info(10, BSR_LC_SEND_BUFFER, NO_OBJECT,"bab(%s) size(%d)\n", name, length);
#ifdef SENDBUF_TRACE
		...
		if (!ring->static_big_buf) {
			//ExFreePool(ring);
			//kfree2(ring);
			bsr_err(11, BSR_LC_SEND_BUFFER, NO_OBJECT, "Failed to allocate memory for static big buffer. name(%s), size(%d)\n", name, MAX_ONETIME_SEND_BUF);
			return NULL;
		}
	} else {
		bsr_err(12, BSR_LC_SEND_BUFFER, NO_OBJECT, "Failed to get stream information. name(%s) size(%lld)\n", name, sz);
	}
	return ring;
}
...
int do_send(struct socket *socket, struct ring_buffer *bab, int timeout, KEVENT *send_buf_kill_event)
{
	...
	while (true) {
		...
		if (ret != tx_sz) {
			if (ret < 0) {
				if (ret != -EINTR) {
					bsr_info(17, BSR_LC_SEND_BUFFER, NO_OBJECT, "Send Error(%d)\n", ret);
					ret = 0;
				}
				break;
			} else {
...
#define MAX_EVT		2
	...
	while (true) {
		...
		default:
			bsr_err(21, BSR_LC_SEND_BUFFER, NO_OBJECT, "unexpected wakeup case(0x%x). ignore.\n", status);
			goto done;
		}
	}
	...
	bsr_info(23, BSR_LC_SEND_BUFFER, NO_OBJECT, "sendbuf thread[%p] terminate!!\n", KeGetCurrentThread());
