...
void lc_printf_stats(struct lru_cache *lc, struct lc_element *e){
	...
	if (lc){
		...
		bsr_crit(16, BSR_LC_LRU, NO_OBJECT,"lru : %s", print_lru);
	}
	...
	if (e){
		...
		bsr_crit(17, BSR_LC_LRU, NO_OBJECT, "element : %s", print_ele);
	}
}
#endif 
		...
		bool include_changing)
{
	...
	if (!lc ||
		!lc->nr_elements)
	{
		bsr_err(18, BSR_LC_BITMAP, NO_OBJECT,"inaccessible, it could be not initialized or destroyed.");
		return NULL;
	}
#else // _LIN
	BUG_ON(!lc);
