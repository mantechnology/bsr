...
int dtt_init(struct bsr_transport *transport)
{
	...
	for (i = DATA_STREAM; i <= CONTROL_STREAM ; i++) {
		...
		if (!buffer) {
			...
			bsr_warn(0, BSR_LC_SOCKET, NO_OBJECT,"dtt_init kzalloc %s allocation fail\n", i ? "CONTROL_STREAM" : "DATA_STREAM" );
			goto fail;
		}
#else  // _LIN
...
static int dtt_recv_pages(struct bsr_transport *transport, struct bsr_page_chain_head *chain, size_t size)
{
	...
	if (err < 0) {
		...
		bsr_err(0, BSR_LC_TEMP, NO_OBJECT,"Wrong data (expected size:%d, received size:%d)\n", (int)size, err);
		err = -EIO;		
...
static int dtt_try_connect(struct bsr_transport *transport, struct dtt_path *path, struct socket **ret_socket)
{
	...
	if (!NT_SUCCESS(status)) {
		...
		default: 
			bsr_err(0, BSR_LC_TEMP, NO_OBJECT,"create-connect failed with status 0x%08X \n", status);
			err = -EINVAL; 
			break;
		}
	} else {
	...
	*/
	...
	if (!NT_SUCCESS(status)) {
		bsr_err(0, BSR_LC_TEMP, NO_OBJECT,"Bind() failed with status 0x%08X \n", status);
		err = -EINVAL;
		goto out;
	}
#else // _LIN
...
out:
	if (err < 0) {
		...
		if (!NT_SUCCESS(status)) {
			bsr_err(0, BSR_LC_TEMP, NO_OBJECT,"Failed to set WSK_EVENT_DISCONNECT. err(0x%x)\n", status);
			err = -1;
			goto out;
		}
...
#endif
{
	...
	*AcceptSocketContext = s_estab;
	...
	if (path2) {
		...
		{
			bsr_info(0, BSR_LC_TEMP, resource, "accept socket(0x%p) exists. \n", path2->socket);
			goto not_accept;
		}
		else {
			path2->socket = s_estab;
			       ...
			       struct bsr_listener **ret_listener)
{
    ...
    if (!NT_SUCCESS(status)) {
        bsr_err(0, BSR_LC_TEMP, NO_OBJECT,"ControlSocket: s_listen socket SO_REUSEADDR: failed=0x%x\n", status);
        err = -1;
        goto out;
    }
#else // _LIN
	...
	*ret_listener = &listener->listener;
    ...
    if (!NT_SUCCESS(status)) {
        bsr_err(0, BSR_LC_TEMP, NO_OBJECT,"Failed to set WSK_EVENT_ACCEPT. err(0x%x)\n", status);
    	err = -1;
        goto out;
    }
#endif	
...
static int dtt_connect(struct bsr_transport *transport)
{
	...
	NTSTATUS status;
	if (transport == NULL) {
		bsr_err(0, BSR_LC_TEMP, NO_OBJECT,"dtt_connect transport is null.\n");
		return -EDESTADDRREQ;
	}
#endif
	struct bsr_tcp_transport *tcp_transport =
		...
		if (s) {
			...
			if (use_for_data) {
				...
				if (dtt_send_first_packet(tcp_transport, dsocket, P_INITIAL_DATA, DATA_STREAM) <= 0) {
					bsr_err(0, BSR_LC_TEMP, NO_OBJECT,"failed to send first packet, dsocket (%p)\n", dsocket->sk);
				...
				if (dtt_send_first_packet(tcp_transport, csocket, P_INITIAL_META, CONTROL_STREAM) <= 0) {
					bsr_err(0, BSR_LC_TEMP, NO_OBJECT,"failed to send first packet, csocket (%p)\n", csocket->sk);
...
randomize:
    ...
    if (!NT_SUCCESS(status)) {
        bsr_err(0, BSR_LC_TEMP, NO_OBJECT,"ControlSocket: SO_REUSEADDR: failed=0x%x\n", status);
		// DW-1896 
	...
	for (int i = 0; i < 2; i++) {
		if (tcp_transport->stream[i] != NULL) {
			...
			if (attr->send_buf_thread_handle != NULL) {
				...
				bsr_warn(0, BSR_LC_SEND_BUFFER, NO_OBJECT, "No send_buffering thread(%s)\n", tcp_transport->stream[i]->name);
			}
		}
		else {
			bsr_warn(0, BSR_LC_SEND_BUFFER, NO_OBJECT, "No stream(channel:%d)\n", i);
		}
	}
	return;
}
	...
	for (i = 0; i < 2; i++) {
		if (tcp_transport->stream[i] != NULL) {
			...
			if (attr->send_buf_thread_handle != NULL) {
				...
				bsr_warn(0, BSR_LC_SEND_BUFFER, NO_OBJECT,"No send_buffering thread(channel:%d)\n", i);
			}
		}
		else {
			bsr_warn(0, BSR_LC_SEND_BUFFER, NO_OBJECT,"No stream(channel:%d)\n", i);
