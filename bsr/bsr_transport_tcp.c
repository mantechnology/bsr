...
int dtt_init(struct bsr_transport *transport)
{
	...
	for (i = DATA_STREAM; i <= CONTROL_STREAM ; i++) {
		...
		if (!buffer) {
			...
			bsr_warn(NO_OBJECT,"dtt_init kzalloc %s allocation fail", i ? "CONTROL_STREAM" : "DATA_STREAM" );
			goto fail;
		}
#else  // _LIN
...
static int dtt_recv_pages(struct bsr_transport *transport, struct bsr_page_chain_head *chain, size_t size)
{
	...
	if (err < 0) {
		...
		bsr_err(NO_OBJECT,"Wrong data (expected size:%d, received size:%d)", (int)size, err);
		err = -EIO;		
	...
	page_chain_for_each(page) {
		...
		if (err < 0) {
			...
			// DW-1502 If the size of the received data differs from the expected size, the consistency will be broken.
			bsr_err(NO_OBJECT,"Wrong data (expected size:%d, received size:%d)", (int)len, err);
			err = -EIO;
			goto fail;
		}
		size -= len;
...
static int dtt_try_connect(struct bsr_transport *transport, struct dtt_path *path, struct socket **ret_socket)
{
	...
	if (my_addr.ss_family == AF_INET6) {
		bsr_debug(NO_OBJECT,"dtt_try_connect: Connecting: %s -> %s", get_ip6(sbuf, sizeof(sbuf), (struct sockaddr_in6*)&my_addr), get_ip6(dbuf, sizeof(dbuf), (struct sockaddr_in6*)&peer_addr));
	} else {
		bsr_debug(NO_OBJECT,"dtt_try_connect: Connecting: %s -> %s", get_ip4(sbuf, sizeof(sbuf), (struct sockaddr_in*)&my_addr), get_ip4(dbuf, sizeof(dbuf), (struct sockaddr_in*)&peer_addr));
	}
	...
	if (!NT_SUCCESS(status)) {
		err = status;
		bsr_debug(NO_OBJECT,"dtt_try_connect: CreateSocketConnect fail status:%x socket->sk:%p",status,socket->sk);
		switch (status) {
		...
		default: 
			bsr_err(NO_OBJECT,"create-connect failed with status 0x%08X ", status);
			err = -EINVAL; 
			break;
		}
	} else {
	...
	*/
	...
	if (!NT_SUCCESS(status)) {
		bsr_err(NO_OBJECT,"Bind() failed with status 0x%08X ", status);
		err = -EINVAL;
		goto out;
	}
#else // _LIN
...
out:
	if (err < 0) {
		...
		if (!NT_SUCCESS(status)) {
			bsr_err(NO_OBJECT,"Failed to set WSK_EVENT_DISCONNECT. err(0x%x)", status);
			err = -1;
			goto out;
		}
...
static void unregister_state_change(struct sock *sock, struct dtt_listener *listener)
{
	...
	bsr_debug(NO_OBJECT,"WSK_EVENT_DISABLE (listener = 0x%p)", listener);
	if (!NT_SUCCESS(status)) {
		bsr_debug(NO_OBJECT,"WSK_EVENT_DISABLE failed (listener = 0x%p)", listener);
	}
#else // _LIN
...
#endif
{
	...
	*AcceptSocketContext = s_estab;
	...
	if (path2) {
		...
		{
			bsr_info(resource, "accept socket(0x%p) exists. ", path2->socket);
			goto not_accept;
		}
		else {
			path2->socket = s_estab;
		...
		{
			bsr_info(resource, "accept socket(0x%p) exists.", listener2->paccept_socket);
			goto not_accept;
		}
		else {
			       ...
			       struct bsr_listener **ret_listener)
{
    ...
    if (!NT_SUCCESS(status)) {
        bsr_err(NO_OBJECT,"ControlSocket: s_listen socket SO_REUSEADDR: failed=0x%x", status);
        err = -1;
        goto out;
    }
#else // _LIN
	...
	if (!NT_SUCCESS(status)) {
    	if(my_addr.ss_family == AF_INET) {
			bsr_err(NO_OBJECT,"AF_INET Failed to socket Bind(). err(0x%x) %02X.%02X.%02X.%02X:0x%X%X", status, (UCHAR)my_addr.__data[2], (UCHAR)my_addr.__data[3], (UCHAR)my_addr.__data[4], (UCHAR)my_addr.__data[5],(UCHAR)my_addr.__data[0],(UCHAR)my_addr.__data[1]);
    	} else {
			bsr_err(NO_OBJECT,"AF_INET6 Failed to socket Bind(). err(0x%x) [%02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X:%02X]:0x%X%X", status, (UCHAR)my_addr.__data[2],(UCHAR)my_addr.__data[3], (UCHAR)my_addr.__data[4],(UCHAR)my_addr.__data[5],
	...
	*ret_listener = &listener->listener;
    ...
    if (!NT_SUCCESS(status)) {
        bsr_err(NO_OBJECT,"Failed to set WSK_EVENT_ACCEPT. err(0x%x)", status);
    	err = -1;
        goto out;
    }
#endif	
...
static int dtt_connect(struct bsr_transport *transport)
{
	...
	list_for_each_entry_ex(struct bsr_path, bsr_path, &transport->paths, list) {
		...
		{		
			if (path->path.my_addr.ss_family == AF_INET6) {
				bsr_debug(NO_OBJECT,"dtt_connect: dtt_connect: path: %s -> %s.", get_ip6(sbuf, (struct sockaddr_in6*)&path->path.my_addr), get_ip6(dbuf, (struct sockaddr_in6*)&path->path.peer_addr));
			}
			else {
				bsr_debug(NO_OBJECT,"dtt_connect: dtt_connect: path: %s -> %s.", get_ip4(sbuf, (struct sockaddr_in*)&path->path.my_addr), get_ip4(dbuf, (struct sockaddr_in*)&path->path.peer_addr));
			}
		}
#endif
	...
	if (bsr_path == NULL) {
		...
		if (bsr_path->my_addr.ss_family == AF_INET6) {
			bsr_debug(NO_OBJECT,"dtt_connect: bsr_path: %s -> %s ", get_ip6(sbuf, sizeof(sbuf), (struct sockaddr_in6*)&bsr_path->my_addr), get_ip6(dbuf, sizeof(dbuf), (struct sockaddr_in6*)&bsr_path->peer_addr));
		} else {
			bsr_debug(NO_OBJECT,"dtt_connect: bsr_path: %s -> %s ", get_ip4(sbuf, sizeof(sbuf), (struct sockaddr_in*)&bsr_path->my_addr), get_ip4(dbuf, sizeof(dbuf), (struct sockaddr_in*)&bsr_path->peer_addr));
		}
	}
#endif
...
#ifdef _WIN
	{
		if(connect_to_path->path.my_addr.ss_family == AF_INET6) {
			bsr_debug(NO_OBJECT,"dtt_connect: connect_to_path: %s -> %s ", get_ip6(sbuf, sizeof(sbuf), (struct sockaddr_in6*)&connect_to_path->path.my_addr), get_ip6(dbuf, sizeof(dbuf), (struct sockaddr_in6*)&connect_to_path->path.peer_addr));
		} else {
			bsr_debug(NO_OBJECT,"dtt_connect: connect_to_path: %s -> %s ", get_ip4(sbuf, sizeof(sbuf), (struct sockaddr_in*)&connect_to_path->path.my_addr), get_ip4(dbuf, sizeof(dbuf), (struct sockaddr_in*)&connect_to_path->path.peer_addr));
		}
	}
#endif
	spin_unlock(&tcp_transport->paths_lock);
		...
		if (s) {
...
#ifdef _WIN
				if (connect_to_path->path.my_addr.ss_family == AF_INET6) {
					bsr_debug(NO_OBJECT,"dtt_connect: Connected: %s -> %s", get_ip6(sbuf, sizeof(sbuf), (struct sockaddr_in6*)&connect_to_path->path.my_addr), get_ip6(dbuf, sizeof(dbuf), (struct sockaddr_in6*)&connect_to_path->path.peer_addr));
				} else {
					bsr_debug(NO_OBJECT,"dtt_connect: Connected: %s -> %s", get_ip4(sbuf, sizeof(sbuf), (struct sockaddr_in*)&connect_to_path->path.my_addr), get_ip4(dbuf, sizeof(dbuf), (struct sockaddr_in*)&connect_to_path->path.peer_addr));
				}
#endif
			}
#endif
			...
			if (use_for_data) {
				...
				if (dtt_send_first_packet(tcp_transport, dsocket, P_INITIAL_DATA, DATA_STREAM) <= 0) {
					bsr_err(NO_OBJECT,"failed to send first packet, dsocket (%p)", dsocket->sk);
				...
				if (dtt_send_first_packet(tcp_transport, csocket, P_INITIAL_META, CONTROL_STREAM) <= 0) {
					bsr_err(NO_OBJECT,"failed to send first packet, csocket (%p)", csocket->sk);
...
#ifdef _WIN
				if (connect_to_path->path.my_addr.ss_family == AF_INET6) {
					bsr_debug(NO_OBJECT,"dtt_connect:(%p) Accepted:  %s <- %s", KeGetCurrentThread(), get_ip6(sbuf, sizeof(sbuf), (struct sockaddr_in6*)&connect_to_path->path.my_addr), get_ip6(dbuf, sizeof(dbuf), (struct sockaddr_in6*)&connect_to_path->path.peer_addr));
				} else {
					bsr_debug(NO_OBJECT,"dtt_connect:(%p) Accepted:  %s <- %s", KeGetCurrentThread(), get_ip4(sbuf, sizeof(sbuf), (struct sockaddr_in*)&connect_to_path->path.my_addr), get_ip4(dbuf, sizeof(dbuf), (struct sockaddr_in*)&connect_to_path->path.peer_addr));
				}				
#endif				
			}
#endif
...
randomize:
    ...
    if (!NT_SUCCESS(status)) {
        bsr_err(NO_OBJECT,"ControlSocket: SO_REUSEADDR: failed=0x%x", status);
		// DW-1896 
			...
			if (tcp_transport->stream[i] != NULL) {
				...
				if ((attr->bab = create_ring_buffer(connection, tcp_transport->stream[i]->name, size, i)) != NULL) {
					...
					if (i == CONTROL_STREAM) {
						...
						KeSetEvent(&attr->send_buf_kill_event, 0, FALSE);
						//bsr_info(NO_OBJECT,"wait for send_buffering_data_thread(%s) ack", tcp_transport->stream[i]->name);
						KeWaitForSingleObject(&attr->send_buf_killack_event, Executive, KernelMode, FALSE, NULL);
						//bsr_info(NO_OBJECT,"send_buffering_data_thread(%s) acked", tcp_transport->stream[i]->name);
						//ZwClose(attr->send_buf_thread_handle);
						attr->send_buf_thread_handle = NULL;
	...
	for (int i = 0; i < 2; i++) {
		if (tcp_transport->stream[i] != NULL) {
			...
			if (attr->send_buf_thread_handle != NULL) {
				...
				bsr_warn(NO_OBJECT,"No send_buffering thread(%s)", tcp_transport->stream[i]->name);
			}
		}
		else {
			bsr_warn(NO_OBJECT,"No stream(channel:%d)", i);
		}
	}
	return;
}
	...
	for (i = 0; i < 2; i++) {
		if (tcp_transport->stream[i] != NULL) {
			...
			if (attr->send_buf_thread_handle != NULL) {
				...
				bsr_warn(NO_OBJECT,"No send_buffering thread(channel:%d)", i);
			}
		}
		else {
			bsr_warn(NO_OBJECT,"No stream(channel:%d)", i);
