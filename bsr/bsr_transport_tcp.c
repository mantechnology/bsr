...
int dtt_init(struct bsr_transport *transport)
{
	...
	for (i = DATA_STREAM; i <= CONTROL_STREAM ; i++) {
		...
		if (!buffer) {
			...
			bsr_warn(BSR_LC_SOCKET, NO_OBJECT,"dtt_init kzalloc %s allocation fail\n", i ? "CONTROL_STREAM" : "DATA_STREAM" );
			goto fail;
		}
#else  // _LIN
...
static int dtt_recv_pages(struct bsr_transport *transport, struct bsr_page_chain_head *chain, size_t size)
{
	...
	bsr_debug_rs("kernel_recvmsg(%d) socket(0x%p) size(%d) all_pages(0x%p)\n", err, socket, (int)size, page);
	if (err < 0) {
		goto fail;
	}
		...
		bsr_err(BSR_LC_TEMP, NO_OBJECT,"Wrong data (expected size:%d, received size:%d)\n", (int)size, err);
		err = -EIO;		
	...
	page_chain_for_each(page) {
		...
		bsr_debug_rs("kernel_recvmsg(%d) socket(0x%p) size(%d) data(0x%p)\n", err, socket, (int)len, data);
		if (err < 0) {
			goto fail;
		}
		else if (err != (int)len) {
...
static bool dtt_path_cmp_addr(struct dtt_path *path, struct bsr_connection *connection)
{
	...
	if (bsr_path->my_addr_len == bsr_path->peer_addr_len) {
		...
		bsr_debug_conn("my_node_id = %d, peer_node_id = %d\n", my_node_id, peer_node_id);
...
static int dtt_try_connect(struct bsr_transport *transport, struct dtt_path *path, struct socket **ret_socket)
{
	...
	if (!NT_SUCCESS(status)) {
		err = status;
		bsr_debug(BSR_LC_TEMP, NO_OBJECT,"dtt_try_connect: CreateSocketConnect fail status:%x socket->sk:%p\n",status,socket->sk);
		switch (status) {
		...
		default: 
			bsr_err(BSR_LC_TEMP, NO_OBJECT,"create-connect failed with status 0x%08X \n", status);
			err = -EINVAL; 
			break;
		}
	} else {
	...
	*/
	...
	if (!NT_SUCCESS(status)) {
		bsr_err(BSR_LC_TEMP, NO_OBJECT,"Bind() failed with status 0x%08X \n", status);
		err = -EINVAL;
		goto out;
	}
#else // _LIN
...
out:
	if (err < 0) {
		...
		if (!NT_SUCCESS(status)) {
			bsr_err(BSR_LC_TEMP, NO_OBJECT,"Failed to set WSK_EVENT_DISCONNECT. err(0x%x)\n", status);
			err = -1;
			goto out;
		}
...
static bool dtt_socket_ok_or_free(struct socket **socket)
{
	...
	if ((*socket)->sk_state == WSK_ESTABLISHED) {
		bsr_debug_conn("socket->sk_state == WSK_ESTABLISHED wsk = %p\n", (*socket)->sk);
		return true;
	}

	bsr_debug_conn("wsk = %p socket->sk_state = %d\n", (*socket)->sk, (*socket)->sk_state);
...
static void unregister_state_change(struct sock *sock, struct dtt_listener *listener)
{
	...
	bsr_debug(BSR_LC_TEMP, NO_OBJECT,"WSK_EVENT_DISABLE (listener = 0x%p)\n", listener);
	if (!NT_SUCCESS(status)) {
		bsr_debug(BSR_LC_TEMP, NO_OBJECT,"WSK_EVENT_DISABLE failed (listener = 0x%p)\n", listener);
	}
#else // _LIN
...
retry:
	...
	if (path->socket) {
		bsr_debug_conn("path->socket s_estab = path->socket(%p)\n", path->socket->sk);
		s_estab = path->socket;
		path->socket = NULL;
		...
		if (listener->paccept_socket) {
			...
			bsr_debug_conn("create estab_sock s_estab = listener->paccept_socket(%p)\n", s_estab);
		}
		else {
			if (status == STATUS_TIMEOUT) {
	...
	bsr_debug_co("%p dtt_wait_for_connect ok done.\n", KeGetCurrentThread());
	spin_unlock_bh(&listener->listener.waiters_lock);
	*socket = s_estab;
	*ret_path = path;
	return 0;
...
#ifdef _WIN
    bsr_debug_sk("socket(0x%p) err(%d) header_size(%u)\n", socket, err, header_size);
#endif
...
#endif
{
	...
	*AcceptSocketContext = s_estab;
	...
	if (path2) {
		...
		{
			bsr_info(BSR_LC_TEMP, resource, "accept socket(0x%p) exists. \n", path2->socket);
			goto not_accept;
		}
		else {
			path2->socket = s_estab;
	...
	spin_unlock(&listener->waiters_lock);
	spin_unlock_bh(&resource->listeners_lock);
	bsr_debug_sk("s_estab(0x%p) wsk(0x%p) wake!!!!\n", s_estab, AcceptSocket);
			       ...
			       struct bsr_listener **ret_listener)
{
    ...
    if (!NT_SUCCESS(status)) {
        bsr_err(BSR_LC_TEMP, NO_OBJECT,"ControlSocket: s_listen socket SO_REUSEADDR: failed=0x%x\n", status);
        err = -1;
        goto out;
    }
#else // _LIN
	...
	*ret_listener = &listener->listener;
    ...
    if (!NT_SUCCESS(status)) {
        bsr_err(BSR_LC_TEMP, NO_OBJECT,"Failed to set WSK_EVENT_ACCEPT. err(0x%x)\n", status);
    	err = -1;
        goto out;
    }
#endif	
...
static int dtt_connect(struct bsr_transport *transport)
{
	...
	NTSTATUS status;
	if (transport == NULL) {
		bsr_err(BSR_LC_TEMP, NO_OBJECT,"dtt_connect transport is null.\n");
		return -EDESTADDRREQ;
	}
#endif
	struct bsr_tcp_transport *tcp_transport =
		...
		if (s) {
...
#ifdef _WIN
				if (connect_to_path->path.my_addr.ss_family == AF_INET6) {
					bsr_debug(BSR_LC_TEMP, NO_OBJECT,"dtt_connect: Connected: %s -> %s\n", get_ip6(sbuf, sizeof(sbuf), (struct sockaddr_in6*)&connect_to_path->path.my_addr), get_ip6(dbuf, sizeof(dbuf), (struct sockaddr_in6*)&connect_to_path->path.peer_addr));
				} else {
					bsr_debug(BSR_LC_TEMP, NO_OBJECT,"dtt_connect: Connected: %s -> %s\n", get_ip4(sbuf, sizeof(sbuf), (struct sockaddr_in*)&connect_to_path->path.my_addr), get_ip4(dbuf, sizeof(dbuf), (struct sockaddr_in*)&connect_to_path->path.peer_addr));
				}
#endif
			...
			if (use_for_data) {
				...
				if (dtt_send_first_packet(tcp_transport, dsocket, P_INITIAL_DATA, DATA_STREAM) <= 0) {
					bsr_err(BSR_LC_TEMP, NO_OBJECT,"failed to send first packet, dsocket (%p)\n", dsocket->sk);
				...
				if (dtt_send_first_packet(tcp_transport, csocket, P_INITIAL_META, CONTROL_STREAM) <= 0) {
					bsr_err(BSR_LC_TEMP, NO_OBJECT,"failed to send first packet, csocket (%p)\n", csocket->sk);
...
retry:
		...
		if (err < 0 && err != -EAGAIN) {
			bsr_debug_conn("dtt_wait_for_connect fail err = %d goto out\n", err); 
			goto out;
		}
...
randomize:
    ...
    if (!NT_SUCCESS(status)) {
        bsr_err(BSR_LC_TEMP, NO_OBJECT,"ControlSocket: SO_REUSEADDR: failed=0x%x\n", status);
		// DW-1896 
...
static void dtt_update_congested(struct bsr_tcp_transport *tcp_transport)
{
	...
	bsr_debug_tr("dtt_update_congested:  sndbuf=%d sk_wmem_queued=%d\n", sock->sk_sndbuf, sk_wmem_queued);
			...
			if (tcp_transport->stream[i] != NULL) {
				...
				if ((attr->bab = create_ring_buffer(connection, tcp_transport->stream[i]->name, size, i)) != NULL) {
					...
					if (i == CONTROL_STREAM) {
						...
						KeSetEvent(&attr->send_buf_kill_event, 0, FALSE);
						//bsr_info(BSR_LC_TEMP, NO_OBJECT,"wait for send_buffering_data_thread(%s) ack\n", tcp_transport->stream[i]->name);
						KeWaitForSingleObject(&attr->send_buf_killack_event, Executive, KernelMode, FALSE, NULL);
						//bsr_info(BSR_LC_TEMP, NO_OBJECT,"send_buffering_data_thread(%s) acked\n", tcp_transport->stream[i]->name);
						//ZwClose(attr->send_buf_thread_handle);
						attr->send_buf_thread_handle = NULL;
	...
	for (int i = 0; i < 2; i++) {
		if (tcp_transport->stream[i] != NULL) {
			...
			if (attr->send_buf_thread_handle != NULL) {
				...
				bsr_warn(BSR_LC_SEND_BUFFER, NO_OBJECT, "No send_buffering thread(%s)\n", tcp_transport->stream[i]->name);
			}
		}
		else {
