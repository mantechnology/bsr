...
#endif
{
	...
	if( !NT_SUCCESS(status) ) {
		bsr_err(18, BSR_LC_THREAD, NO_OBJECT, "Cannot create client security, err=0x%x", status);
		return status;
	}
	...
	if( !NT_SUCCESS(status) ) {
		bsr_err(19, BSR_LC_THREAD, NO_OBJECT, "Failed to Create Thread, err(0x%x)", status);
...
mvolWorkThread(PVOID arg)
{
	...
	bsr_debug(32, BSR_LC_THREAD, NO_OBJECT,"WorkThread [%ws]: handle 0x%x start", VolumeExtension->PhysicalDeviceName, KeGetCurrentThread());
#endif
	...
	for (;;) {
...
#else
			bsr_debug(33, BSR_LC_THREAD, NO_OBJECT,"WorkThread [%ws]: Terminate Thread", VolumeExtension->PhysicalDeviceName);
#endif
		...
		while ((request = ExInterlockedRemoveHeadList(&pThreadInfo->ListHead, &pThreadInfo->ListLock)) != 0) {
			...
			case IRP_MJ_FLUSH_BUFFERS:
				...
				break;
			default:
				bsr_err(21, BSR_LC_THREAD, NO_OBJECT, "Invalid IRP Major Function (0x%x)", irpSp->MajorFunction);
		...
		if (loop > 1) {
			if (high < loop) {
				high = loop;
				bsr_info(22, BSR_LC_THREAD, NO_OBJECT, "hooker[%ws]: irp processing peek(%d)",
