...
#endif
{
	...
	{
		drbd_err(NO_DEVICE,"cannot create client security, err=0x%x\n", status);
		return status;
	}
	...
	if( !NT_SUCCESS(status) )
	{
		drbd_err(NO_DEVICE,"cannot create Thread, err=0x%x\n", status);
...
mvolWorkThread(PVOID arg)
{
	...
	for (;;)
	{
		...
		{
#ifdef _WIN32_MULTIVOL_THREAD
			drbd_info(NO_DEVICE,"Terminating mvolWorkThread\n");
#else
			drbd_debug(NO_DEVICE,"WorkThread [%ws]: Terminate Thread\n", VolumeExtension->PhysicalDeviceName);
#endif
		...
		while ((request = ExInterlockedRemoveHeadList(&pThreadInfo->ListHead, &pThreadInfo->ListLock)) != 0)
		{
...
#ifdef DRBD_TRACE	
			DbgPrint("\n");
			drbd_debug(NO_DEVICE,"I/O Thread:IRQL(%d) start I/O(%s) loop(%d) .......................!\n", 
			...
			case IRP_MJ_FLUSH_BUFFERS:
				...
				break;
			default:
				drbd_err(NO_DEVICE,"WorkThread: invalid IRP MJ=0x%x\n", irpSp->MajorFunction);
		...
		if (loop > 1)
		{
			...
			{
				high = loop;
				drbd_info(NO_DEVICE,"hooker[%ws]: irp processing peek(%d)\n",
...
VOID mvolQueueWork (PMVOL_THREAD pThreadInfo, PDEVICE_OBJECT DeviceObject, PIRP irp)
{
    ...
    if(!wr) {
        drbd_err(NO_DEVICE,"Could not allocate mvol work.\n");
        return;
    }
