...
IOCTL_GetAllVolumeInfo( PIRP Irp, PULONG ReturnLength )
{
	*ReturnLength = 0;
	...
	if (outlen < (count * sizeof(BSR_VOLUME_ENTRY))) {
		//bsr_err(NO_OBJECT,"IOCTL_GetAllVolumeInfo buffer too small outlen:%d required len:%d",outlen,(count * sizeof(BSR_VOLUME_ENTRY)) );
...
IOCTL_GetVolumeInfo( PDEVICE_OBJECT DeviceObject, PIRP Irp, PULONG ReturnLength )
{
	...
	if( outlen < sizeof(MVOL_VOLUME_INFO) )	{
		...
		bsr_err(NO_OBJECT,"buffer too small out %d sizeof(MVOL_VOLUME_INFO) %d", outlen, sizeof(MVOL_VOLUME_INFO));
...
IOCTL_GetVolumeSize( PDEVICE_OBJECT DeviceObject, PIRP Irp )
{
	...
	if( DeviceObject == mvolRootDeviceObject ) {
		...
		if( VolumeExtension == NULL ) {
			...
			bsr_err(NO_OBJECT,"cannot find volume, PD=%ws", pVolumeInfo->PhysicalDeviceName);
	...
	if( !NT_SUCCESS(status) ) {
		...
		bsr_err(NO_OBJECT,"cannot get volume size, err=0x%x", status);
	}
...
IOCTL_SetSimulDiskIoError( PDEVICE_OBJECT DeviceObject, PIRP Irp)
{
	...
	if(Irp->AssociatedIrp.SystemBuffer) {
		...
		bsr_info(NO_OBJECT,"IOCTL_MVOL_SET_SIMUL_DISKIO_ERROR ErrorFlag:%d ErrorType:%d", gSimulDiskIoError.ErrorFlag, gSimulDiskIoError.ErrorType);
	} else {
		return STATUS_INVALID_PARAMETER;
	}
...
IOCTL_SetMinimumLogLevel(PDEVICE_OBJECT DeviceObject, PIRP Irp)
{
	...
	if (Irp->AssociatedIrp.SystemBuffer) {
		...
		if (pLoggingMinLv->nType == LOGGING_TYPE_SYSLOG) {
			...
			bsr_warn(NO_OBJECT,"invalidate logging type(%d)", pLoggingMinLv->nType);
		}
...
IOCTL_SetLogFileMaxCount(PDEVICE_OBJECT DeviceObject, PIRP Irp)
{
	...
	if (Irp->AssociatedIrp.SystemBuffer) {
		...
		bsr_info(NO_OBJECT, "set log file max count %lu => %lu", atomic_read(&g_log_file_max_count), log_file_max_count);
...
IOCTL_SetHandlerUse(PDEVICE_OBJECT DeviceObject, PIRP Irp)
{
	...
	if (Irp->AssociatedIrp.SystemBuffer) {
		...
		bsr_debug(NO_OBJECT,"IOCTL_MVOL_SET_HANDLER_USE : %d ", g_handler_use);
	}
	else {
		return STATUS_INVALID_PARAMETER;
	}
...
--*/	
{
	...
	if (VolumeExtension == NULL) {
		bsr_err(NO_OBJECT,"cannot find volume, PDO=0x%p", pDeviceObject);
		return;
	}

	bsr_info(NO_OBJECT,"volume [%ws] is extended.", VolumeExtension->PhysicalDeviceName);
	...
	if (VolumeExtension->Active) {	
		...
		if (device) {
			...
			if (err) {
				bsr_err(NO_OBJECT,"bsr resize failed. (err=%d)", err);
			}
...
--*/
{
	...
	if (!NT_SUCCESS(status)) {
		bsr_info(NO_OBJECT,"ExCreateCallback failed, status : 0x%x", status);
		return status;
	}
