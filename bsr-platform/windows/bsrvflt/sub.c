...
mvolRemoveDevice(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp)
{
	...
	status = mvolRunIrpSynchronous(DeviceObject, Irp);
	if (!NT_SUCCESS(status)) {
		bsr_err(BSR_LC_TEMP, NO_OBJECT,"cannot remove device, status=0x%x\n", status);
	}
	...
	if (VolumeExtension->WorkThreadInfo.Active) {
		...
		bsr_debug(BSR_LC_TEMP, NO_OBJECT,"[%ws]: WorkThread Terminate Completely\n",	VolumeExtension->PhysicalDeviceName);
	}
#endif
	...
	if (device/* && (mdev->state.role == R_PRIMARY)*/) {
		...
		for (io_id = 0; io_id < loop; io_id++) {
			...
			if (Io == IRP_MJ_READ) {
				...
				if (!newbuf) {
					status = STATUS_NO_MEMORY;
					bsr_err(BSR_LC_TEMP, NO_OBJECT,"HOOKER malloc fail!!!\n");
					goto fail_put_dev;
				}
			}
			else {
		...
		if (rest) {
			...
			if (Io == IRP_MJ_READ) {
				...
				if (!newbuf) {
					status = STATUS_NO_MEMORY;
					bsr_err(BSR_LC_TEMP, NO_OBJECT,"HOOKER rest malloc fail!!\n");
					goto fail_put_dev;
				}
			}
			else {
...
fail:
	bsr_err(BSR_LC_TEMP, NO_OBJECT,"failed. status=0x%x\n", status);
	return status;
}
...
mvolGetVolumeSize(PDEVICE_OBJECT TargetDeviceObject, PLARGE_INTEGER pVolumeSize)
{
    ...
    if (KeGetCurrentIrql() > APC_LEVEL) {
        bsr_err(BSR_LC_TEMP, NO_OBJECT,"cannot run IoBuildDeviceIoControlRequest becauseof IRP(%d)\n", KeGetCurrentIrql());
    }
    ...
    if (!newIrp) {
        bsr_err(BSR_LC_TEMP, NO_OBJECT,"cannot alloc new IRP\n");
        return STATUS_INSUFFICIENT_RESOURCES;
    }
    ...
    if (!NT_SUCCESS(status)) {
        bsr_err(BSR_LC_TEMP, NO_OBJECT,"cannot get volume information, err=0x%x\n", status);
        return status;
    }
...
mvolUpdateMountPointInfoByExtension(PVOLUME_EXTENSION pvext)
{
	...
	bsr_info(BSR_LC_TEMP, NO_OBJECT,"----------QueryMountPoint--------------------pvext:%p\n",pvext);
	for (ULONG i = 0; i < pmps->NumberOfMountPoints; i++) {
		...
		bsr_info(BSR_LC_TEMP, NO_OBJECT,"SymbolicLink num:%lu %wZ\n",i,&name);
		...
		if (MOUNTMGR_IS_DRIVE_LETTER(&name)) {
			...
			bsr_debug(BSR_LC_TEMP, NO_OBJECT,"Free letter link\n");
		}
		else if (MOUNTMGR_IS_VOLUME_NAME(&name)) {

			link = &pvext->VolumeGuid;
			//FreeUnicodeString(link);
			bsr_debug(BSR_LC_TEMP, NO_OBJECT,"Free volume guid link\n");
		}

		if(link) {
			ucsdup(link, name.Buffer, name.Length);
			bsr_debug(BSR_LC_TEMP, NO_OBJECT,"link alloc\n");
		}
		
	}
	bsr_info(BSR_LC_TEMP, NO_OBJECT,"----------QueryMountPoint--------------------pvext:%p end..............\n",pvext);
cleanup:
	kfree(inbuf);
	kfree(otbuf);
...
mvolLogError(PDEVICE_OBJECT DeviceObject, ULONG UniqID, NTSTATUS ErrorCode, NTSTATUS Status)
{
	...
	if (pLogEntry == NULL) {
		bsr_err(BSR_LC_TEMP, NO_OBJECT,"cannot alloc Log Entry\n");
		return;
	}
