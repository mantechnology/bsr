...
mvolRemoveDevice(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp)
{
	...
	if (!NT_SUCCESS(status))
	{
		drbd_err(,"cannot remove device, status=0x%x\n", status);
	}
	...
	if (VolumeExtension->WorkThreadInfo.Active)
	{
		...
		drbd_debug(,"[%ws]: WorkThread Terminate Completely\n",	VolumeExtension->PhysicalDeviceName);
	}
#endif
	...
	if (device/* && (mdev->state.role == R_PRIMARY)*/) {
		...
		if (rest)
		{
			...
			if (Io == IRP_MJ_READ)
			{
				...
				{
					status = STATUS_NO_MEMORY;
					drbd_err(,"HOOKER rest malloc fail!!\n");
					goto fail_put_dev;
				}
			}
			else
...
mvolGetVolumeSize(PDEVICE_OBJECT TargetDeviceObject, PLARGE_INTEGER pVolumeSize)
{
    ...
    if (KeGetCurrentIrql() > APC_LEVEL) {
        drbd_err(,"cannot run IoBuildDeviceIoControlRequest becauseof IRP(%d)\n", KeGetCurrentIrql());
    }
    ...
    if (!NT_SUCCESS(status)) {
        drbd_err(,"cannot get volume information, err=0x%x\n", status);
        return status;
    }
...
mvolUpdateMountPointInfoByExtension(PVOLUME_EXTENSION pvext)
{
	...
	drbd_info(,"----------QueryMountPoint--------------------pvext:%p\n",pvext);
	for (ULONG i = 0; i < pmps->NumberOfMountPoints; i++) {
		...
		drbd_info(,"SymbolicLink num:%d %wZ\n",i,&name);
		...
		if (MOUNTMGR_IS_DRIVE_LETTER(&name)) {
			...
			drbd_debug(,"Free letter link\n");
		}
		else if (MOUNTMGR_IS_VOLUME_NAME(&name)) {

			link = &pvext->VolumeGuid;
			//FreeUnicodeString(link);
			drbd_debug(,"Free volume guid link\n");
		}

		if(link) {
			ucsdup(link, name.Buffer, name.Length);
			drbd_debug(,"link alloc\n");
		}
		
	}
	drbd_info(,"----------QueryMountPoint--------------------pvext:%p end..............\n",pvext);
cleanup:
	kfree(inbuf);
	kfree(otbuf);
...
mvolLogError(PDEVICE_OBJECT DeviceObject, ULONG UniqID, NTSTATUS ErrorCode, NTSTATUS Status)
{
	...
	if (pLogEntry == NULL)
	{
		drbd_err(,"cannot alloc Log Entry\n");
		return;
	}
...
--*/
{
    ...
    if (KeGetCurrentIrql() > PASSIVE_LEVEL) // DRBD_DOC: DV: skip api RtlStringCchPrintfW(PASSIVE_LEVEL)
    {
        ...
        drbd_warn(,"IRQL(%d) too high. Log canceled.\n", KeGetCurrentIrql());
        return 1;
    }
#endif
...
static USHORT getStackFrames(PVOID *frames, USHORT usFrameCount)
{
	...
	if (NULL == frames ||
		0 == usFrameCount)
	{
		drbd_err(,"Invalid Parameter, frames(%p), usFrameCount(%d)\n", frames, usFrameCount);
		return 0;
	}
	...
	if (0 == usCaptured)
	{
		drbd_err(,"Captured frame count is 0\n");
		return 0;
	}
...
VOID WriteOOSTraceLog(int bitmap_index, ULONG_PTR startBit, ULONG_PTR endBit, ULONG_PTR bitsCount, unsigned int mode)
{
	...
	if (NULL == stackFrames)
	{
		drbd_err(,"Failed to allcate pool for stackFrames\n");
		return;
	}
