...
DriverEntry(IN PDRIVER_OBJECT DriverObject, IN PUNICODE_STRING RegistryPath)
{
	...
	if (*InitSafeBootMode > 0) {
		bsr_info(NO_OBJECT, "booted to safe mode %u", *InitSafeBootMode);
	}
	else {
    ...
    if (!NT_SUCCESS(status)) {
        bsr_err(NO_OBJECT,"Can't create root, err=%x", status);
        return status;
    }
    ...
    if (!NT_SUCCESS(status)) {
        bsr_err(NO_OBJECT,"cannot create symbolic link, err=%x", status);
    ...
    bsr_info(NO_OBJECT,"MVF Driver loaded.");
...
mvolAddDevice(IN PDRIVER_OBJECT DriverObject, IN PDEVICE_OBJECT PhysicalDeviceObject)
{
	...
	if (*InitSafeBootMode > 0) {
		...
		//2 :SAFEBOOT_NETWORK
		//3 :SAFEBOOT_DSREPAIR
		bsr_info(NO_OBJECT, "safe boot mode %u", *InitSafeBootMode);
    ...
    if (!NT_SUCCESS(status)) {
        ...
        bsr_err(NO_OBJECT,"cannot create device, err=0x%x", status);
        return status;
    }
    ...
    if (do_add_minor(VolumeExtension->Minor)) {
        ...
        if (!NT_SUCCESS(status)) {
            bsr_err(NO_OBJECT,"Failed to initialize WorkThread. status(0x%x)", status);
            //return status;
        }
#endif
...
mvolCreate(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp)
{
#if 0 // DW-1380
    if (DeviceObject == mvolRootDeviceObject) {
        bsr_debug(NO_OBJECT,"mvolRootDevice Request");
    ...
    if (VolumeExtension->Active) 
	{
		...
		if (device && ((R_PRIMARY != device->resource->role[NOW]) || (device->resource->bPreDismountLock == TRUE) || device->disk_state[NOW] == D_DISKLESS))   // V9
		{
			...
			//bsr_debug(NO_OBJECT,"DeviceObject(0x%x), MinorFunction(0x%x) STATUS_INVALID_DEVICE_REQUEST", DeviceObject, irpSp->MinorFunction);
    ...
    if (VolumeExtension->Active) {
		...
		if (device && device->resource && (device->resource->role[NOW] == R_PRIMARY) && (device->resource->bPreSecondaryLock == FALSE) && (device->disk_state[NOW] > D_FAILED)) {
			...
			if (offset_sector + size_sector > vol_size_sector) {
				...
				bsr_debug(NO_OBJECT,"Upper driver WRITE vol(%ws) sect(0x%llx+%u) VolumeExtension->IrpCount(%d) ......................Skipped Irp:%p Irp->Flags:%x",
				...
				if (offset_sector + size_sector > saved_size && real_size > saved_size) {
					bsr_debug(NO_OBJECT,"saved_size (%llu), real_size (%llu) vol_sector(%llu) off_sector(%llu)", saved_size >> 9, real_size >> 9, vol_size_sector, offset_sector + size_sector);
					bsr_debug(NO_OBJECT,"need to temporary bm write");
				}				
...
#ifdef BSR_TRACE
			bsr_debug(NO_OBJECT,"Upper driver WRITE vol(%wZ) sect(0x%llx+%u) ................Queuing(%d)!",
    ...
    switch (irpSp->Parameters.DeviceIoControl.IoControlCode) {
		...
		case IOCTL_MOUNTDEV_LINK_CREATED:
		{
			...
			if (MOUNTMGR_IS_DRIVE_LETTER(&d)) {
				...
				bsr_debug(NO_OBJECT, "IOCTL_MOUNTDEV_LINK_CREATED %ws, minor %d", VolumeExtension->MountPoint, VolumeExtension->Minor);
			}
				...
				bsr_debug(NO_OBJECT, "IOCTL_MOUNTDEV_LINK_CREATED %ws", VolumeExtension->VolumeGuid);
			}
		...
		case IOCTL_MOUNTDEV_LINK_DELETED:
		{
			...
			MVOL_LOCK();
			if (MOUNTMGR_IS_DRIVE_LETTER(&d)) {
				bsr_debug(NO_OBJECT, "IOCTL_MOUNTDEV_LINK_DELETED %ws", VolumeExtension->MountPoint);
				memset(VolumeExtension->MountPoint, 0, sizeof(VolumeExtension->MountPoint));
				VolumeExtension->Minor = 0;
			}
			else if (MOUNTMGR_IS_DRIVE_LETTER(&d)) {
				bsr_debug(NO_OBJECT, "IOCTL_MOUNTDEV_LINK_DELETED %ws", VolumeExtension->VolumeGuid);
				memset(VolumeExtension->VolumeGuid, 0, sizeof(VolumeExtension->VolumeGuid));
			}
			MVOL_UNLOCK();
