...
static int bsr_set_minlog_lv(LOGGING_MIN_LV __user * args)
{
	...
	if (err) {
		bsr_err(0, BSR_LC_TEMP, NO_OBJECT, "LOGGING_MIN_LV copy from user failed.\n");
		return -1;
	}
	...
	// DW-2008
	bsr_info(0, BSR_LC_TEMP, NO_OBJECT,"set minimum log level, type : %s(%d), minumum level : %s(%d) => %s(%d)\n", 
...
static int bsr_get_log(BSR_LOG __user *bsr_log) 
{
	...
	if (err) {
		bsr_warn(0, BSR_LC_DRIVER, NO_OBJECT, "gTotalLogCnt copy to user failed.\n");
		return err;
	}
	err = copy_to_user(&bsr_log->LogBuf, &gLogBuf.b, MAX_BSRLOG_BUF*LOGBUF_MAXCNT);
	if (err) {
		bsr_warn(0, BSR_LC_DRIVER, NO_OBJECT, "gLogBuf copy to user failed.\n");
	}
...
static int bsr_set_handler_use(HANDLER_INFO __user *args)
{
	...
	if (err) {
		bsr_err(0, BSR_LC_TEMP, NO_OBJECT, "HANDLER_INFO copy from user failed.\n");
		return -1;
	}
	
	bsr_info(0, BSR_LC_TEMP, NO_OBJECT, "set handler_use %d => %d\n", g_handler_use, h_info.use);
	g_handler_use = h_info.use;
...
int bsr_readdir(char * dir_path, struct log_rolling_file_list * rlist)
{
	...
	if (fdir) {
		...
		bsr_err(0, BSR_LC_TEMP, NO_OBJECT, "failed to open log directory\n");
	}
	set_fs(oldfs);
