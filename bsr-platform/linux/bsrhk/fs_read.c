...
static char * read_superblock(struct file *fd)
{
	...
	*/
	...
	if (ret < 0 || ret != sizeof(super_block)) {
		bsr_err(200, BSR_LC_RESYNC_OV, NO_OBJECT, "Failed to read super block. err(%ld)\n", ret);
		return NULL;
	}
...
PVOID GetVolumeBitmap(struct bsr_device *device, ULONGLONG * ptotal_block, ULONG * pbytes_per_block)
{
	...
	if (fd == NULL || IS_ERR(fd)) {
		bsr_err(201, BSR_LC_RESYNC_OV, device, "Failed to get volume bitmap due to failure to open %s\n", disk_name);
		goto out;
	}
