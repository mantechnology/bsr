...
PVOLUME_BITMAP_BUFFER read_xfs_bitmap(struct file *fd, struct xfs_sb *xfs_sb)
{
	...
	if (debug_fast_sync) {
		...
		bsr_info(0, BSR_LC_TEMP, NO_OBJECT, "version : %d \n", be16_to_cpu(XFS_SB_VERSION_NUM(xfs_sb)));
		bsr_info(0, BSR_LC_TEMP, NO_OBJECT, "ag_count : %d \n", ag_count);
		bsr_info(0, BSR_LC_TEMP, NO_OBJECT, "total block count : %llu \n", total_block);	
		bsr_info(0, BSR_LC_TEMP, NO_OBJECT, "blocks_per_ag : %ld \n", (long int)ag_blocks_offset);
		bsr_info(0, BSR_LC_TEMP, NO_OBJECT, "block size : %d \n", blk_size);
		bsr_info(0, BSR_LC_TEMP, NO_OBJECT, "sector size : %d \n", sect_size);
	...
	for (ag_no = 0; ag_no < ag_count; ag_no++) {
		...
		if (offset < 0) {
			bsr_err(0, BSR_LC_TEMP, NO_OBJECT, "failed to lseek first leaf node of btree_block (err=%lld)\n", offset);
			goto fail_and_free;
		}
		...
		if (ret < 0 || ret != sizeof(struct xfs_btree_block)) {
			bsr_err(0, BSR_LC_TEMP, NO_OBJECT, "failed to read first leaf node of btree_block (err=%d)\n", ret);
			goto fail_and_free;
		}
		...
		if (!xfs_sb_version_hascrc(xfs_sb)) {
			...
			if (offset < 0) {
				bsr_err(0, BSR_LC_TEMP, NO_OBJECT, "failed to lseek reduced addition size of btree_block since version5(err=%lld)\n", offset);
				goto fail_and_free;
			}
		}
		...
		if(bb_level != 0) {
			bsr_err(0, BSR_LC_TEMP, NO_OBJECT, "failed to read leaf node (err=%hd)\n", bb_level);
			goto fail_and_free;
		}
		bb_numrecs = be16_to_cpu(btsb.bb_numrecs);

		if (debug_fast_sync) {
			bsr_info(0, BSR_LC_TEMP, NO_OBJECT, "[ag_no:%d] first leaf node bb_level : %hd bb_numrecs : %hd\n", ag_no, bb_level, bb_numrecs);
		}
		...
		do {
			if(bb_rightsib > 0) {
				...
				if (offset < 0) {
					bsr_err(0, BSR_LC_TEMP, NO_OBJECT, "failed to lseek secondary btree_block (err=%lld)\n", offset);
					goto fail_and_free;
				}
				...
				ret = bsr_read(fd, (char *)&btsb, sizeof(struct xfs_btree_block), &fd->f_pos);
				if (ret < 0 || ret != sizeof(struct xfs_btree_block)) {
					bsr_err(0, BSR_LC_TEMP, NO_OBJECT, "failed to read secondary btree_block (err=%d)\n", ret);
					goto fail_and_free;
				}

				bb_level = be16_to_cpu(btsb.bb_level);
				if(bb_level != 0) {
					bsr_err(0, BSR_LC_TEMP, NO_OBJECT, "failed to read secondary leaf node (err=%hd)\n", bb_level);
					goto fail_and_free;
				}
				bb_numrecs = be16_to_cpu(btsb.bb_numrecs);
			}
			...
			for(bb_numrecs_no = 0 ; bb_numrecs_no < bb_numrecs ; bb_numrecs_no++) {
				...
				for(bitcount_no = startbit ; bitcount_no < (blockcount + startbit) ; bitcount_no++) {
					if(offset + (bitcount_no/BITS_PER_BYTE) >= bitmap_buf->BitmapSize) {
						bsr_err(0, BSR_LC_TEMP, NO_OBJECT, "failed to read free block info, bitmap buffer overflow! (startblock:%d, blockcount:%d)\n", startblock, blockcount);
						goto fail_and_free;
					}
	...
	if (debug_fast_sync) {
		bsr_info(0, BSR_LC_TEMP, NO_OBJECT, "total free_blocks : %llu free_bits : %llu\n", free_blocks_co, free_bits_co);
	}
