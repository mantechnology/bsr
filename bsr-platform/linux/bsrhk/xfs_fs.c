...
PVOLUME_BITMAP_BUFFER read_xfs_bitmap(struct file *fd, struct xfs_sb *xfs_sb)
{
	...
	if (bitmap_buf == NULL) {
		bsr_err(202, BSR_LC_RESYNC_OV, NO_OBJECT, "Failed to read xfs bitmap due to failure to allocate %d size memory for bitmap buffer\n", (sizeof(VOLUME_BITMAP_BUFFER) + bitmap_size));
		return NULL;
	}
	...
	for (ag_no = 0; ag_no < ag_count; ag_no++) {
		...
		if (offset < 0) {
			bsr_err(212, BSR_LC_RESYNC_OV, NO_OBJECT, "Failed to read xfs bitmap due to failure to lseek first leaf node of btree_block (err=%lld)\n", offset);
			goto fail_and_free;
		}
		...
		if (ret < 0 || ret != sizeof(struct xfs_btree_block)) {
			bsr_err(213, BSR_LC_RESYNC_OV, NO_OBJECT, "Failed to read xfs bitmap due to failure to read first leaf node of btree_block (err=%d)\n", ret);
			goto fail_and_free;
		}
		...
		if (!xfs_sb_version_hascrc(xfs_sb)) {
			...
			if (offset < 0) {
				bsr_err(214, BSR_LC_RESYNC_OV, NO_OBJECT, "Failed to read xfs bitmap due to failure to lseek reduced addition size of btree_block since version5(err=%lld)\n", offset);
				goto fail_and_free;
			}
		}
		...
		if(bb_level != 0) {
			bsr_err(215, BSR_LC_RESYNC_OV, NO_OBJECT, "Failed to read xfs bitmap due to failure to read leaf node (err=%hd)\n", bb_level);
			goto fail_and_free;
		}
		bb_numrecs = be16_to_cpu(btsb.bb_numrecs);

		if (debug_fast_sync) {
			bsr_info(215, BSR_LC_RESYNC_OV, NO_OBJECT, "[ag_no:%d] first leaf node bb_level : %hd bb_numrecs : %hd\n", ag_no, bb_level, bb_numrecs);
		}
		...
		do {
			if(bb_rightsib > 0) {
				...
				if (offset < 0) {
					bsr_err(216, BSR_LC_RESYNC_OV, NO_OBJECT, "Failed to read xfs bitmap due to failure to lseek secondary btree_block (err=%lld)\n", offset);
					goto fail_and_free;
				}
				...
				bb_level = be16_to_cpu(btsb.bb_level);
				if(bb_level != 0) {
					bsr_err(218, BSR_LC_RESYNC_OV, NO_OBJECT, "Failed to read xfs bitmap due to failure to read secondary leaf node (err=%hd)\n", bb_level);
					goto fail_and_free;
				}
				bb_numrecs = be16_to_cpu(btsb.bb_numrecs);
			}
			...
			for(bb_numrecs_no = 0 ; bb_numrecs_no < bb_numrecs ; bb_numrecs_no++) {
				...
				for(bitcount_no = startbit ; bitcount_no < (blockcount + startbit) ; bitcount_no++) {
					if(offset + (bitcount_no/BITS_PER_BYTE) >= bitmap_buf->BitmapSize) {
						bsr_err(219, BSR_LC_RESYNC_OV, NO_OBJECT, "Failed to read xfs bitmap due to failure to read free block info, bitmap buffer overflow! (startblock:%d, blockcount:%d)\n", startblock, blockcount);
						goto fail_and_free;
					}
	...
	if (debug_fast_sync) {
		bsr_info(220, BSR_LC_RESYNC_OV, NO_OBJECT, "total free_blocks : %llu free_bits : %llu\n", free_blocks_co, free_bits_co);
	}
