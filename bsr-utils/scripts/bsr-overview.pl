...
# sets $bsr{minor}->{name} (and possibly ->{ll_dev})
sub map_minor_to_resource_names()
{
	my @bsradm_sh_status = `bsradm sh-status`;
	my ($ll_res, $ll_dev, $ll_minor, $conf_res, $conf_vnr, $minor, $name, $vnr);

	for (@bsradm_sh_status) {
		# volumes only present in >= 8.4
		# some things generated by bsradm
		...
		if (defined $conf_vnr) {
			...
			# from the config.
			$bsr{$minor}{name} .= defined $vnr ? "/$vnr" : "/$conf_vnr";
		}
		$minor_of_name{$name} = $minor;
		$bsr{$minor}{ll_dev} = defined($ll_dev) ? $ll_minor : $ll_res
			if $ll_res;
	}
	...
	for my $i (keys %bsr) {
		next unless exists $bsr{$i}->{ll_dev};
		my $lower = $bsr{$i}->{ll_dev};
		next if $lower =~ /^\d+$/;
		next unless exists $minor_of_name{$lower};
		$bsr{$i}->{ll_dev} = $minor_of_name{$lower};
	}
	# fix up to be able to report "lower dev of:"
	for my $i (keys %bsr) {
		next unless exists $bsr{$i}->{ll_dev};
		my $lower = $bsr{$i}->{ll_dev};
		$bsr{$lower}->{ll_dev_of} = $i;
	}
}

sub ll_dev_info {
	my $i = shift;
	( "ll-dev of:", $i, $bsr{$i}{name} )
}

# sets $bsr{minor}->{state} and (and possibly ->{sync})
sub slurp_proc_bsr_or_exit() {
	$_=<PD>;
	my ($BSR_VERSION) = /version: ([\d\.]+)/;
	@BSR_VERSION = split(/\./, $BSR_VERSION);
	my $minor;
	while (defined($_ = <PD>)) {
		chomp;
		/^ *(\d+):/ and do {
# skip unconfigured devices
			...
			if (/^ *(\d+): cs:Unconfigured/) {
				next
					unless exists $bsr{$minor}
				and exists $bsr{$minor}{name};
			}
			my $uc = /Unconfigured/ ? "." : undef;
			($bsr{$minor}{conn})   =  $uc || m{\bcs:(\w+)\b};
			($bsr{$minor}{role})   =  $uc || m{\bro:(\w+/\w+)\b};
			($bsr{$minor}{dstate}) =  $uc || m{\bds:(\w+/\w+)\b};
		};
		/^\t\[.*sync.ed:/ and do {
			$bsr{$minor}{sync} = $_;
		};
...
sub slurp_bsrsetup() {
    unless (open(DS,"bsrsetup events2 --now --statistics " .
                ($use_colors ? "--color=always " : "") .
                " |")) {
		print "bsrsetup not started\n";
		exit 0;
	}
...
# sets $bsr{minor}->{pv_info}
sub get_pv_info()
{
	for (`pvs --noheadings --units g -o pv_name,vg_name,pv_size,pv_used`) {
		m{^\s*/dev/bsr(\d+)\s+(\S+)\s+(\S+)\s+(\S+)\s*$} or next;
		#  PV  VG  PSize  Used
		$bsr{$1}{pv_info} = { vg => $2, size => $3, used => $4 };
	}
}
...
# sets $bsr{minor}->{df_info}
sub get_df_info()
{
	for (`df -TPhl -x tmpfs --local`) {
		#  Filesystem  Type  Size  Used  Avail  Use%  Mounted  on
		m{^/dev/bsr(\d+)\s+(\S+)\s+(\S+)\s+(\S+)\s+(\S+)\s+(\S+)\s+(\S+)} or next;
		$bsr{$1}{df_info} = { type => $2, size => $3, used => $4,
...
sub get_swap_info()
{
	...
	while (<$fd>) {
        ...
        #  /dev/bsr100             partition       262140  0       -1
		m{^/dev/bsr(\d+)\s+(\S+)\s+(\d+)\s+(\d+)} or next;
		$bsr{$1}{df_info} = { type => 'swap', size => $3, used => $4, };
	}
}

# sets $bsr{minor}->{xen_info}
sub get_xen_info()
{
	my $dom;
	my $running = 0;
...
sub get_virsh_info()
{
	...
	for my $dom (keys %info) {
		...
		for (m{<disk\ [^>]*>.*?</disk>}gs) {
			m{<source\ (?:dev|file)='/dev/bsr([^']+)'/>} or next;
			my $dev = $1;
			if ($dev !~ /^\d+$/) {
				my @stat = stat("/dev/bsr$dev") or next;
				$dev = $stat[6] & 0xff;
			}
...
# allow reading from a prepared file instead of /proc/bsr
...
unless (open(PD,$PROC_BSR)) {
    print "bsr not loaded\n";
    exit 0;
}

map_minor_to_resource_names;
slurp_proc_bsr_or_exit;

slurp_bsrsetup if $BSR_VERSION[0] >= 9;
...
my @minors_sorted = sort { $a <=> $b } keys %bsr;
