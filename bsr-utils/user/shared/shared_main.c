...
struct ifreq *get_ifreq(void) {
	...
	do {
		...
		if (NULL == (ifc.ifc_req = realloc(ifc.ifc_req, ifc.ifc_len))) {
			CLI_ERRO_LOG_STDERR( "Out of memory.\n");
			return NULL;
		}
				      ...
				      char *substitution)
{
	if (!strcmp(*c, deprecated)) {
		CLI_ERRO_LOG_STDERR( "'%s %s' is deprecated, use '%s %s' instead.\n",
...
void m__system(char **argv, int flags, const char *res_name, pid_t *kid, int *fd, int *ex)
{
	...
	if (pipe(pipe_fds) < 0) {
		perror("pipe");
		CLI_ERRO_LOG_STDERR( "Error in pipe, giving up.\n");
		exit(E_EXEC_ERROR);
	}

	pid = my_fork();
	if (pid == -1) {
		CLI_ERRO_LOG_STDERR( "Can not fork\n");
		exit(E_EXEC_ERROR);
	}
	if (pid == 0) {
#ifdef _LIN
		...
		if (flags & SUPRESS_STDERR) {
			...
			if (!f)
				...
				CLI_ERRO_LOG_STDERR( "reopen null service failed\n");
		}
		if (argv[0]) {
#ifdef _WIN
...
#define BSRADM_MAX_ENV_LEN		64	
...
#ifdef _WIN
		CLI_ERRO_LOG_STDERR( "Can not exec %s\n", argv[0]);
		perror("Failed");
#else // _LIN
		CLI_ERRO_LOG_STDERR( "Can not exec\n");
#endif
		exit(E_EXEC_ERROR);
	}
	...
	if (flags & SLEEPS_FINITE) {
		...
		case SLEEPS_VERY_LONG:
			...
			break;
		default:
			CLI_ERRO_LOG_STDERR( "logic bug in %s:%d\n", __FILE__,
				__LINE__);
			exit(E_THINKO);
		}
		alarm(timeout);
	...
	if (flags & SLEEPS_FINITE) {
		if (rv >= 10
			...
			CLI_ERRO_LOG_STDERR( "Command '");
			for (cmdline = argv; *cmdline; cmdline++) {
				CLI_ERRO_LOG_STDERR( "%s", *cmdline);
				if (cmdline[1])
					fputc(' ', stderr);
			}
			if (alarm_raised) {
