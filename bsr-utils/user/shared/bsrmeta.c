...
#define ASSERT(x) do { if (!(x)) {				\
	CLI_ERRO_LOG_STDERR(progname, "%s:%u:%s: ASSERT(%s) failed.\n",	\
...
static int confirmed(const char *text)
{
	...
	if (force) {
	    CLI_ERRO_LOG_STDERR(progname, "*** confirmation forced via --force option ***\n");
	    ok = 1;
	}
	else {
	    CLI_ERRO_LOG_STDERR(progname, "[need to type '%s' to confirm] ", yes);
...
int v06_validate_md(struct format *cfg)
{
	if (cfg->md.magic != BSR_MD_MAGIC_06) {
		CLI_ERRO_LOG_STDERR(progname, "v06 Magic number not found\n");
		return -1;
	}
	return 0;
}
	...
	const struct md_cpu * const md, const int md_index, const uint64_t ll_size)
{
	...
	if ((f == BSR_V07 && md->magic != BSR_MD_MAGIC_07) ||
		...
		if (verbose >= 1)
			CLI_ERRO_LOG_STDERR(progname, "%s Magic number not found\n", v);
		return 0;
	}

	if (md->max_peers < 1 || md->max_peers > BSR_PEERS_MAX) {
		CLI_ERRO_LOG_STDERR(progname, "%s max-peers value %d out of bounds\n",
			v, md->max_peers);
		return 0;
	}
	if (md->node_id < -1 || md->node_id > BSR_PEERS_MAX + 1) {
		CLI_ERRO_LOG_STDERR(progname, "%s device node-id value %d out of bounds\n",
			v, md->node_id);
		return 0;
	}
	for (n = 0; n < md->max_peers; n++) {
		if (md->peers[n].bitmap_index < -1 || md->peers[n].bitmap_index > BSR_PEERS_MAX + 1) {
			CLI_ERRO_LOG_STDERR(progname, "%s peer device %d node-id value %d out of bounds\n",
	...
	case BSR_MD_INDEX_FLEX_EXT:
		if (md->al_offset != MD_AL_OFFSET_07) {
			CLI_ERRO_LOG_STDERR(progname, "%s Magic number (al_offset) not found\n", v);
			CLI_ERRO_LOG_STDERR(progname, "\texpected: %d, found %d\n",
				MD_AL_OFFSET_07, md->al_offset);
			return 0;
		}
		if (md->bm_offset != MD_AL_OFFSET_07 + al_size_sect) {
			CLI_ERRO_LOG_STDERR(progname, "%s bm_offset: expected %d, found %d\n", v,
	...
	case BSR_MD_INDEX_FLEX_INT:
		if (md->al_offset != -al_size_sect) {
			CLI_ERRO_LOG_STDERR(progname, "%s al_offset: expected %d, found %d\n", v,
		...
		if (md->bm_offset != -(int64_t)md_size_sect + MD_AL_OFFSET_07) {
			CLI_ERRO_LOG_STDERR(progname, "strange bm_offset %d (expected: "D64")\n",
					md->bm_offset, -(int64_t)md_size_sect + MD_AL_OFFSET_07);
			return 0;
		};
		if (md->md_size_sect != md_size_sect) {
			CLI_ERRO_LOG_STDERR(progname, "strange md_size_sect %u (expected: "U64")\n",
...
void pread_or_die(struct format *cfg, void *buf, size_t count, off_t offset, const char* tag)
{
	...
	if(cfg->md_handle == INVALID_HANDLE_VALUE){
		CLI_ERRO_LOG_STDERR(progname, " pread_or_die : unable to use handle \n");
		exit(10); 
	}
	...
	if(err == FALSE){
		CLI_ERRO_LOG_STDERR(progname, "Unable to read from file.\n GetLastError=%08x\n", GetLastError()); 
	...
	if (verbose >= 2) {
		fflush(stdout);
		CLI_ERRO_LOG_STDERR(progname, " %-26s: pread(%u, ...,%6lu,%12llu)\n", tag,
			fd, (unsigned long)count, (unsigned long long)offset);
		if (count & ((1<<12)-1))
			CLI_ERRO_LOG_STDERR(progname, "\tcount will cause EINVAL on hard sect size != 512\n");
		if (offset & ((1<<12)-1))
			CLI_ERRO_LOG_STDERR(progname, "\toffset will cause EINVAL on hard sect size != 512\n");
	}
	if (c < 0) {
		CLI_ERRO_LOG_STDERR(progname, "pread(%u,...,%lu,%llu) in %s failed: %s\n",
			fd, (unsigned long)count, (unsigned long long)offset,
			tag, strerror(errno));
		exit(10);
	} else if ((size_t)c != count) {
		CLI_ERRO_LOG_STDERR(progname, "confused in %s: expected to read %d bytes,"
			" actually read %d\n",
			tag, (int)count, (int)c);
		exit(10);
	}
...
void validate_offsets_or_die(struct format *cfg, size_t count, off_t offset, const char* tag)
{
	...
	if (al_offset != cfg->al_offset)
		CLI_ERRO_LOG_STDERR(progname, "%s: ambiguous al_offset: "U64" vs %llu\n",
			tag, cfg->al_offset, (unsigned long long)al_offset);
	if (bm_offset != cfg->bm_offset)
		CLI_ERRO_LOG_STDERR(progname, "%s: ambiguous bm_offset: "U64" vs %llu\n",
	...
	if (min_offset < 0)
		CLI_ERRO_LOG_STDERR(progname, "%s: negative minimum offset: %lld\n", tag, (long long)min_offset);
	...
	if (offset < min_offset || (offset + count) > max_offset) {
		CLI_ERRO_LOG_STDERR(progname, "%s: offset+count ("U64"+%zu) not in meta data area range ["U64"; "U64"], aborted\n",
			tag, offset, count, min_offset, max_offset);
		if (ignore_sanity_checks) {
			CLI_ERRO_LOG_STDERR(progname, "Ignored due to --ignore-sanity-checks\n");
		} else {
			CLI_ERRO_LOG_STDERR(progname, "If you want to force this, tell me to --ignore-sanity-checks\n");
			exit(10);
		}
	}
}
...
void pwrite_or_die(struct format *cfg, const void *buf, size_t count, off_t offset, const char* tag)
{
	...
	if(cfg->md_handle == INVALID_HANDLE_VALUE){
		CLI_ERRO_LOG_STDERR(progname, " pwrite_or_die : unable to use handle \n"); 
		exit(10); 
	}
#else 	
	int fd = cfg->md_fd;
	...
	++n_writes;
	if (dry_run) {
		CLI_ERRO_LOG_STDERR(progname, " %-26s: pwrite(%u, ...,%6lu,%12llu) SKIPPED DUE TO DRY-RUN\n",
	...
	if(err == FALSE){
		CLI_ERRO_LOG_STDERR(progname, "Unable to write file. \n");
		CloseHandle(cfg->md_handle);	
	}
#else 	
	...
	if (verbose >= 2) {
		fflush(stdout);
		CLI_ERRO_LOG_STDERR(progname, " %-26s: pwrite(%u, ...,%6lu,%12llu)\n", tag,
			fd, (unsigned long)count, (unsigned long long)offset);
		if (count & ((1<<12)-1))
			CLI_ERRO_LOG_STDERR(progname, "\tcount will cause EINVAL on hard sect size != 512\n");
		if (offset & ((1<<12)-1))
	...
	if (c < 0) {
		CLI_ERRO_LOG_STDERR(progname, "pwrite(%u,...,%lu,%llu) in %s failed: %s\n",
			fd, (unsigned long)count, (unsigned long long)offset,
			tag, strerror(errno));
		exit(10);
	} else if ((size_t)c != count) {
		/* FIXME we might just now have corrupted the on-disk data */
		CLI_ERRO_LOG_STDERR(progname, "confused in %s: expected to write %d bytes,"
			" actually wrote %d\n", tag, (int)count, (int)c);
		exit(10);
	}
}
...
{
	if (offset >= limit) {
		CLI_ERRO_LOG_STDERR(progname, "confused in %s: offset (%llu) > limit (%llu)\n",
			tag, (unsigned long long)offset, (unsigned long long)limit);
		exit(10);
	}
	if (count > limit - offset) {
		CLI_ERRO_LOG_STDERR(progname, "in %s: truncating byte count from %lu to %lu\n", tag,
				(unsigned long)count,
...
int m_strsep_u32(char **s, uint32_t *val)
{
	...
	if ((t = strsep(s, ":"))) {
		if (strlen(t)) {
			...
			v = strtoul(t, &e, 0);
			if (*e != 0) {
				CLI_ERRO_LOG_STDERR(progname, "'%s' is not a number.\n", *s);
				exit(10);
			}
			if (errno) {
...
int m_strsep_bit(char **s, uint32_t *val, int mask)
{
	...
	if (d > 1) {
		CLI_ERRO_LOG_STDERR(progname, "'%d' is not 0 or 1.\n", d);
		exit(10);
	}
...
{
	CLI_ERRO_LOG_STDERR(progname, "Can not outdate GC based meta data!\n");
...
int v06_parse(struct format *cfg, char **argv, int argc, int *ai)
{
	...
	if (argc < 1) {
		CLI_ERRO_LOG_STDERR(progname, "Too few arguments for format\n");
		exit(20);
	}
	...
	if (*e != 0 || minor > 255UL) {
		CLI_ERRO_LOG_STDERR(progname, "'%s' is not a valid minor number.\n", argv[0]);
		exit(20);
	}
	if (asprintf(&e, "%s/bsr%lu", BSR_LIB_DIR, minor) <= 18) {
		CLI_ERRO_LOG_STDERR(progname, "asprintf() failed.\n");
		exit(20);
	};
...
int v06_md_open(struct format *cfg)
{
	...
	if (!S_ISREG(sb.st_mode)) {
		CLI_ERRO_LOG_STDERR(progname, "'%s' is not a plain file!\n",
			cfg->md_device_name);
		return NO_VALID_MD_FOUND;
	}
...
void re_initialize_md_offsets(struct format *cfg)
{
	...
	case BSR_MD_INDEX_FLEX_INT:
		...
		if (md_size_sect > (MD_BM_MAX_BYTE_FLEX>>9)) {
			CLI_ERRO_LOG_STDERR(progname, "Bitmap for that device got too large.\n");
			if (BITS_PER_LONG == 32)
				CLI_ERRO_LOG_STDERR(progname, "Maybe try a 64bit arch?\n");
			exit(10);
		}
		/* plus the "bsr meta data super block",
		 * and the activity log; unit still sectors */
	...
	if (verbose >= 2) {
		CLI_ERRO_LOG_STDERR(progname, "md_offset: "U64"\n", cfg->md_offset);
		CLI_ERRO_LOG_STDERR(progname, "al_offset: "U64" (%d)\n", cfg->al_offset, cfg->md.al_offset);
		CLI_ERRO_LOG_STDERR(progname, "bm_offset: "U64" (%d)\n", cfg->bm_offset, cfg->md.bm_offset);
		CLI_ERRO_LOG_STDERR(progname, "md_size_sect: "U32"\n", cfg->md.md_size_sect);
		CLI_ERRO_LOG_STDERR(progname, "max_usable_sect: "U64"\n", cfg->max_usable_sect);
	}
}
...
static void zeroout_bitmap(struct format *cfg)
{
	...
	CLI_ERRO_LOG_STDERR(progname, "initializing bitmap (%u KB) to all zero\n",
...
#endif
	CLI_ERRO_LOG_STDERR(progname, "Using slow(er) fallback.\n");
	{
		...
		while (i) {
			...
			if (percent_done != percent_last_report) {
				CLI_ERRO_LOG_STDERR(progname, "\r%u%%", percent_done);
				percent_last_report = percent_done;
			}
		}
		CLI_ERRO_LOG_STDERR(progname, "\r100%%\n");	
	}
}
...
int md_initialize_common(struct format *cfg, int do_disk_writes)
{
	...
	if (MD_AL_MAX_SECT_07 * 512 > buffer_size) {
		CLI_ERRO_LOG_STDERR(progname, "%s:%u: LOGIC BUG\n", __FILE__, __LINE__);
		exit(111);
	}
	initialize_al(cfg);
...
#define AL_EXTENTS_PT 61
	...
	if (!found_valid) {
		...
		CLI_ERRO_LOG_STDERR(progname, "No usable activity log found.\n");
	...
	if (found_valid != mx) {
		CLI_ERRO_LOG_STDERR(progname, "%u corrupt or uninitialized AL transactions found\n", mx - found_valid);
		CLI_ERRO_LOG_STDERR(progname, "You can safely ignore this if this node was cleanly stopped (no crash).\n");
	}
	...
	for (b = oldest.i; b <= newest.i; b++) {
		...
		for (i = AL_EXTENTS_PT; (int)i >= 0; i--) {
			...
			if (slot >= AL_EXTENTS_MAX) {
				CLI_ERRO_LOG_STDERR(progname, "slot number out of range: tr:%u slot:%u\n",
						idx, slot);
				continue;
			}
...
int replay_al_84(struct format *cfg, uint32_t *hot_extent)
{
	...
	if (!al_cpu) {
		CLI_ERRO_LOG_STDERR(progname, "Could not calloc(%u, sizeof(*al_cpu))\n", mx);
	...
	if (!found_valid) {
		/* not even one transaction was valid.
		 * Has this ever been initialized correctly? */
		CLI_ERRO_LOG_STDERR(progname, "No usable activity log found. Do you need to create-md?\n");
		free(al_cpu);
		return -ENODATA;
	}
	 ...
	 * things went wrong during transaction write. */
	if (found_valid != mx)
		CLI_ERRO_LOG_STDERR(progname, "%u corrupt AL transactions found\n", mx - found_valid);
	...
	if (!found_valid_updates) {
		...
		CLI_ERRO_LOG_STDERR(progname, "No valid AL update transaction found.\n");
		return -EINVAL;
	}
	...
	for (b = oldest.i; b <= newest.i; b++) {
		...
		for (i = 0; i < AL_UPDATES_PER_TRANSACTION; i++) {
			...
			if (i >= al_cpu[idx].n_updates && slot == UINT16_MAX)
				continue;
			if (slot >= AL_EXTENTS_MAX) {
				CLI_ERRO_LOG_STDERR(progname, "update slot number out of range: tr:%u slot:%u\n",
						idx, slot);
				continue;
			}
...
void apply_al(struct format *cfg, uint32_t *hot_extent)
{
	...
	for (i = 0; i < AL_EXTENTS_MAX; i++) {
		...
		if (bm_pos >= bm_bytes) {
			CLI_ERRO_LOG_STDERR(progname, "extent %u beyond end of bitmap!\n", hot_extent[i]);
	...
	if (i != 0) {
		pwrite_or_die(cfg, on_disk_buffer, chunk,
				bm_on_disk_off + bm_on_disk_pos,
				"apply_al");
		CLI_ERRO_LOG_STDERR(progname, "Marked additional %s as out-of-sync based on AL.\n",
			ppsize(ppb, additional_bits_set * 4));
	} else
		CLI_ERRO_LOG_STDERR(progname, "Nothing to do.\n");
}
...
int need_to_apply_al(struct format *cfg)
{
	...
	case BSR_UNKNOWN:
		CLI_ERRO_LOG_STDERR(progname, "BUG in %s().\n", __FUNCTION__);
	}
	return 0;
}
...
int meta_apply_al(struct format *cfg, char **argv __attribute((unused)), int argc)
{
	...
	if (argc > 0)
		CLI_ERRO_LOG_STDERR(progname, "Ignoring additional arguments\n");

	if (format_version(cfg) < BSR_V07) {
		CLI_ERRO_LOG_STDERR(progname, "apply-al only implemented for BSR >= 0.7\n");
		return -1;
	}
	...
	if (err != VALID_MD_FOUND) {
		CLI_ERRO_LOG_STDERR(progname, "No valid meta data found\n");
		return -1;
	}
	...
	if (err < 0) {
		...
		if (need_to_apply_al(cfg)) {
			...
			CLI_ERRO_LOG_STDERR(progname, "Error ignored, no need to apply the AL\n");
			re_initialize_anyways = 1;
		}
	}
	...
	if (need_to_update_md_flags) {
		...
		if (err)
			CLI_ERRO_LOG_STDERR(progname, "update of super block flags failed\n");
	}
		...
		if (!ret) {
			...
			switch (err) {
				...
				case ERROR_NOT_A_REPARSE_POINT:	// pass through intentionally
					break;
				default:
					//CLI_ERRO_LOG_STDERR(progname, "GetVolumeNameForVolumeMountPoint() failed err(%d)\n", err);
					return NULL;
			}
			// DW-1423 retrieve path.
			wdn = (char*)malloc(strlen(temp) + 5);
			if (!wdn) {
				CLI_ERRO_LOG_STDERR(progname, "malloc failed while getting directory path for mounting point\n");
				return NULL;
			}
			strcpy(wdn, "\\\\.\\");
			strcat(wdn, temp);
 ...
 retry:
	...
	if (cfg->md_fd == -1) {
		...
		if (save_errno == ENOENT && cfg->vhd_dev_path &&
			...
			if (!_attach_vhd_script(cfg->vhd_dev_path)) {
				...
				CLI_ERRO_LOG_STDERR(progname, "Attaching %s for meta data\n", cfg->vhd_dev_path);
				if (_call_script(_argv)) {
					remove("./"ATTACH_VHD_SCRIPT);
					CLI_ERRO_LOG_STDERR(progname, "diskpart attach failed.\n");
					exit(20);
				}
				remove("./"ATTACH_VHD_SCRIPT);
				goto retry;
		...
		if (save_errno == ENOENT && cfg->loop_file_path) {
			...
			CLI_ERRO_LOG_STDERR(progname, "set up the loop device (%s)\n", cfg->md_device_name);
			loopctlfd = open("/dev/loop-control", O_RDWR);
			if (loopctlfd == -1) {
				CLI_ERRO_LOG_STDERR(progname, "failed to open /dev/loop-control\n");
				exit(20);
			}
			...
			close(loopctlfd);
			if (err != devnr) {
				CLI_ERRO_LOG_STDERR(progname, "failed to add the new loop device (%s)\n", cfg->md_device_name);
				exit(20);
			}
			goto retry;
		}
		...
		if (save_errno == EINVAL && (open_flags & O_DIRECT)) {
			...
			CLI_ERRO_LOG_STDERR(progname, "could not open with O_DIRECT, retrying without\n");
			open_flags &= ~O_DIRECT;
			opened_odirect = 0;
			goto retry;
		}
	...
	if (cfg->loop_file_path) {
		...
		if (ioctl(cfg->md_fd, LOOP_GET_STATUS, &info) != 0) {
			...
			if (file_fd < 0) {
				CLI_ERRO_LOG_STDERR(progname, "failed to open mete file (%s)\n", cfg->loop_file_path);
				exit(20);
			}

			// associate the loop device with the open file.			
			if (ioctl(cfg->md_fd, LOOP_SET_FD, file_fd) != 0) {
				CLI_ERRO_LOG_STDERR(progname, "failed to associate the loop device (%s) with the meta file (%s)\n", 
...
#endif
	if (ioctl_err) {
		CLI_ERRO_LOG_STDERR(progname, "ioctl(md_fd, BLKSSZGET) returned %d, "
		...
		if (verbose >= 2)
			CLI_ERRO_LOG_STDERR(progname, "hard_sect_size is %d Byte\n",
	...
	if (cfg->bd_size < SO_MUCH) {
		CLI_ERRO_LOG_STDERR(progname, "%s is only %llu bytes. That's not enough.\n",
	...
	if (!opened_odirect &&
		...
		if (ioctl_err)
			CLI_ERRO_LOG_STDERR(progname, "ioctl(md_fd, BLKFLSBUF) returned %d, "
...
int v07_parse(struct format *cfg, char **argv, int argc, int *ai)
{
	...
	if (!strcmp(argv[1],"internal")) {
		...
		if (*e != 0 || 0 > index || index > 255 || errno != 0) {
			CLI_ERRO_LOG_STDERR(progname, "'%s' is not a valid index number.\n", argv[1]);
			return -1;
		}
	}
	cfg->md_index = index;
...
void v08_check_for_resize(struct format *cfg)
{
	...
	if (lk_bdev_load(cfg->minor, &cfg->lk_bd)) {
		if (verbose)
			CLI_ERRO_LOG_STDERR(progname, "no last-known offset information available.\n");
		return;
	}

	if (verbose) {
		CLI_ERRO_LOG_STDERR(progname, " last known info: %llu %s\n",
			(unsigned long long)cfg->lk_bd.bd_size,
			cfg->lk_bd.bd_name ?: "-unknown device name-");
		if (cfg->lk_bd.bd_uuid)
			CLI_ERRO_LOG_STDERR(progname, " last known uuid: "X64(016)"\n",
				cfg->lk_bd.bd_uuid);
	}
	...
	if (verbose) {
		CLI_ERRO_LOG_STDERR(progname, "While checking for internal meta data for bsr%u on %s,\n"
		...
		if (cfg->lk_bd.bd_name &&
			...
			CLI_ERRO_LOG_STDERR(progname, "on %s ", cfg->lk_bd.bd_name);
		}
		CLI_ERRO_LOG_STDERR(progname, "at byte offset %llu", (unsigned long long)flex_offset);

		if (!found) {
			CLI_ERRO_LOG_STDERR(progname, ", but I cannot find it now.\n");
			if (flex_offset >= cfg->bd_size)
				CLI_ERRO_LOG_STDERR(progname, "Device is too small now!\n");
		} else
			CLI_ERRO_LOG_STDERR(progname, ", and seems to still be valid.\n");
	}

	if (found) {
		if (cfg->lk_bd.bd_uuid && md_test.device_uuid != cfg->lk_bd.bd_uuid) {
			CLI_ERRO_LOG_STDERR(progname, "Last known and found uuid differ!?\n"
					X64(016)" != "X64(016)"\n",
					cfg->lk_bd.bd_uuid, cfg->md.device_uuid);
			if (!force) {
				found = 0;
				CLI_ERRO_LOG_STDERR(progname, "You may --force me to ignore that.\n");
			} else
				CLI_ERRO_LOG_STDERR(progname, "You --force'ed me to ignore that.\n");
		}
	}
	if (found)
		cfg->md = md_test;
...
int meta_set_gi(struct format *cfg, char **argv, int argc)
{
	...
	if (argc < 1) {
		CLI_ERRO_LOG_STDERR(progname, "Required Argument missing\n");
		exit(10);
	}
	...
	err = cfg->ops->close(cfg) || err;
	if (err)
		CLI_ERRO_LOG_STDERR(progname, "update failed\n");
...
int meta_dump_md(struct format *cfg, char **argv __attribute((unused)), int argc)
{
	...
	if (!al_is_clean) {
		CLI_ERRO_LOG_STDERR(progname, "Found meta data is \"unclean\", please apply-al first\n");
		if (!force)
			return -1;
	}
...
void md_parse_error(int expected_token, int seen_token,const char *etext)
{
	...
	fflush(stdout);
	CLI_ERRO_LOG_STDERR(progname, "Parse error in line %u: %s%s",
		yylineno, etext,
		(etext[0] == ' ' ? ":" : " expected")
		);
	...
	switch(seen_token) {
	case 0:
		CLI_ERRO_LOG_STDERR(progname, ", but end of file encountered\n"); break;
	...
	case 126 ... 257:
		...
		CLI_ERRO_LOG_STDERR(progname, "; got token value %u (this should never happen!)\n", seen_token); break;
		break;

	case TK_INVALID_CHAR:
		CLI_ERRO_LOG_STDERR(progname, "; got invalid input character '\\x%02x' [%c]\n",
			(unsigned char)yylval.txt[0], yylval.txt[0]);
		break;
	case ';': case '{': case '}':
		CLI_ERRO_LOG_STDERR(progname, ", not '%c'\n", seen_token); break;
	case TK_NUM:
	case TK_U32:
	case TK_U64:
		CLI_ERRO_LOG_STDERR(progname, ", not some number\n"); break;
	case TK_INVALID:
		/* already reported by scanner */
		CLI_ERRO_LOG_STDERR(progname, "\n"); break;
	default:
		CLI_ERRO_LOG_STDERR(progname, ", not '%s'\n", yylval.txt);
	}
	exit(10);
}
...
int parse_bitmap_window_one_peer(struct format *cfg, int window, int peer_nr, int parse_only)
{
	...
	if (format_version(cfg) < BSR_V09)
		...
		if (yylval.u64 != peer_nr) {
			CLI_ERRO_LOG_STDERR(progname, "Parse error in line %u: "
		...
		if (words > 0 && !truncated) {
			...
			if (parse_only) {
				...
				c = pwrite_with_limit_or_die(cfg, on_disk_buffer,
				      ...
				      "meta_restore_md");
			if (s != c) {
				CLI_ERRO_LOG_STDERR(progname, "Bitmap info too large, truncated!\n");
...
int verify_dumpfile_or_restore(struct format *cfg, char **argv, int argc, int parse_only)
{
	...
	if (argc > 0) {
		yyin = fopen(argv[0],"r");
		if(yyin == NULL) {
			CLI_ERRO_LOG_STDERR(progname, "open of '%s' failed.\n",argv[0]);
			exit(20);
		}
	}
	...
	if(strcmp(yylval.txt,cfg->ops->name)) {
		CLI_ERRO_LOG_STDERR(progname, "dump is '%s' you requested '%s'.\n",
	...
	if (!parse_only) {
		CLI_ERRO_LOG_STDERR(progname, "reinitializing\n");
	...
	if (format_version(cfg) >= BSR_V08) {
		...
		if (yylval.u64 < 4096 || (yylval.u64 & (yylval.u64 -1)) != 0) {
			CLI_ERRO_LOG_STDERR(progname, "Invalid value for bm-byte-per-bit: "
	...
	if (option_al_stripes != cfg->md.al_stripes ||
		...
		if (option_al_stripes_used) {
			CLI_ERRO_LOG_STDERR(progname, "override activity log striping from commandline\n");
			cfg->md.al_stripes = option_al_stripes;
			cfg->md.al_stripe_size_4k = option_al_stripe_size_4k;
		}
		if (verbose >= 2)
			CLI_ERRO_LOG_STDERR(progname, "adjusting activity-log and bitmap offsets\n");
		re_initialize_md_offsets(cfg);
	}
	...
	err = cfg->ops->close(cfg) || err;
	if (err) {
		CLI_ERRO_LOG_STDERR(progname, "Writing failed\n");
		return -1;
	}
...
void md_convert_07_to_08(struct format *cfg)
{
	...
	if (!is_valid_md(BSR_V08, &cfg->md, cfg->md_index, cfg->bd_size)) {
		CLI_ERRO_LOG_STDERR(progname, "Conversion failed.\nThis is a bug :(\n");
		exit(111);
	}
}
...
void convert_md(struct format *cfg, enum md_format from)
{
	...
	case BSR_UNKNOWN:
	case BSR_V06:
		CLI_ERRO_LOG_STDERR(progname, "BUG in %s() %d.\n", __FUNCTION__, __LINE__);
		exit(10);
	case BSR_V07:
		switch(from) {
		case BSR_V09:
...
int guessed_size_from_pvs(struct fstype_s *f, char *dev_name)
{
	...
	if (!ret) {
		for (i = 0; i < N_ERR_LINES; i++) {
			...
			if (b[0] == 0)
				continue;
			CLI_ERRO_LOG_STDERR(progname, "pvs stderr:%s", b);
		}
		CLI_ERRO_LOG_STDERR(progname, "\n");
	}
...
void check_internal_md_flavours(struct format * cfg) {
	...
	CLI_ERRO_LOG_STDERR(progname, "You want me to create a %s%s style %s internal meta data block.\n",
		cfg->ops->name,
		(is_v07(cfg) && cfg->md_index == BSR_MD_INDEX_FLEX_INT) ? "(plus)" : "",
		cfg->md_index == BSR_MD_INDEX_FLEX_INT ? "flexible-size" : "fixed-size");


	CLI_ERRO_LOG_STDERR(progname, "There appears to be a %s %s internal meta data block\n"
...
int v08_move_internal_md_after_resize(struct format *cfg)
{
	...
	CLI_ERRO_LOG_STDERR(progname, "Moving the internal meta data to its proper location\n");

	if (verbose >= 2) {
		CLI_ERRO_LOG_STDERR(progname, "old md_offset: "U64"\n", old_offset);
		CLI_ERRO_LOG_STDERR(progname, "old al_offset: %llu (%d)\n", old_offset + md_old.al_offset * 512LL, md_old.al_offset);
		CLI_ERRO_LOG_STDERR(progname, "old bm_offset: %llu (%d)\n", old_offset + md_old.bm_offset * 512LL, md_old.bm_offset);

		CLI_ERRO_LOG_STDERR(progname, "new md_offset: "U64"\n", cfg->md_offset);
		CLI_ERRO_LOG_STDERR(progname, "new al_offset: "U64" (%d)\n", cfg->al_offset, cfg->md.al_offset);
		CLI_ERRO_LOG_STDERR(progname, "new bm_offset: "U64" (%d)\n", cfg->bm_offset, cfg->md.bm_offset);
	...
	err = cfg->ops->close(cfg) || err;
	if (err)
		CLI_ERRO_LOG_STDERR(progname, "operation failed\n");
...
int meta_create_md(struct format *cfg, char **argv __attribute((unused)), int argc)
{
	...
	if (is_v09(cfg)) {
		if (argc < 1) {
			CLI_ERRO_LOG_STDERR(progname, "USAGE: %s MINOR v09 ... create-md MAX_PEERS\n"
				"\n"
	...
	if (max_peers < 1 || max_peers > BSR_PEERS_MAX) {
		CLI_ERRO_LOG_STDERR(progname, "MAX_PEERS argument not in allowed range 1 .. %d.\n", BSR_PEERS_MAX);
		exit(20);
	}
#ifdef _WIN_VHD_META_SUPPORT
	...
	if (cfg->vhd_dev_path && (F_OK != access_ret || opendir_ret)) {
		...
		if (!_create_vhd_script(cfg->vhd_dev_path, evsm, cfg->md_device_name)) {
			...
			CLI_ERRO_LOG_STDERR(progname, "Creating %s for meta data...\n", cfg->vhd_dev_path);
			if (_call_script(_argv)) {
				remove("./"CREATE_VHD_SCRIPT);
				CLI_ERRO_LOG_STDERR(progname, "diskpart create failed.\n");
				exit(20);
			}
			remove("./"CREATE_VHD_SCRIPT);
		}
	...
	if (err == VALID_MD_FOUND_AT_LAST_KNOWN_LOCATION) {
		if (option_al_stripes_used) {
			if (option_al_stripes != cfg->md.al_stripes
			||  option_al_stripe_size_4k != cfg->md.al_stripe_size_4k) {
				CLI_ERRO_LOG_STDERR(progname, "Cannot move after offline resize and change AL-striping at the same time, yet.\n");
				exit(20);
			}
		}
		if (!force &&
...
int meta_wipe_md(struct format *cfg, char **argv __attribute((unused)), int argc)
{
	...
	if (virgin) {
		CLI_ERRO_LOG_STDERR(progname, "There appears to be no bsr meta data to wipe out?\n");
		return 0;
	}
...
int meta_outdate(struct format *cfg, char **argv __attribute((unused)), int argc)
{
	...
	if (cfg->ops->outdate_gi(&cfg->md)) {
		CLI_ERRO_LOG_STDERR(progname, "Device is inconsistent.\n");
		exit(5);
	}
...
int parse_format(struct format *cfg, char **argv, int argc, int *ai)
{
	...
	if (argc < 1) {
		CLI_ERRO_LOG_STDERR(progname, "Format identifier missing\n");
		return -1;
	}
	...
	if (f == BSR_UNKNOWN) {
		CLI_ERRO_LOG_STDERR(progname, "Unknown format '%s'.\n", argv[0]);
		return -1;
	}
...
static enum bsr_disk_state bsr_str_disk(const char *str)
{
	...
	CLI_ERRO_LOG_STDERR(progname, "Unexpected output from bsrsetup >%s<\n", str);
	exit(20);
}
...
int is_attached(int minor)
{
	...
	if (pid == 0) {
		...
		if (!f)
			...
			CLI_ERRO_LOG_STDERR(progname, "open null service failed\n");

		close(pipes[0]);
		dup2(pipes[1], 1);

		execvp(argv[0], argv);
		CLI_ERRO_LOG_STDERR(progname, "Can not exec bsrsetup\n");
		exit(20);
	}
	close(pipes[1]);
	...
	if (err == VALID_MD_FOUND) {
		...
		*/
		...
		if (format_version(cfg) < BSR_V08 || cfg->md_index != BSR_MD_INDEX_FLEX_INT) {
			CLI_ERRO_LOG_STDERR(progname, "Operation only supported for >= v8 internal meta data\n");
			return -1;
		}
		CLI_ERRO_LOG_STDERR(progname, "no suitable meta data found :(\n");
		return -1; /* sorry :( */
	}
...
struct format *new_cfg()
{
	...
	if (!cfg) {
		CLI_ERRO_LOG_STDERR(progname, "could not calloc() cfg\n");
		exit(20);
	}
	err = posix_memalign(&on_disk_buffer, pagesize, ALIGN(buffer_size, pagesize));
	if (err) {
		CLI_ERRO_LOG_STDERR(progname, "could not posix_memalign() on_disk_buffer\n");
		exit(20);
	}
	return cfg;
}
...
int main(int argc, char **argv)
{
	...
	if (sizeof(struct md_on_disk_07) != 4096) {
		CLI_ERRO_LOG_STDERR(progname, "Where did you get this broken build!?\n"
	...
	while (1) {
	    ...
	    case 'p':
			...
			option_peer_max_bio_size > 1024 * 1024) {
				CLI_ERRO_LOG_STDERR(progname, "peer-max-bio-size out of range (0...1M)\n");
			    exit(10);
		    }
		    break;
	    case 'i':
		    ...
		    if (option_node_id < 0 || option_node_id > (BSR_PEERS_MAX - 1)) {
				CLI_ERRO_LOG_STDERR(progname, "node-id out of range (0...%d)\n", BSR_PEERS_MAX - 1);
			    exit(10);
		    }
		    break;
	    case 's':
	...
	if (ai >= argc) {
		CLI_ERRO_LOG_STDERR(progname, "command missing\n");
		exit(20);
	}
	...
	if (command == NULL) {
		CLI_ERRO_LOG_STDERR(progname, "Unknown command '%s'.\n", argv[ai]);
		exit(20);
	}
	ai++;
	...
	if (strcmp(cfg->bsr_dev_name, "-")) {
		...
		if (minor_attached && command->modifies_md) {
			CLI_ERRO_LOG_STDERR(progname, "Device '%s' is configured!\n",
				cfg->bsr_dev_name);
			exit(20);
		}
	} else {
	...
	if (option_peer_max_bio_size &&
		...
		CLI_ERRO_LOG_STDERR(progname, "The --peer-max-bio-size option is only allowed with create-md\n");
		exit(10);
	}
	if (option_al_stripes_used &&
	    command->function != &meta_create_md &&
	    command->function != &meta_restore_md) {
		CLI_ERRO_LOG_STDERR(progname, "The --al-stripe* options are only allowed with create-md and restore-md\n");
		exit(10);
	}
	...
	if ((uint64_t)option_al_stripes * option_al_stripe_size_4k > (buffer_size/4096)) {
		CLI_ERRO_LOG_STDERR(progname, "invalid (too large) al-stripe* settings\n");
		    exit(10);
	}
	if (option_al_stripes * option_al_stripe_size_4k < 32/4) {
		CLI_ERRO_LOG_STDERR(progname, "invalid (too small) al-stripe* settings\n");
		    exit(10);
	}

	if (option_node_id != -1 && !command->node_id_required) {
		CLI_ERRO_LOG_STDERR(progname, "The %s command does not accept the --node-id option\n",
			command->name);
		exit(10);
	}
	...
	if (!is_v09(cfg) && command->node_id_required) {
		if (option_node_id == -1)
			...
			CLI_ERRO_LOG_STDERR(progname, "Not v09, implicitly set --node-id = 0\n");
	}

	if (option_node_id == -1 && command->node_id_required) {
		CLI_ERRO_LOG_STDERR(progname, "The %s command requires the --node-id option\n",
			command->name);
		exit(10);
	}
	...
	if (minor_attached)
		CLI_ERRO_LOG_STDERR(progname, "# Output might be stale, since minor %d is attached\n", cfg->minor);
