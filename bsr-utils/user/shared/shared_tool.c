...
DWORD _GetVolumeSize(LPSTR device_name, GET_LENGTH_INFORMATION * pgli)
{
	...
	if (INVALID_HANDLE_VALUE == hDevice) {
		err = GetLastError();
		CLI_ERRO_LOG_STDERR(false, "Failed to CreateFile(%s). error_code=%d", device_name, err);
		return err;
	}
	...
	if (!bResult) {
		err = GetLastError();
		CLI_ERRO_LOG_STDERR(false,  "Failed to IOCTL_DISK_GET_LENGTH_INFO(%s) error_code=%d", device_name, err);
		CloseHandle(hDevice);
		return err;
	}
...
DWORD _GetDriveGeometry(LPSTR device_name, DISK_GEOMETRY *pdg)
{
	...
	if (!bResult) {
		err = GetLastError();
		CLI_ERRO_LOG_STDERR(false,  "Failed to IOCTL_DISK_GET_DRIVE_GEOMETRY_EX(%s) error_code=%d", device_name, err);
		CloseHandle(hDevice);
		return err;
	}
...
int lk_bdev_save(const unsigned minor, const struct bdev_info *bd)
{
 ...
 fail_no_fp:
	if (ok <= 0)
		...
		CLI_ERRO_LOG_STDERR(false,  "lk_bdev_save(%s) failed: %m", path);
...
int lk_bdev_delete(const unsigned minor)
{
	...
	int rc = unlink(path);
	if (rc && errno != ENOENT)
		CLI_ERRO_LOG_STDERR(false,  "lk_bdev_delete(%s) failed: %m", path);
	free(path);
	return rc;
}
...
int lk_bdev_load(const unsigned minor, struct bdev_info *bd)
{
	...
	if (!fp) {
		if (errno != ENOENT)
			CLI_ERRO_LOG_STDERR(false,  "lk_bdev_load(%s) failed: %m", path);
		goto out;
	}
	...
	if (rc < 1) {
		CLI_ERRO_LOG_STDERR(false,  "lk_bdev_load(%s): parse error", path);
		rc = -1;
	} else
		rc = 0;
...
m_strtoll(const char *s, const char def_unit)
{
	...
	case MSE_DEFAULT_UNIT:
		CLI_ERRO_LOG_STDERR(false,  "unexpected default unit: %d",def_unit);
		exit(100);
	case MSE_MISSING_NUMBER:
		CLI_ERRO_LOG_STDERR(false,  "missing number argument");
		exit(100);
	case MSE_INVALID_NUMBER:
		CLI_ERRO_LOG_STDERR(false,  "%s is not a valid number", s);
		exit(20);
	case MSE_INVALID_UNIT:
		CLI_ERRO_LOG_STDERR(false,  "%s is not a valid number", s);
		exit(20);
	case MSE_OUT_OF_RANGE:
		CLI_ERRO_LOG_STDERR(false,  "%s: out of range", s);
		exit(20);
	default:
...
int get_fd_lockfile_timeout(const char *path, int seconds)
{
	...
	return 1;
#else // _LIN
	CLI_ERRO_LOG_STDERR(false, "open(%s): %m",path);
	return -1;
#endif
    }
	...
	close(fd);
	errno = err;
	CLI_ERRO_LOG_STDERR(false, "fcntl(%s,...): %m", path);
	return -1;
    }
