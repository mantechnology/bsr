...
int do_proxy_conn_up(const struct cfg_ctx *ctx)
{
	...
	for_each_connection(conn, &ctx->res->connections) {
		...
		argv[2] = ssprintf(
				...
				conn_name,
				path->my_proxy->public_inside.addr ? path->my_proxy->public_inside.addr : path->my_proxy->inside.addr,
				path->my_proxy->public_inside.port ? path->my_proxy->public_inside.port : path->my_proxy->inside.port,
				...
				path->my_proxy->public_outside.addr ? path->my_proxy->public_outside.addr : path->my_proxy->outside.addr,
				path->my_proxy->public_outside.port ? path->my_proxy->public_outside.port : path->my_proxy->outside.port,
